...
**Dernière révision du plan** : 7 avril 2026 — ajout **lot G** (sauvegardes chiffrées, API et backoffice admin, délocalisation, PCA/PRI) ; colonnes **État** / **Validé (porteur)** ; synthèses % par lot ; **pas de PR** tant que le porteur ne l’a pas demandé dans la conversation ; validation usage **uniquement** par vous ; **`make tests`** : **lot F1** / **`STATUS.md`**.
...
|---|--------|------|------------------|------------------|
| A1 | **Monitoring détaillé par service** : CPU / mémoire / réseau / disque avec **précision**, **historique** (snapshots dispo + session), **auto-rafraîchissement** (10–60 s), **PIDs** / **Block I/O** documentés ; sonde HTTP santé via **nom conteneur Docker** (plus `localhost` depuis l’agrégateur) ; axes graphiques en **heure locale** + jour si série &gt; 24 h | Partiel (07/04/2026) | Non | `frontend/.../services/[serviceName]/page.tsx` ; `metrics-aggregator-service` (`docker.routes.js`, `server.js`) ; **`make status-watch`** pour statut détaillé en boucle |
...
| A3 | Corréler logs techniques et événements sécurité dans les vues détail service | À faire | Non | `(development)/services/**`, `/backoffice/services/...` |
...
| C2 | Cohérence base principale vs base test (sans supprimer `admin@jobbingtrack.com`) | À faire | Non | Makefile, `docs/database/MIGRATIONS_ET_BASES.md` |
...
| # | Tâche | État | Validé (porteur) | Fichiers / notes |
|---|--------|------|------------------|----------------|
| F1 | Tests ciblés API + E2E (monitoring / services, sécurité, backoffice, suivi-intérim, logs) | Partiel | Non | **`make tests`** (= `test-all`, script `run-all-tests-with-reports.sh`) : **exige la stack Docker** (`make up-full`, `make db-push-all`, seed auth si besoin, MailHog si tests email). Sans stack, échecs massifs **attendus** — voir **`STATUS.md`** (dernier run documenté) et **`ERRORS.md`**. Jest gate : `npm run test:unit-and-analytics` ; audit : `npm run test:audit-jest-scope`. Rapports : **`tests/results/<horodatage>/`** |
