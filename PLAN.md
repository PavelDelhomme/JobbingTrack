...
**Dernière révision du plan** : 7 avril 2026 — **A1** : **`/service/:name/history`** (fichiers + BDD) ; **socle graphes** avancé — **`serviceHistoryChartModel.ts`** + **`useServiceHistoryChartData`** ; détail **sous-lots A1a–A1g** dans **`TODOS.md`**. **A5** + graphiques analytics/statistique : toujours **à poursuivre**. **B (roadmap)** : **B11** / **B12**. **D** : analytics utilisateur. *(23/04 : Block I/O **aggregated** ; **B** 7/04 ; 22/04 : **B6**, **STATS**, **A2** ; **C1** ; **F1**.)*
...
| A3 | Corréler logs techniques et événements sécurité dans les vues détail service | Partiel (07/04/2026) | Non | Encart liens **sécurité** + **logs multi-services** sur `/backoffice/services/[nom]` ; corrélation données unifiée (timeline / filtres croisés) encore à faire. **30/04 (corrélation perf)** : table incidents alignée aux points métriques + tri/filtres ; **05/2026 (forensics partiel)** : services auth/dashboard/security/application/company/contact + **interview / call / followup / event / profile / notification / deployment** (middleware `requestContext`, Winston → metrics-aggregator pour WARN/ERROR) ; **reste prioritaire A3/B6** : **`api-gateway`** (logs structurés requête entrante), **`workflow-service`** si exposé, validation bout-en-bout sur la vue corrélation. |
