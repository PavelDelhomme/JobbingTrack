...
Configuration pour utiliser OVH avec authentification `noreply@maily.ovh` mais affichage `noreply@jobbingtrack.com` :
...
```
Dans ce cas, l'email partira et s'affichera depuis `noreply@maily.ovh`, mais avec le nom d'affichage "JobbingTrack".

**Alternative** : Configurer un alias email `noreply@jobbingtrack.com` qui redirige vers `noreply@maily.ovh` dans votre panneau OVH, puis utiliser :
```env
