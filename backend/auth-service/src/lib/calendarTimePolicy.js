...
function resolveWindow(options = {}) {
  const min = parseHourMinute(
    options.minHour || process.env.EMAIL_TRIAGE_CALENDAR_MIN_HOUR || DEFAULT_MIN_HOUR,
  );
  const max = parseHourMinute(
    options.maxHour || process.env.EMAIL_TRIAGE_CALENDAR_MAX_HOUR || DEFAULT_MAX_HOUR,
  );
