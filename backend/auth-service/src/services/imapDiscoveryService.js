...
const DOMAIN_HINTS = [
  { test: (d) => d.endsWith('.ovh') || d === 'ovh.net', host: 'imap.mail.ovh.net', port: 993, provider: 'OVH' },
  { test: (d) => d === 'gmail.com' || d === 'googlemail.com', host: 'imap.gmail.com', port: 993, provider: 'Gmail' },
  { test: (d) => d === 'outlook.com' || d === 'hotmail.com' || d === 'live.com' || d.endsWith('.outlook.com'), host: 'outlook.office365.com', port: 993, provider: 'Microsoft' },
  { test: (d) => d === 'proton.me' || d === 'protonmail.com' || d.endsWith('.proton.me'), host: '127.0.0.1', port: 1143, provider: 'Proton', note: 'proton_bridge_required' },
  { test: (d) => d === 'yahoo.com' || d.endsWith('.yahoo.com'), host: 'imap.mail.yahoo.com', port: 993, provider: 'Yahoo' },
  { test: (d) => d === 'icloud.com' || d === 'me.com' || d === 'mac.com', host: 'imap.mail.me.com', port: 993, provider: 'Apple' },
];

const MX_IMAP_HINTS = [
  { pattern: /ovh\.net/i, host: 'imap.mail.ovh.net', port: 993, provider: 'OVH' },
  { pattern: /google\.com/i, host: 'imap.gmail.com', port: 993, provider: 'Gmail' },
  { pattern: /outlook\.com|microsoft\.com/i, host: 'outlook.office365.com', port: 993, provider: 'Microsoft' },
  { pattern: /yahoodns\.net/i, host: 'imap.mail.yahoo.com', port: 993, provider: 'Yahoo' },
];
  ...
  return new Promise((resolve) => {
    ...
    const req = https.get(url, { timeout: 6000 }, (res) => {
      ...
      res.on('end', () => {
        ...
        const host = incoming[1].match(/<hostname>([^<]+)<\/hostname>/i)?.[1];
        const port = incoming[1].match(/<port>(\d+)<\/port>/i)?.[1];
        const socketType = incoming[1].match(/<socketType>([^<]+)<\/socketType>/i)?.[1];
        if (!host) {
          resolve(null);
          return;
        }
        resolve({
          ...
          imapUseTls: String(socketType || 'SSL').toUpperCase() !== 'PLAIN',
          provider: 'autoconfig',
