...
```bash
python3 email_service.py send_password_reset "redacted@example.invalid" "John Doe" "reset-token-123" "user-id-456"
```

#### Envoyer un email de vérification

```bash
python3 email_service.py send_verification "redacted@example.invalid" "John Doe" "verification-token-123" "user-id-456"
```

#### Envoyer un email générique

```bash
python3 email_service.py send_generic "redacted@example.invalid" "Sujet" "Message texte" "<html>Message HTML</html>"
```
...
{
  "success": true,
  "message": "Email envoyé avec succès à redacted@example.invalid"
}
```
