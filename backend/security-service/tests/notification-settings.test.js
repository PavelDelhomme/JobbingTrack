...
function mockRequest(body = {}, user = { id: 'admin-1', email: 'redacted@example.invalid', role: 'SUPER_ADMIN' }) {
  return {
    body,
    ip: '127.0.0.1',
    connection: {},
...
describe('Notification settings - réauth et test email', () => {
  beforeEach(() => {
    ...
    securityNotificationSettings.saveSettings.mockReturnValue({
      enabled: true,
      recipients: ['redacted@example.invalid', 'redacted@example.invalid'],
      levels: ['critical', 'high'],
      updatedAt: new Date().toISOString()
    });
    securityAlertEmailNotifier.notifySecurityAlert.mockResolvedValue({
      sent: true,
      results: [{ to: 'redacted@example.invalid', sent: true }]
    });
  });
  ...
  test('refuse la mise à jour sans réauthentification valide', async () => {
    ...
    await notificationSettingsController.updateNotificationSettings(
      mockRequest({ recipients: ['redacted@example.invalid'], currentPassword: 'wrong' }),
      res
    );
