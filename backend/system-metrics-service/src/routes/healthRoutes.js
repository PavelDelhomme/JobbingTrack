...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({
      status: 'OK',
      service: 'system-metrics-service',
...
router.get('/detailed', async (req, res) => {
  try {
    ...
    try {
      const recentMetrics = await prisma.systemMetric.count({
        where: {
          timestamp: {
      ...
      healthCheck.checks.recentMetrics = {
        status: 'OK',
        message: `${recentMetrics} metrics in last 5 minutes`
      };
    } catch (error) {
      healthCheck.checks.recentMetrics = {
        status: 'ERROR',
        message: 'Unable to check recent metrics',
        error: error.message
      };
    }
