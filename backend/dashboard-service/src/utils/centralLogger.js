...
const METRICS_SERVICE_URL =
  process.env.METRICS_SERVICE_URL || process.env.METRICS_AGGREGATOR_URL || 'http://jobbingtrack-metrics-aggregator:3014';
const SERVICE_NAME = process.env.SERVICE_NAME || process.env.npm_package_name || 'dashboard-service';
...
class CentralLogger {
  ...
  async flushLogs() {
    ...
    try {
      ...
      console.error('[CENTRAL_LOGGER dashboard] Erreur envoi logs:', error.message);
    }
  }
