...
| P1C | Sécurité — logs triés et fiche menace score/corrélation | local | Sur `/b4ck0ff1ce/security/logs`, vérifier que le tri est explicite et que `Plus récent d’abord` affiche bien les événements récents avant les anciens ; tester recherche IP/message/type sans résultat trompeur lié à la page courante. Sur une fiche menace high/critical sans log corrélé, vérifier que le risque est affiché comme score retenu/estimé depuis la sévérité plutôt qu’un `Risque max 0` trompeur. | | | | [ ] | Preuve agent 12/06 : filtres serveur `eventType`/`q`/`order`, `timestamp desc` par défaut, `effectiveRiskScore` + `riskSource` côté investigation ; `node --check` JS OK, `git diff --check` OK ; Jest/TS/ESLint complets non exécutables sans dépendances locales. |
