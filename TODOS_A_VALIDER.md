...
| P1C | A1G Performances — libellés sources historiques | local | Ouvrir `/b4ck0ff1ce/performances/cpu-memory`, `/performances/containers`, `/performances/network` et `/performances/disk` : vérifier qu’un encart indique clairement la période, la source des séries (`system_metrics`, `container_metrics`, fallback Docker live ou séries dérivées) et, quand disponible, le nombre de points source vs points rendus. Sur 30 j, vérifier que le libellé aide à comprendre les trous éventuels sans donner l’impression que 30 j réels existent si la BDD locale n’a pas encore historisé 30 j. | | | | [ ] | **Preuve agent 15/06** : composant partagé `PerformanceHistoryCaption` ajouté et branché sur CPU & Mémoire, Conteneurs, Réseau et Disque. Validations sans `make` : TypeScript direct OK ; ESLint ciblé **0 erreur** ; lints IDE OK ; Jest direct `chartTimeDomain` **1/1 OK** ; Playwright direct Performances **7/7 OK** en **56,1 s**. |
