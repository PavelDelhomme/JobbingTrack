...
| P1A | Alertes email sécurité (MailHog / SMTP) | local | Après validation des P0 : vérifier Paramètres → Notifications (activation, niveaux, destinataires), déclencher une alerte `critical/high` test ; email visible MailHog ou SMTP configuré ; pas d’`AUTH PLAIN` vide ; `EmailLog.status=SENT` si table disponible. | [ ] | Réauth + audit + bouton test implémentés côté agent (07/06) — validation porteur à faire. |
