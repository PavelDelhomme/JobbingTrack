...
| P1C | Performances — points historiques CPU/mémoire/conteneurs/réseau/disque/latence réactivés | local | Ouvrir `/b4ck0ff1ce/performances`, `/performances/cpu-memory`, `/performances/containers`, `/performances/network`, `/performances/disk`, `/performances/latency` et `/performances/correlation` : vérifier que les graphes affichent de nouveau des points dans le temps, que la période 24h montre des courbes système CPU/mémoire/disque/réseau/temps de réponse, que les conteneurs ont des courbes CPU/mémoire/réseau sans trou durable, et que Corrélation reste utilisable sans surcharge. Tester aussi une bascule 24h → 7j → retour 24h. | | | | [ ] | **Preuve agent 13/06** : causes backend/UI corrigées dans `metrics-aggregator` et pages Performances. Historique conteneur lit `container_metrics`; alias courts disponibilité (`api-gateway`, `frontend`, `metrics-aggregator`) résolus vers `jobbingtrack-*`; Corrélation charge 8 services par défaut (16 en complet, concurrence 3) ; Disque limite le Block I/O à 8 conteneurs. Audits API : système **180/180** points CPU/mémoire/temps réponse/réseau/disque ; échantillon conteneurs **90/90** CPU/mémoire/réseau ; alias temps réponse **20/20**. Validations : `npm run type-check` OK ; `npm run lint` OK 0 erreur ; Jest Performances/metrics **4 suites / 14 tests OK** ; backend stable **3 suites / 18 tests OK**. Validation navigateur non faite : MCP redirigé `/login` sans session admin. |
...
| P1C | Frontend mémoire/performance — dev server stabilisé | local | Vérifier localement que `/b4ck0ff1ce/email-monitor`, `/b4ck0ff1ce/security` et `/b4ck0ff1ce/performances/correlation` ouvrent toujours la page attendue après connexion admin, que le suivi temps réel reste acceptable malgré les refreshs moins fréquents, et que le conteneur frontend ne remonte plus durablement au plafond **2 Go** après navigation entre ces pages. | | | | [ ] | **Preuve agent 13/06** : dev frontend Docker basculé sur `next dev --webpack`, doublon `-H` Compose supprimé, `Email Monitor` polling **10 s**, `Sécurité` logs aperçu **500** + refresh **15 s** visible-tab, `useMetrics` timer bootstrap annulé. Recreate frontend : `restart=0`, `oomKilled=false`, health `healthy`; smokes `/health` **200**, routes backoffice **200 → /login** sans session ; mémoire ~**499 MiB** repos, ~**1.41 GiB** après smokes/tests, ~**984 MiB** après court idle. Validations : `npm run type-check` OK ; `npm run lint` OK 0 erreur / 1430 warnings historiques ; Jest ciblé **5 suites / 18 tests OK** ; charge API légère **15/15**, avancée légère **46/46**. |
