...
|----------|--------------------|---------------|-----------------|--------|----------------|
| P0 | Comparaison de rapports sécurité (CVE) | local | Sur `/b4ck0ff1ce/test-reports`, mode comparaison : sélectionner **2** rapports catégorie **Sécurité** (ex. `security-results-cve-20260521-201336` + un autre CVE), lancer la comparaison → **pas** d’erreur « Rapport non trouvé » ; cartes Critical/High/Medium/Low/Info par rapport ; tableau par surface avec statut et écarts ; notes/payloads bruts non exposés dans la comparaison par défaut ; explication claire des cas `Absent`, `skipped`, `fail/vulnerable`, doublons Docker/dépendances, et priorité exploitable. Les surfaces avec `critical/high` doivent être marquées **à traiter/vulnerable**, jamais `OK`. La liste des rapports et l’aperçu doivent être alignés visuellement en hauteur, avec scroll interne lisible. | [ ] | **KO 21/05** : comparaison trop pauvre / 0 partout. **KO 08/06** : chiffres énormes, libellés trompeurs. **08/06 agent** : tri exploitabilité, filtres docker/node + critical/high, badges Ignoré/Absent, texte d’aide. **08/06 porteur 16h** : `node .` avec `high js-cookie` affiché `ok`. **09/06 porteur** : comparaison `cve-20260609-092335` vs `cve-20260608-140222` lisible et statut `À traiter` OK, mais couverture trop courte (4 surfaces, plusieurs `skipped`). **09/06 agent** : scan complet `/workspace`, audits temporaires pour Node sans lockfile, Flutter `pub outdated`, smoke 27 surfaces `skipped=0`; rapport sécurité applicatif `summary.json` + `report.html` corrigé ; terminal backoffice avec progression live par job et étapes `[CVE]`; liste rapports enrichie depuis `generatedAtISO`; alignement liste/aperçu corrigé. À revalider après merge/recreate : nouveau Scan CVE backoffice, vérifier progression terminal, apparition du rapport avec bonne date/heure, alignement liste/aperçu, puis comparaison avec rapport précédent. |
...
| P1B | Statistics / Vue d’ensemble — signaux sécurité cohérents | local | `/b4ck0ff1ce` et `/b4ck0ff1ce/statistics/security` : événements sécurité récents ≠ 0 si logs sécurité critical/high présents ; score sécurité global expliqué par les mêmes sources ; pas de contradiction avec `/b4ck0ff1ce/security/logs`. | [ ] | **09/06 porteur** : Vue d’ensemble affiche 0 signaux sécurité alors que des logs critical de tests existent et score global ≈ 20 %. À vérifier après P0 CVE. |
...
| P1C | Logs sécurité — pas de boucle de reload page | local | Ouvrir `/b4ck0ff1ce/security/logs` et surveiller les logs frontend : pas de rafale `GET /b4ck0ff1ce/security/logs` toutes les ~50 ms ; refresh borné et explicite. | [ ] | **09/06 porteur** : rafale de navigations page dans les logs frontend ; investiguer après P0 CVE. |
