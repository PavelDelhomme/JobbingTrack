...
|----------|--------------------|---------------|-----------------|--------|----------------|
| P1A | Alertes email critiques JobbingTrack | local/preprod | Suivre § “P1A — Alertes email critiques JobbingTrack” : page `/b4ck0ff1ce/security/alerts`, alias publics, réauth admin, bouton test, email MailHog + miroir SMTP réel si activé, diagnostic `/b4ck0ff1ce/email-monitor?type=NOTIFICATION`. | [ ] | KO navigation/MailHog corrigé ; destinataires actifs `security@jobbingtrack.com`, `dev@delhomme.ovh`, `admin@delhomme.ovh`. Correctifs 20:15 + 21:30 : miroir réactivé, `From` miroir aligné sur compte SMTP authentifié, faux positif metrics-aggregator corrigé, crash-report `From`/`Reply-To` alignés. Tests isolés admin + E2E `security-service` (`TEST ADMIN …`, `TEST P1A E2E security-service 2026-06-10T19:34:26Z`) ; MailHog + miroir OVH OK, `metadata.mirror` visible en Email Monitor et panneau diagnostic `/security/alerts` — **attendre confirmation boîte réelle admin**. |
