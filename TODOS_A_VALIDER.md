...
| P1C | Sécurité — titres FR, libellés et navigation | local | Suivre § “P1C — UX backoffice” : Politiques, Menaces, Firewall, Logs, Analyse, Réseau, Incidents et Alertes email avec titres FR, sous-nav cohérente, page Logs sécurité accessible, badges/libellés lisibles (`Critique`, `Force brute`, `Automatique`, `Bloquer (DROP)`) au lieu des codes API bruts. | | | | [ ] | Preuve agent 12/06 : helper `securityLabels`, route `/b4ck0ff1ce/security/logs`, `tsc` direct OK, Jest libellés **6/6 OK**, ESLint ciblé **0 erreur** sur nouveau helper/page/spec ; warnings historiques sur anciennes pages sécurité. |
