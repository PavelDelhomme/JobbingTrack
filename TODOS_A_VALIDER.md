...
| P1C | Lot A graphes — brush synchronisé page Performances Disque | local | Ouvrir `/b4ck0ff1ce/performances/disk` : glisser la barre brush sous le dernier graphe système (volume utilisé/total) ou, si pas d’historique système, sous le débit Block I/O Mo/min ; vérifier que **tous** les graphes visibles (usage %, volume, Block I/O cumulé, débit) zooment sur la même fenêtre temporelle. Cliquer **Réinitialiser le zoom** et vérifier le retour à la fenêtre récente par défaut. | | | | [ ] | **Preuve agent 15/06** : `useSyncedChartBrushRange` + `rowsInBrushWindow` sur 4 graphes Disque ; brush master = séries système si présentes. Validations sans `make` : `tsc --noEmit` OK ; ESLint ciblé **0 erreur** ; Jest hook **2/2 OK** ; Playwright Performances **6 passed + 1 flaky** Disque (rejeu ciblé **2/2 OK**). |
