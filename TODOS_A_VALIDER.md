...
| P1C | Graphes conteneurs multi-séries lisibles | local | Suivre § “P1C — UX backoffice” : Performances → Conteneurs → Tous les conteneurs, couleurs distinctes/stables CPU/mémoire. | | | | [ ] | **Preuve agent fraîche (12/06 16h00)** : liste conteneurs vide en HTTPS = routage Nginx corrigé ; message d’erreur explicite si API indisponible ; échelle Y CPU adaptative ; Playwright conteneurs **OK** ; **23** conteneurs via proxy. **À revalider sur `:5443`.** |
| P1C | Performances — CPU & Mémoire dédié | local | Ouvrir `/b4ck0ff1ce/performances/cpu-memory` : entrée **CPU & Mémoire** visible sous Synthèse et avant Temps de réponse ; cartes CPU/mémoire live ; graph CPU global et mémoire globale ; sous-vues **CPU détaillé** / **Mémoire détaillée** ; cases services activables/masquables ; période 24h↔7j↔personnalisé conserve les graphes sans écran vide durable. | | | | [ ] | Implémentation agent 13/06 sur `feat/performance-cpu-memory-charts` ; optimisation suite retour porteur : la vue globale charge une seule série système et les détails ne chargent que les services cochés (6 par défaut). Validations techniques `npm run type-check` OK, `npm run lint` OK (0 erreur, warnings historiques), HTTP `/cpu-memory` 200. |
