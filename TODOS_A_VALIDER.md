...
| P1C | Lot A graphes — brush synchronisé page Performances Réseau | local | Ouvrir `/b4ck0ff1ce/performances/network` : glisser la barre brush sous un des graphes (RX/TX, débit, temps de réponse ou corrélation CPU) et vérifier que **tous** les graphes de la page zooment sur la même fenêtre temporelle. Cliquer **Réinitialiser le zoom** et vérifier le retour à la fenêtre récente par défaut. | | | | [ ] | **Preuve agent 15/06** : hook `useSyncedChartBrushRange` + brush partagé sur 4 graphes Réseau. Validations sans `make` : Jest hook **1 suite / 2 tests OK** ; `type-check` OK ; `lint --quiet` OK ; Playwright Performances **7/7 OK** via `PLAYWRIGHT_BASE_URL=http://localhost:5003`. |
| P1C | Lot A graphes — brush synchronisé page Performances Disque | local | Ouvrir `/b4ck0ff1ce/performances/disk` : vérifier que **Block I/O cumulé — lecture / écriture** et **Débit Block I/O estimé — Mo/min** affichent des valeurs/courbes quand la plage contient des snapshots Block I/O (ex. 24 h local actuel). Glisser la barre brush sous le dernier graphe système (volume utilisé/total) ou, si pas d’historique système, sous le débit Block I/O Mo/min ; vérifier que **tous** les graphes visibles (usage %, volume, Block I/O cumulé, débit) zooment sur la même fenêtre temporelle. Cliquer **Réinitialiser le zoom** et vérifier le retour à la fenêtre récente par défaut. | | | | [ ] | **Preuve agent 15/06** : `useSyncedChartBrushRange` + `rowsInBrushWindow` sur 4 graphes Disque ; correction backend `getContainerMetricsHistory()` pour fusionner `container_metrics` et `container_metrics_snapshots` afin d’exposer `blockReadBytes/blockWriteBytes`. Validations sans `make` : `node --check` backend OK ; TypeScript direct frontend OK ; ESLint ciblé Disque OK ; API proxy 24 h **16 points Block I/O non nuls** sur les 8 conteneurs Disque ; Playwright Disque **2/2 OK**. Wrappers `npm run type-check` / `npm run lint` : code 1 sans sortie, dette connue. |
