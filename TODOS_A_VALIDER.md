...
| P1B | Statistics — graphes disponibilité / erreur (vue d’ensemble) | local | Suivre § “P1B — Statistics sécurité/logs/données applicatives/vue d’ensemble” : courbes chargées, légende source visible, plage 24h↔7j OK. | | | | [ ] | **Preuve agent 15/06** : Playwright vérifie désormais `/statistics` avec graphes **Disponibilité dans le temps**, source visible `Persistance system_metrics`, période affichée et mention **taux d’erreur dérivé (100 − disponibilité)**. API metrics directe : `/persistence/system/metrics` 24h et 7j OK, **500** points chaque, disponibilité **500/500**, `errorRate` explicite **0/500** donc dérivation depuis disponibilité confirmée. Validations sans `make` : frontend `format:check` OK, `type-check` OK, `lint --quiet` OK, Jest `statisticsTimeSeries` **1 suite / 3 tests OK**, Jest CI frontend **45 suites / 187 tests OK**, Playwright Statistics **5/5 OK**, perf HTTP moy. `/statistics` ~97 ms, `/statistics/security` ~69 ms, `/statistics/app-data` ~66 ms. Mail récap `[JobbingTrack] Recap P1B Statistics overview 2026-06-15` : `EmailLog` **3/3 SENT**, miroir SMTP **3/3 true**. |
...
| P1B | Performances — Réseau et Corrélation visibles | local | Ouvrir `/b4ck0ff1ce/performances/network` puis `/b4ck0ff1ce/performances/correlation` : vérifier que Réseau affiche des courbes RX/TX et que Corrélation montre un chargement explicite puis des graphes, pas une page vide durable. Vérifier aussi `/b4ck0ff1ce` : le panneau “Temps de réponse — services prioritaires P1B” ne doit plus afficher Postgres / `Santé Docker`. | | | | [ ] | **Preuve agent 15/06 (revalidée post-infra)** : Playwright `performances-range-smoke.spec.ts` **7/7 OK** (Synthèse, Réseau, Disque, Conteneurs, Latence, Corrélation) ; Jest ciblé P1B **12 tests OK** ; `format:check`, `type-check`, `lint --quiet` OK. Infra Postgres recreate : `max_connections=200`, burst `/stats` **20/20**. Mail récap `[JobbingTrack] Recap P1B Performances Reseau Correlation 2026-06-15` : **3/3 SENT**, miroir **3/3**. |
