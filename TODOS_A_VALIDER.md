...
| P1C | Audit charge API mobile + mémoire frontend | local | Revoir le résultat agent : confirmer que les paliers API bornés suffisent pour l’instant ou demander une campagne authentifiée plus longue. Points à valider : gateway tient les tests locaux contrôlés, WAF bloque un payload XSS quand le trafic simule une IP externe/proxy, mais le frontend dev Next/Turbopack est saturé proche de **2 Go** et doit être traité en priorité avant de parler de “ultra performant”. Décider aussi si l’on fournit un token/test user pour charger les vrais endpoints mobiles authentifiés (`applications`, `companies`, `contacts`, `followups`, `interviews`, `calls`) au-delà du simple périmètre auth/401. | | | | [ ] | **Preuve agent 13/06** : stack `full` healthy ; préflight offensif contrôlé OK ; scope lab plan-only **17** checks ; WAF direct localhost bypassé par design, retest avec `X-Forwarded-For` externe **403 WAF_BLOCKED** ; `test-performance.js PERF_LIGHT=1` **15/15** endpoints et **13/13** charge OK ; `test-load-advanced.js PERF_LIGHT=1` **46/46** OK ; palier avancé borné non extreme **455/455** requêtes OK, spike health **200/200** concurrence 50 ; snapshot backend sain (gateway ~42–52 MiB, services métier ~59–81 MiB, metrics ~116–132 MiB). Alerte : `jobbingtrack-frontend` **1.98–2.00 GiB / 2 GiB**, CPU >200%, `restart=111`, `next-server` ~1.4 GiB RSS. Limites : pas de DDoS destructif, pas de rate-limit prod-like en `NODE_ENV=development`, pas de charge mobile CRUD authentifiée sans token de test, pas de profil navigateur connecté. |
