...
| P1C | Mode sombre persistant après refresh | local | Suivre § “P1C — UX backoffice” : choisir sombre, rafraîchir login/backoffice, le thème reste sombre. | [ ] | **Preuve agent fraîche (12/06 13h56)** : Jest `theme.test.ts` + `apply.test.ts` **9/9 OK** ; `setStoredTheme` synchronise `theme`, `ui-preferences-v1`, `customization-settings` ; script init `layout.tsx` ; rapport `p1c-ux-theme-settings/20260612-135642` **4/4 OK**. Réponse : `OK Mode sombre persistant après refresh` ou `KO` + détail. |
| P1C | Popup paramètres fermeture | local | Suivre § “P1C — UX backoffice” : clic extérieur et `Escape` ferment la mini-fenêtre sans perte de clic interne. | [ ] | **Preuve agent fraîche (12/06 13h56)** : Jest `SettingsPopup.test.tsx` **5/5 OK** (Escape, backdrop, bouton fermer, clic dialogue stoppé) ; même rapport `p1c-ux-theme-settings/20260612-135642`. Réponse : `OK Popup paramètres fermeture` ou `KO` + détail. |
