...
export function useOfflineSync() {
  ...
  const saveToStorage = useCallback(() => {
    try {
      localStorage.setItem(PENDING_OPERATIONS_KEY, JSON.stringify(state.pendingOperations));
  ...
  const setCache = useCallback((key: string, data: any, ttlMinutes?: number) => {
  ...
  const syncPendingOperations = useCallback(async () => {
    ...
    console.log(`🔄 Synchronisation de ${state.pendingOperations.length} opérations...`);
