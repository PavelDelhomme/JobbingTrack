...
describe('threatSignals', () => {
  it('détecte SQLi par type ou payload', () => {
    ...
    expect(isSqliThreat({ threatType: 'PORT_SCAN', metadata: { payload: "union select" } })).toBe(true);
  ...
  it('compte les logs de détection et respecte excludeEventTypes', () => {
    ...
    expect(countDetectionLikeLogs(logs, { excludeEventTypes: ['network_threat_detected'] })).toBe(1);
  });
});
