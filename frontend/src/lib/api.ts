...
apiClient.interceptors.response.use(
  ...
  (error) => {
    ...
    if (
      ...
      const fullServiceName = error.config?.url?.includes("/applications")
        ? "application-service"
        : error.config?.url?.includes("/companies")
          ? "company-service"
            ...
            : error.config?.url?.includes("/interviews")
              ...
              : error.config?.url?.includes("/calls")
                ...
                : error.config?.url?.includes("/events")
                  ? "event-service"
                  : error.config?.url?.includes("/followups")
                    ? "followup-service"
                    : error.config?.url?.includes("/profile")
                      ? "profile-service"
                      : error.config?.url?.includes("/notifications")
                        ? "notification-service"
                        : error.config?.url?.includes("/workflow")
                          ? "workflow-service"
                          : error.config?.url?.includes("/dashboard")
                            ? "dashboard-service"
                            : error.config?.url?.includes("/security")
                              ? "security-service"
                              : "unknown-service";
...
criticalApiClient.interceptors.response.use(
  (response) => response,
  (error) => {
    // Gestion des erreurs d'authentification
    if (error.response?.status === 401) {
...
export const authService = {
  ...
  getAllUsers: () =>
    cachedRequest("auth-users", () => apiClient.get("/auth/users")),
...
export const applicationService = {
  getAll: (params?: { limit?: number; agencyId?: string }) =>
  ...
  getContacts: (id: string) => apiClient.get(`/applications/${id}/contacts`),
};
...
export const callService = {
  ...
  complete: (id: string, data: any) =>
    apiClient.put(`/calls/${id}/complete`, data),
  getStats: () => apiClient.get("/calls/stats/overview"),
  getByApplication: (applicationId: string) =>
    apiClient.get(`/calls/application/${applicationId}`),
};
...
export const dashboardService = {
  getKPIs: () =>
    cachedRequest("dashboard-kpis", () => apiClient.get("/dashboard/kpis")),
  getStats: () =>
    cachedRequest("dashboard-stats", () => apiClient.get("/dashboard/stats")),
};
...
export const adminService = {
  ...
  getServiceLogs: (
    serviceName: string,
    lines = 100,
    extra?: { since?: string; until?: string },
  ) =>
    apiClient.get(`/admin/logs/${serviceName}`, {
      params: { lines, ...extra },
    }),
  ...
  emptyTrash: () => apiClient.post("/admin/trash/empty"),
  ...
  clearTestData: () => apiClient.post("/admin/test-data/clear"),
  getTestDataStatus: () => apiClient.get("/admin/test-data/status"),
};
