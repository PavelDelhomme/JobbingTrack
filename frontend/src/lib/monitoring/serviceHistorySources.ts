...
export function historyPointsFromAggregatorChartData(
  metrics: { chartData?: unknown[]; servicesList?: unknown[] } | null | undefined,
  fullServiceName: string,
  serviceName: string,
  ...
  if (!metrics || !Array.isArray(chartData) || chartData.length === 0 || !Array.isArray(servicesList)) {
    return []
  }
  ...
  return chartData
    .map((point: any) => ({
      ...
      block_read_mb:
        Number(
          point.services?.[serviceKey]?.block_read_mb ?? point.services?.[serviceKey]?.block_read ?? 0
        ) || 0,
      block_write_mb:
        Number(
          point.services?.[serviceKey]?.block_write_mb ?? point.services?.[serviceKey]?.block_write ?? 0
        ) || 0
    }))
  ...
  try {
    ...
    if (historyResponse.ok) {
      ...
      serverHistoryPoints = normalizeServerHistoryRows(raw).sort(
        (a, b) => (metricTimestampToMs(a.timestamp) ?? 0) - (metricTimestampToMs(b.timestamp) ?? 0)
      )
    }
  } catch {
