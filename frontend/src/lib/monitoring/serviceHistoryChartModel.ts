...
export function buildHistoryChartRowsIo(
  rows: ServiceHistoryChartRow[],
): ServiceHistoryIoRow[] {
  return rows.map((row, i) => {
    ...
    const dr = Math.max(
      0,
      Number(row.block_read_mb) - Number(prev.block_read_mb),
    );
    const dw = Math.max(
      0,
      Number(row.block_write_mb) - Number(prev.block_write_mb),
    );
    return {
      ...row,
...
export function historyCpuMaxY(serviceHistory: ServiceHistoryPoint[]): number {
  ...
  const m = Math.max(
    0.02,
    ...serviceHistory.map((h) => Number(h.cpu_percent) || 0),
  );
...
export function historyMemMaxY(serviceHistory: ServiceHistoryPoint[]): number {
  ...
  const m = Math.max(
    0.5,
    ...serviceHistory.map((h) => Number(h.memory_percent) || 0),
  );
  return Math.min(100, m * 1.15 + 0.5);
}

export function historyAxisShowDateForSpan(
  rows: ServiceHistoryChartRow[],
): boolean {
...
export function historyIoRateMaxY(rows: ServiceHistoryIoRow[]): number {
  ...
  const m = Math.max(
    ...
    ...rows.map((h) =>
      Math.max(
        Number(h.block_read_mb_per_min) || 0,
        Number(h.block_write_mb_per_min) || 0,
      ),
    ),
  );
  return m * 1.15 + 0.01;
