...
export class AdbRunner {
  ...
  async run(
    ...
    this.log(
      `Parcours "${scenario.name}" demarre (${steps.length} etapes) [UI reelle]`,
    );
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      const isAbortError = (err: any) =>
        err?.name === "AbortError" ||
        /abort|NS_BINDING_ABORTED/i.test(String(err?.message ?? ""));

      try {
        let msg: string;
        try {
          msg = await executeStep(stepId, this.adb, {
            isCancelled: () => this.cancelled,
          });
        } catch (firstErr: any) {
          if (!this.cancelled && isAbortError(firstErr)) {
            this.log(
              `  ⚠ Requête interrompue (NS_BINDING_ABORTED / Abort), retry une fois...`,
            );
