...
export interface MobileAction {
  ...
  name: string;
  description: string;
  category: 'navigation' | 'saisie' | 'geste' | 'verification' | 'auth' | 'attente';
  icon: string;
  params: ActionParam[];
}
...
export const MOBILE_ACTIONS: MobileAction[] = [
  ...
  },
  {
    ...
    icon: '🔐',
    params: [
      { key: 'email', label: 'Email', type: 'text', default: 'admin@jobbingtrack.test', placeholder: 'Email de connexion', required: true },
      { key: 'password', label: 'Mot de passe', type: 'text', default: 'password123', placeholder: 'Mot de passe', required: true },
    ],
  },
  {
    id: 'mob_logout',
  ...
  // ─── Navigation ────────────────────────────────────────────────
  {
    ...
    icon: '👆',
    params: [
      { key: 'text', label: 'Texte a chercher', type: 'text', placeholder: 'Ex: Se connecter', required: true },
      { key: 'index', label: 'Index (si plusieurs)', type: 'number', default: 0 },
    ],
  },
  {
    id: 'mob_tap_tab',
    ...
    icon: '📑',
    params: [
      { key: 'tab', label: 'Numero d\'onglet', type: 'select', default: '1', required: true, options: [
  ...
  },
  {
    ...
    icon: '📂',
    params: [
      { key: 'text', label: 'Texte de l\'item', type: 'text', placeholder: 'Ex: Relances', required: true },
  ...
  // ─── Saisie ────────────────────────────────────────────────────
  {
    ...
    icon: '⌨️',
    params: [
      { key: 'hint', label: 'Hint du champ', type: 'text', placeholder: 'Ex: Email', required: true },
      { key: 'value', label: 'Texte a saisir', type: 'text', placeholder: 'Ex: admin@jobbingtrack.test', required: true },
    ],
  },
  {
  ...
  },
  {
    ...
    icon: '👉',
    params: [
      { key: 'x1', label: 'X depart', type: 'number', default: 540, required: true },
      { key: 'y1', label: 'Y depart', type: 'number', default: 1600, required: true },
      { key: 'x2', label: 'X arrivee', type: 'number', default: 540, required: true },
      { key: 'y2', label: 'Y arrivee', type: 'number', default: 600, required: true },
  ...
  // ─── Verification ──────────────────────────────────────────────
  {
    ...
    icon: '✅',
    params: [
      { key: 'text', label: 'Texte attendu', type: 'text', placeholder: 'Ex: Bonjour', required: true },
    ],
  },
  {
    ...
    icon: '🚫',
    params: [
      { key: 'text', label: 'Texte qui ne doit pas etre present', type: 'text', required: true },
    ],
  },
  {
    id: 'mob_wait_for',
  ...
  // ─── Attente ───────────────────────────────────────────────────
  {
    ...
    icon: '⏸️',
    params: [
      { key: 'ms', label: 'Duree (ms)', type: 'number', default: 2000, required: true },
    ],
  },
];
...
export async function executeMobileAction(
  ...
  switch (actionId) {
    ...
    case 'mob_type_in_field': {
      ...
      if (!hint || !value) throw new Error('Parametres "hint" et "value" requis');
    ...
    case 'mob_swipe': {
      await adb.swipe(
        params.x1 ?? 540, params.y1 ?? 1600,
        params.x2 ?? 540, params.y2 ?? 600,
        params.duration ?? 400,
      );
      await adb.wait(1000);
    ...
    case 'mob_assert_text': {
      ...
      if (!found) throw new Error(`Texte "${params.text}" non trouve a l'ecran`);
      return `"${params.text}" present`;
    }

    case 'mob_assert_not_text': {
      const present = await adb.uiContains(params.text);
      if (present) throw new Error(`Texte "${params.text}" est present alors qu'il ne devrait pas`);
      return `"${params.text}" absent (OK)`;
    }

    case 'mob_wait_for': {
      const found = await adb.waitForElement(params.text, params.timeout || 30000);
      if (!found) throw new Error(`"${params.text}" non apparu apres ${params.timeout || 30000}ms`);
