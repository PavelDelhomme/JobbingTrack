...
const validateJwtToken = (token: string): boolean => {
  ...
  if (parts.length !== 3) {
    console.error(
      "Format de token invalide: Le token ne contient pas 3 parties",
    );
    return false;
  }
...
export function AuthProvider({ children }: { children: ReactNode }) {
      ...
      try {
        ...
        if (storedToken) {
          ...
          loadUserProfile(storedToken).catch((error) => {
            console.warn(
              "⚠️ Error loading profile, continuing without authentication:",
              error.message,
            );
  ...
  const loadUserProfile = async (authToken: string) => {
    ...
    if (
      !authToken ||
      (!validateJwtToken(authToken) && !authToken.startsWith("mock-jwt-token"))
    ) {
      console.warn(
        "⚠️ Token invalide dans loadUserProfile, nettoyage sans bloquer l'application",
      );
      clearAuthData();
      return;
    }
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (
          ...
          console.warn(
            "Session expired or unauthorized (server error confirmed)",
          );
        ...
        if (isNetworkError(error)) {
          console.warn(
            "⚠️ Persistent network error, but keeping token to retry later",
          );
          setLoading(false);
          return;
        }
      ...
      try {
        ...
        if (response.data?.success && response.data.token) {
          ...
          if (
            !(
              process.env.NODE_ENV === "development" &&
              newToken.startsWith("mock-jwt-token")
            ) &&
