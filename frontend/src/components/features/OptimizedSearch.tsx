...
export function OptimizedSearch({
  ...
  const [searchMode, setSearchMode] = useState<"online" | "offline" | "hybrid">(
    "online",
  );
  ...
  const { search, isIndexing, stats, buildSearchIndex } = useSearchIndex();
    ...
    async (searchQuery: string) => {
      ...
      try {
        ...
        if (response.data.success) {
          const searchResults = response.data.results.flatMap(
            (r: any) => r.results || [],
          );
    ...
    <div className={`space-y-4 ${className}`}>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {[
                ...
                <Badge
                  key={module}
                  variant={
                    selectedModules.includes(module) ? "default" : "outline"
                  }
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-muted-foreground">
                      Index: {stats.totalEntries} entrées
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
            ...
            {!isLoading && query.length >= 2 && results.length === 0 && (
              <div className="p-6 text-center">
                ...
                <p className="text-xs text-muted-foreground">
                  Essayez avec des termes différents ou sélectionnez plus de
                  modules
                </p>
              </div>
            )}
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <span className="text-muted-foreground">
                                {getModuleIcon(formatted.module)}
                              </span>
      ...
      <Alert>
        ...
        <AlertDescription className="text-xs">
          <strong>Recherche complète :</strong> Support du mode sombre,
          recherche dans 10 modules différents (candidatures, entreprises,
          contacts, entretiens, appels, utilisateurs, événements, notifications,
          archives, corbeille). Interface moderne avec recherche avancée
          manuelle et filtres avancés (date, statut, priorité).
        </AlertDescription>
      </Alert>
    </div>
  );
