...
export function SystemCpuNetworkCorrelationChart({
  ...
  return (
    <ResponsiveContainer width="100%" height={height} minHeight={240}>
      <LineChart data={rows} margin={{ top: 8, right: 36, left: 8, bottom: bottom }}>
        ...
        <YAxis
          ...
          unit=" %"
          tick={{ fontSize: 11 }}
          label={{ value: 'CPU %', angle: -90, position: 'insideLeft', fill: '#60A5FA', fontSize: 11 }}
        />
        <YAxis
          yAxisId="right"
          orientation="right"
          ...
          label={{ value: 'Mo/min', angle: 90, position: 'insideRight', fill: '#C4B5FD', fontSize: 11 }}
        />
        <Tooltip
          ...
          formatter={(value: number, name: string) => {
            ...
            if (name === 'cpu') return [`${Number.isFinite(n) ? n.toFixed(2) : '—'} %`, 'CPU']
            if (name === 'networkRxMbPerMin') return [`${n.toFixed(4)} Mo/min`, 'RX débit']
            if (name === 'networkTxMbPerMin') return [`${n.toFixed(4)} Mo/min`, 'TX débit']
