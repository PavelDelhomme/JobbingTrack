...
describe("Performance UI shared components", () => {
  it("rend le shell de page avec retour, sous-navigation, titre et actions", () => {
    ...
    expect(screen.getByRole("link", { name: /retour à performances/i })).toHaveAttribute(
      "href",
      "/b4ck0ff1ce/performances",
    );
    expect(screen.getByRole("navigation", { name: "Performances" })).toBeInTheDocument();
    expect(screen.getByRole("heading", { name: "Performances réseau" })).toBeInTheDocument();
    expect(screen.getByText("Description courte")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Changer période" })).toBeInTheDocument();
    expect(screen.getByText("Contenu page")).toBeInTheDocument();
  });

  it("rend une carte de graphe avec période et contenu", () => {
    render(
      <PerformanceChartCard title="Débit réseau" periodLabel="24 dernières heures">
        <div>Graphique</div>
      </PerformanceChartCard>,
    );

    expect(screen.getByRole("heading", { name: "Débit réseau" })).toBeInTheDocument();
