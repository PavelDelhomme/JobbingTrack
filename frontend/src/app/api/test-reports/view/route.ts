  ...
  try {
    ...
    if (id) {
      ...
      if (id.startsWith("perf-backend-")) {
        ...
        fullPath = join(reportDir, "summary.json");
      } else if (id.startsWith("statistics-overview-")) {
        const suffix = id.replace("statistics-overview-", "");
        const reportDir = join(
          REPORT_DIRS["tests-results"],
          "statistics-overview",
          suffix,
        ...
        fullPath = join(reportDir, "summary.json");
      } else if (id.startsWith("statistics-endpoint-latency-")) {
        const suffix = id.replace("statistics-endpoint-latency-", "");
        const reportDir = join(
          REPORT_DIRS["tests-results"],
          "statistics-endpoint-latency",
          suffix,
        );
        if (!isWithinDirectory(REPORT_DIRS["tests-results"], reportDir)) {
          return NextResponse.json(
        ...
        fullPath = join(reportDir, "summary.json");
      } else if (id.startsWith("p1c-ux-theme-settings-")) {
        const suffix = id.replace("p1c-ux-theme-settings-", "");
        const reportDir = join(
          REPORT_DIRS["tests-results"],
          "p1c-ux-theme-settings",
          suffix,
        );
        if (!isWithinDirectory(REPORT_DIRS["tests-results"], reportDir)) {
          return NextResponse.json(
