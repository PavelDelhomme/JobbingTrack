...
const REPORT_DIRS = {
  ...
  "tests-results":
    ...
    (IS_DOCKER ? "/app/tests/results" : join(PROJECT_ROOT, "tests", "results")),
    ...
    if (all) {
      ...
      try {
        ...
        try {
          ...
          const entries = await readdir(testsResultsDir, {
            withFileTypes: true,
          });
          ...
          for (const dir of reportDirs) {
            ...
            if (success) {
              ...
              try {
                await execAsync(
                  `chmod -R 777 "${resolvedPath}" 2>/dev/null || true`,
                );
      ...
      try {
        ...
        try {
          ...
          const reportFiles = files.filter(
            (f) => f.endsWith(".json") || f.endsWith(".html"),
          );
      ...
      for (const [label, securityDir] of [
        ...
        try {
          ...
          try {
            ...
            const entries = await readdir(resolvedSecurityDir, {
              withFileTypes: true,
            });
      ...
      if (id.startsWith("perf-backend-")) {
        ...
        try {
          try {
            await stat(jsonPath);
            await safeRemove(jsonPath, false);
            deleted = true;
          } catch {}
          try {
            await stat(htmlPath);
            await safeRemove(htmlPath, false);
            deleted = true;
          } catch {}
        } catch (e: any) {
          error = e.message;
      ...
      if (deleted) {
        ...
        return NextResponse.json(
          {
            success: false,
            error: error || "Rapport non trouvé ou erreur de suppression",
          },
      ...
      return NextResponse.json(
        {
          success: false,
          error: 'ID du rapport manquant ou paramètre "all" requis',
        },
        { status: 400 },
      );
    }
