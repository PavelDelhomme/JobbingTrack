...
function parseTestResultsTxt(content: string): TestRow[] {
  ...
  for (const line of lines) {
    ...
    rows.push({
      ...
      name: (name || '').trim(),
      status: (status || 'fail').trim().toLowerCase() === 'pass' ? 'pass' : 'fail',
  ...
  try {
    ...
    if (!idsParam) {
      return NextResponse.json(
        { success: false, error: 'Paramètre ids requis (ex: ids=20260219-223913,20260219-120000)' },
        { status: 400 }
      )
    }
    ...
    if (ids.length < 2) {
      return NextResponse.json(
        { success: false, error: 'Au moins 2 rapports sont requis pour comparer' },
        { status: 400 }
      )
    }
    ...
    for (const id of ids) {
      ...
      let summary: { totalTests?: number; totalPassed?: number; totalFailed?: number; totalSkipped?: number; category?: string; testName?: string } = {}
      if (existsSync(summaryPath)) {
        try {
          const raw = await readFile(summaryPath, 'utf-8')
          const parsed = JSON.parse(raw)
          summary = { ...(parsed.summary || {}), category: parsed.category, testName: parsed.testName }
      ...
      const date = datePart && datePart.length >= 8
        ? `${datePart.slice(0, 4)}-${datePart.slice(4, 6)}-${datePart.slice(6, 8)}`
        : id
      const time = timePart && timePart.length >= 6
        ? `${timePart.slice(0, 2)}:${timePart.slice(2, 4)}:${timePart.slice(4, 6)}`
        : ''

      reportsData.push({
        id,
        name: summary.testName ? `${summary.testName} - ${date} ${time}` : `Rapport ${date} ${time}`,
        date,
        time,
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Erreur lors de la comparaison' },
      { status: 500 }
    )
  }
}
