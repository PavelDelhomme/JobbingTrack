...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (interval) {
      case 'hourly':
        newSchedule.nextRun = new Date(now.getTime() + 60 * 60 * 1000).toISOString()
        break
      case 'daily':
        newSchedule.nextRun = new Date(now.getTime() + 24 * 60 * 60 * 1000).toISOString()
        break
      case 'weekly':
        newSchedule.nextRun = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000).toISOString()
        break
      case 'custom':
