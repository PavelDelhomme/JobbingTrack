...
const INSTALL_TIMEOUT_MS = 10 * 60 * 1000; // 10 min — gros APK (~150 Mo) + adb install
  ...
  return fetch(controllerUrl, {
    ...
    .then(async (res) => {
      ...
      try {
        ...
        return {
          ok: false as const,
          error: `Réponse non-JSON (${res.status})`,
        };
      }
...
/**
 * Proxy POST /install-run — route dédiée (timeout long).
 * Le catch-all peut être coupé par le dev server Next sur les installs longues → Failed to fetch.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const controllerUrl = `${controllerBase}/install-run`;
    let result = await tryFetch(controllerUrl, payload, controller.signal);

    if (!result.ok && controllerBase !== clientUrl) {
      result = await tryFetch(`${clientUrl}/install-run`, payload, controller.signal);
    }
    ...
    return NextResponse.json(
      {
        ...
        _hint:
          "Contrôleur émulateur injoignable ou requête interrompue. Vérifiez : docker ps | grep emulator-controller ; curl http://127.0.0.1:5055/health",
      },
      { status: 502 },
    );
  } catch (err) {
    ...
    return NextResponse.json(
      {
        success: false,
        error: message,
        message: `Erreur proxy install : ${message}`,
        _hint:
          "Relancez le contrôleur : bash scripts/mobile/setup/restart-emulator-controller.sh",
      },
      { status: 502 },
    );
  }
