...
async function runFullPlaywrightSuite(): Promise<{
  success: boolean;
  reportId?: string;
  error?: string;
}> {
...
export async function POST(request: NextRequest) {
  console.log(
    `${TESTS_TAG} Démarrage des Tests Playwright depuis le backoffice — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })}`,
  );
  try {
    ...
    if (!hasScenarios) {
      ...
      console.log(
        `${TESTS_TAG} Fin — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })} — rapport: ${result.reportId ?? "N/A"}`,
      );
      ...
      if (!result.success) {
        return NextResponse.json(
          {
            success: false,
            error: result.error || "Échec exécution Playwright",
            reportId: result.reportId,
          },
          { status: 500 },
        );
    ...
    if (!res.ok) {
      console.log(
        `${TESTS_TAG} Fin (erreur API) — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })}`,
      );
      return NextResponse.json(
        {
          success: false,
    ...
    console.log(
      `${TESTS_TAG} Fin — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })} — executionId: ${data.executionId ?? "N/A"}`,
    );
    return NextResponse.json({
      success: true,
