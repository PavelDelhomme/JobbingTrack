...
export async function POST(_request: NextRequest) {
  console.log(
    `${TAG} Démarrage — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })}`,
  );
  ...
  if (inContainer) {
    console.log(
      `${TAG} Conteneur frontend : make / stack hôte non disponibles — réponse « skipped »`,
    );
  ...
  try {
    ...
    console.log(
      `${TAG} Fin OK — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })}`,
    );
    return NextResponse.json({
      success: true,
