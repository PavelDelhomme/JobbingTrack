...
export async function POST(request: NextRequest) {
  console.log(
    `${TESTS_TAG} Démarrage depuis le backoffice — ${new Date().toLocaleString("fr-FR", { timeZone: process.env.TZ || "Europe/Paris" })}`,
  );
  try {
    ...
    const perfCommand = inContainer
      ? "sh /app/scripts/performance/run-performance-frontend-in-container.sh"
