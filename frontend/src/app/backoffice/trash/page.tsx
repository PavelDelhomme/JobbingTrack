...
export default function TrashManagementPage() {
  ...
  const fetchDeletedItems = async () => {
    setLoading(true)
    try {
      // TODO: Créer l'endpoint /api/v1/admin/trash
  ...
  const handleRestore = async (item: DeletedItem) => {
    ...
    try {
      await api.post(`/admin/trash/${item.type.toLowerCase()}/${item.id}/restore`)
      fetchDeletedItems()
    } catch (error) {
  ...
  const handlePermanentDelete = async (item: DeletedItem) => {
    ...
    try {
      await api.delete(`/admin/trash/${item.type.toLowerCase()}/${item.id}/permanent`)
      fetchDeletedItems()
    } catch (error) {
