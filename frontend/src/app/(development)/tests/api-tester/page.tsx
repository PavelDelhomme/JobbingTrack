...
export default function APITesterPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "request" | "history" | "collections" | "environment" | "cookies" | "tests"
  >("request");
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [authMethod, setAuthMethod] = useState<
    "none" | "bearer" | "basic" | "apikey" | "oauth2"
  >("bearer");
      ...
      const context = {
        pm: {
          environment: {
            ...
            get: (key: string) => {
              const envVar = environmentVariables.find(
                (env) => env.key === key && env.enabled,
              );
              return envVar?.value || "";
            },
          },
  ...
  const executeTestScripts = async (response: any) => {
    try {
      ...
      for (const script of testScripts.filter(
        (s) => s.enabled && s.type === "test",
      )) {
        try {
        ...
        if (cookie) {
          setCookies((prev: any) => {
            const existingIndex = prev.findIndex(
              (c: any) => c.name === cookie.name && c.domain === cookie.domain,
            );
            if (existingIndex >= 0) {
              const updated = [...prev];
  ...
  const quickTests = {
    applications: [
      ...
      {
        name: "Créer candidature",
        method: "POST",
        endpoint: "",
        body: JSON.stringify(
          { position: "Test", companyName: "Test Company" },
          null,
          2,
        ),
      },
      ...
      if (authMethod === "bearer") {
        ...
        const tokenToUse =
          selectedUser && selectedUserData
            ? selectedUserData.token || localStorage.getItem("token")
      ...
      <div>
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">
            Testez les endpoints avec authentification, variables
            d'environnement et collections
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="mt-3 space-y-3">
                      ...
                      {selectedUser && (
                        <div>
                          ...
                          <textarea
                            value={
                              users.find((u) => u.id === selectedUser)?.token ||
                              ""
                            }
                            onChange={(e) => {
                              const updatedUsers = users.map((u) =>
                                u.id === selectedUser
                                  ? { ...u, token: e.target.value }
                                  : u,
                              );
                          ...
                          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            Laissez vide pour utiliser le token par défaut ou
                            collez un token personnalisé
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <button
                        onClick={() => {
                          const newHeaders = { ...customHeaders };
                          newHeaders[
                            `header-${Object.keys(customHeaders).length}`
                          ] = "";
                          setCustomHeaders(newHeaders);
                        }}
                ...
                <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg dark:shadow-gray-900/50 p-6 border border-gray-200 dark:border-gray-800">
                  ...
                  <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="flex items-center space-x-4">
                      <div
                        className={`px-3 py-1 rounded font-mono text-sm font-medium ${
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg dark:shadow-gray-900/50 p-6 border border-gray-200 dark:border-gray-800">
                ...
                <div className="space-y-2">
                  {(quickTests[service as keyof typeof quickTests] || []).map(
                    (test, index) => (
                      <button
                        key={index}
                  ...
                  filteredHistory.map((item) => (
                    <div
                      ...
                      onClick={() => {
                        ...
                        setService(
                          item.url.split("/api/v1/")[1]?.split("/")[0] ||
                            "applications",
                        );
                        setEndpoint(
                          item.url
            ...
            <div className="bg-white dark:bg-gray-900 rounded-lg shadow-lg dark:shadow-gray-900/50 p-6 border border-gray-200 dark:border-gray-800">
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-gray-400 dark:text-gray-500">
                  Organisez vos requêtes API en collections pour une meilleure
                  gestion
                </p>
              </div>
            </div>
              ...
              <div className="space-y-4">
                ...
                {environmentVariables.map((env, index) => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        const newEnv = environmentVariables.filter(
                          (_, i) => i !== index,
                        );
                        setEnvironmentVariables(newEnv);
                        setEnvironments((prev) => ({
                ...
                {testScripts.map((script, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        onClick={() => {
                          const updated = testScripts.filter(
                            (_, i) => i !== index,
                          );
                          setTestScripts(updated);
                        }}
