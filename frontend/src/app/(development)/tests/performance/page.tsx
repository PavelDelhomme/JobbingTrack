...
// Appels API via le même hôte que la page → Next.js rewrite vers api-gateway (WAF « interne », pas d’en-tête de test falsifiable).
...
export default function PerformanceTestsPage() {
  ...
  const startPerformanceTests = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Erreur API: ${response.status.toString()}`);
      }
