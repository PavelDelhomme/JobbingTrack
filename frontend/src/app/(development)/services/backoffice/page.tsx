      ...
      try {
        ...
        if (servicesData && servicesData.length > 0) {
          const updatedServices = services.map((service) => {
            const metricsData = servicesData.find(
              (s: any) => s.name?.includes(service.id) || s.id === service.id,
            );
            return {
              ...service,
              ...
              metrics: metricsData,
              uptime:
                metricsData?.status === "running" ? "En ligne" : "Hors ligne",
            };
          });
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Activity
              className={`h-5 w-5 ${loading ? "animate-pulse text-blue-500" : "text-green-500"}`}
            />
          ...
          {(servicesWithMetrics.length > 0
            ...
            return (
              <div
                ...
                onClick={() =>
                  !maintenance?.isActive && router.push(service.route)
                }
              >
                ...
                {service.metrics && (
                  <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1 bg-gray-50 dark:bg-gray-700/50 rounded p-2">
                    ...
                    <div className="flex justify-between">
                      <span>Mémoire:</span>
                      <span className="font-medium">
                        {service.metrics.memory?.percent !== "N/A" &&
                        service.metrics.memory?.percent !== undefined
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-blue-900 dark:text-blue-300 font-medium">
                {
                  servicesWithMetrics.filter((s) => s.status === "running")
                    .length
                }{" "}
                services en ligne
              </span>
