...
const MemoryChart = memo(function MemoryChart({ data }: { data: any[] }) {
  const chartData = useMemo(
    () => buildChartData(data, "time", "memory_percent", "Mémoire"),
    [data],
  );
  if (chartData.length === 0) return null;
  return (
...
const AvailabilityChart = memo(function AvailabilityChart({
  data,
}: {
  data: any[];
}) {
  const chartData = useMemo(
    () => buildChartData(data, "time", "availability_percent", "Disponibilité"),
    [data],
  );
  if (chartData.length === 0) return null;
  return (
