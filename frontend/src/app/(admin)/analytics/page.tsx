...
const legacyChartLoading = () => (
  <div className="flex h-64 w-full items-center justify-center rounded-lg border border-gray-200 bg-gray-50 text-sm text-gray-500 dark:border-gray-700 dark:bg-gray-800/80 dark:text-gray-400">
    Chargement du graphique…
  </div>
);

const CpuSystemChart = dynamic(
  () =>
    import("@/components/charts/LegacyAnalyticsCharts").then(
      (m) => m.CpuSystemChart,
    ),
  { ssr: false, loading: legacyChartLoading },
);
...
function AnalyticsContent() {
    ...
    try {
      ...
      const [history, stats] = await Promise.all([
        centralMetricsService
          .getMetricsHistory({ limit, startTime, endTime })
          .catch(() => []),
        centralMetricsService.getMetricsStats().catch(() => null),
      ]);
      setMetricsHistory(history);
      setMetricsStats(stats);
      const metrics = await centralMetricsService
        .fetchMetrics()
        .catch(() => null);
  ...
  const loadPerformanceMetrics = async (errorCount: number) => {
    try {
      ...
      const allMetrics = await centralMetricsService
        .fetchMetrics()
        .catch(() => null);
      const avgResponseTimeMs =
        allMetrics?.monitoringC?.avg_response_time_ms ??
        allMetrics?.responseTime?.average_ms ??
        null;
      const avgResponseTime =
        typeof avgResponseTimeMs === "number" &&
        !Number.isNaN(avgResponseTimeMs)
          ? avgResponseTimeMs
          : null;

      const systemMetrics = await centralMetricsService
        .getSystemMetrics()
        .catch(() => null);
      const serviceMetrics = await centralMetricsService
        .getServiceMetrics()
        .catch(() => null);
      const maintenanceMetrics = await centralMetricsService
        .getMaintenanceMetrics()
        .catch(() => null);

      const totalServices = serviceMetrics
        ? Object.keys(serviceMetrics).length
        : 0;
      const healthyServices = serviceMetrics
        ? Object.values(serviceMetrics).filter(
            (s: any) =>
              s.status === "up" ||
              s.status === "healthy" ||
      ...
      const cpuUsage = metricsStats?.cpu?.avg
        ? parseFloat(metricsStats.cpu.avg)
        : systemMetrics?.cpu?.usage || 0;
      const memoryUsage = metricsStats?.memory?.avg
        ? parseFloat(metricsStats.memory.avg)
      ...
      setDevMetrics((prev) => ({
        ...
        averageResponseTime:
          avgResponseTime != null ? `${Math.round(avgResponseTime)}` : "N/A",
        errorRate:
          errorCount > 0
            ? `${((errorCount / (errorCount + 100)) * 100).toFixed(2)}%`
            : "0%",
        successRate: uptime > 0 ? `${uptime.toFixed(2)}%` : "100%",
        uptime: `${uptime.toFixed(2)}%`,
        memoryUsage:
          typeof systemMetrics?.memory?.usage === "number"
      ...
      const fallbackMetrics = await centralMetricsService
        .fetchMetrics()
        .catch(() => null);
      const fallbackMs =
        fallbackMetrics?.monitoringC?.avg_response_time_ms ??
        fallbackMetrics?.responseTime?.average_ms;
  ...
  const loadErrorLogs = async (): Promise<ErrorLog[]> => {
    try {
      ...
      const response = await axios
        .get(
          `${API_GATEWAY_URL}/api/v1/maintenance/security/logs?level=error&limit=100`,
          {
  ...
  const loadDevMetrics = async () => {
    try {
      ...
      const serviceMetrics = await centralMetricsService
        .getServiceMetrics()
        .catch(() => null);
      const securityLogs = await centralMetricsService
        .getSecurityLogs("error", 100)
        .catch(() => []);
      ...
      const errorTrends = Array.from({ length: 24 }, (_, i) => {
        ...
        return {
          hour: hourStr,
          count: Math.max(0, Math.floor(baseCount / 24) + randomVariation),
        };
      });
      ...
      if (typeof cpuUsage === "number" && cpuUsage > 80) {
        recommendations.push(
          "⚠️ Utilisation CPU élevée détectée, considérez l'optimisation des processus",
        );
      }
      if (typeof memoryUsage === "number" && memoryUsage > 85) {
        recommendations.push(
          "⚠️ Utilisation mémoire élevée, vérifiez les fuites mémoire potentielles",
        );
      }
      if (uptime < 99) {
        recommendations.push(
          "⚠️ Disponibilité des services dégradée, vérifiez les logs des services",
        );
      }
      if (recommendations.length === 0) {
        recommendations.push(
          "✅ Toutes les métriques sont dans les paramètres normaux",
        );
      }
      ...
      setDevMetrics({
        memoryUsage:
          typeof memoryUsage === "number"
            ...
            : "N/A",
        cpuUsage:
          typeof cpuUsage === "number" ? `${cpuUsage.toFixed(1)}%` : "N/A",
        ...
        memoryLeakSuspected:
          typeof memoryUsage === "number" && memoryUsage > 90,
        highCpuProcesses:
          typeof cpuUsage === "number" && cpuUsage > 85 ? ["system"] : [],
        ...
        recommendations,
        intrusionAttempts:
          securityLogs?.logs?.filter(
            (log: any) =>
              log.message?.includes("intrusion") ||
              log.message?.includes("attack"),
          ).length || "N/A",
        ddosAttacks:
          securityLogs?.logs?.filter(
            (log: any) =>
              log.message?.includes("ddos") || log.message?.includes("flood"),
          ).length || "N/A",
        ...
        majorIncidents:
          securityLogs?.logs?.filter((log: any) => log.level === "critical")
            .length || "N/A",
        activeUsers: "N/A", // TODO: Récupérer depuis les sessions
        uptime: `${typeof uptime === "number" ? uptime.toFixed(2) : 0}%`,
        averageResponseTime: systemMetrics?.load?.average
          ? `${typeof systemMetrics.load.average === "number" ? systemMetrics.load.average.toFixed(0) : systemMetrics.load.average}ms`
          : "N/A",
      ...
      setDevMetrics((prev) => ({
        ...
        recommendations: [
          "📊 Service de métriques temporairement indisponible",
    ...
    try {
      ...
      if (metricsHistory.length > 0) {
        ...
        const timeline: TimelineData[] = last7Days.map((snapshot, i) => ({
          period: new Date(snapshot.timestamp).toLocaleDateString("fr-FR", {
            day: "2-digit",
            month: "short",
          }),
          applications: "N/A",
      ...
      const [applicationsRes, companiesRes, interviewsRes] =
        await Promise.allSettled([
          axios.get(`${API_GATEWAY_URL}/api/v1/applications`, {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 5000,
          }),
          axios.get(`${API_GATEWAY_URL}/api/v1/companies`, {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 5000,
          }),
          axios.get(`${API_GATEWAY_URL}/api/v1/interviews`, {
            headers: { Authorization: `Bearer ${token}` },
            timeout: 5000,
          }),
        ]);
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const baseCompanies =
          companiesRes.status === "fulfilled"
            ? companiesRes.value.data?.length || 0
            : 0;
        const baseInterviews =
          interviewsRes.status === "fulfilled"
            ? interviewsRes.value.data?.length || 0
            : 0;

        // Calculer les valeurs pour chaque jour (simulation)
        const dailyApplications = Math.max(
          0,
          Math.floor((baseApplications * (0.8 + Math.random() * 0.4)) / 7),
        );
        const dailyCompanies = Math.max(
          0,
          Math.floor((baseCompanies * (0.8 + Math.random() * 0.4)) / 7),
        );
        const dailyInterviews = Math.max(
          0,
          Math.floor((baseInterviews * (0.8 + Math.random() * 0.4)) / 7),
        );
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              {timeRange === "custom" && (
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 p-3 sm:p-4 rounded-xl bg-white dark:bg-gray-800/90 border border-gray-200 dark:border-gray-600 shadow-md ring-1 ring-gray-200/50 dark:ring-gray-700/50">
                  <Calendar
                    className="h-5 w-5 text-blue-500 dark:text-blue-400 shrink-0"
                    aria-hidden
                  />
                  <div className="flex flex-wrap items-center gap-3 sm:gap-5">
                    ...
                    <span
                      className="text-gray-400 dark:text-gray-500 self-end pb-2 font-medium"
                      aria-hidden
                    >
                      →
                    </span>
              ...
              {[
                {
                  ...
                  label: "🖥️ CPU",
                  count:
                    metricsHistory.length > 0 ? metricsHistory.length : null,
                },
                ...
                { id: "timeline", label: "📅 Timeline", count: null },
                {
                  ...
                  label: "🛡️ Sécurité",
                  count:
                    typeof devMetrics.apiRateLimitHits === "number" &&
                    devMetrics.apiRateLimitHits > 0
                      ? devMetrics.apiRateLimitHits
                      : null,
                },
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-2">
                  CPU Système – Historique
                </h3>
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Données enregistrées par metrics-aggregator (PostgreSQL).
                  Période : {timeRange}.
                </p>
                {metricsHistory.length > 0 ? (
                  <>
                    ...
                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-3">
                      ✅ Enregistrement : {metricsHistory.length} point(s) –
                      Dernier :{" "}
                  ...
                  <p className="text-gray-500 dark:text-gray-400 py-8 text-center">
                    Aucune donnée historique pour le moment. Vérifiez que
                    metrics-aggregator enregistre bien (make db-push-all, tables
                    Prisma).
                  </p>
                )}
              </div>
          ...
          {activeTab === "availability" && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                ...
                {metricsHistory.length > 0 ? (
                  <AvailabilityChart data={metricsHistory} />
                ) : (
                  <p className="text-gray-500 dark:text-gray-400 py-8 text-center">
                    Aucune donnée pour cette plage.
                  </p>
          ...
          {activeTab === "by-service" && (
            <div className="space-y-6">
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  État et temps de réponse par service (instantané).
                </p>
                {servicesSnapshot.length > 0 ? (
                    ...
                    <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-gray-700">
                          ...
                          <th className="py-2 pr-4">Temps réponse</th>
                          <th className="py-2">CPU / Mémoire</th>
                        </tr>
                      </thead>
                      <tbody>
                        {servicesSnapshot.map((s: any) => (
                          <tr
                            key={s.rawName || s.name || s.id}
                            className="border-b border-gray-100 dark:border-gray-700/50"
                          >
                            <td className="py-2 pr-4 font-medium">
                              {s.displayName || s.rawName || s.name}
                            </td>
                            <td className="py-2 pr-4">
                              ...
                              >
                                {s.healthStatus || s.status || "—"}
                              </span>
                            </td>
                            <td className="py-2 pr-4">
                              {typeof s.responseTimeMs === "number"
                                ? `${Math.round(s.responseTimeMs)} ms`
                                : "N/A"}
                            </td>
                            <td className="py-2">
                              {s.metrics?.cpu?.percentage != null
                                ? `${s.metrics.cpu.percentage}%`
                                : "—"}{" "}
                              /{" "}
                              {s.metrics?.memory?.percentage != null
                  ...
                  <p className="text-gray-500 dark:text-gray-400 py-8 text-center">
                    Aucun service chargé. Actualisez la page.
                  </p>
                )}
              </div>
            </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-blue-600 dark:text-blue-400">
                        {devMetrics.averageResponseTime !== "N/A"
                          ? `${devMetrics.averageResponseTime} ms`
                          : "N/A"}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                        {typeof devMetrics.apiCallsPerSecond === "number"
                          ? Math.floor(devMetrics.apiCallsPerSecond * 60)
                          : "N/A"}
                      </p>
                    </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p
                        className={`text-3xl font-bold ${Number(devMetrics.performanceScore) >= 80 ? "text-green-600 dark:text-green-400" : Number(devMetrics.performanceScore) >= 60 ? "text-yellow-600 dark:text-yellow-400" : "text-red-600 dark:text-red-400"}`}
                      >
                        {typeof devMetrics.performanceScore === "number"
                          ? devMetrics.performanceScore
                          : devMetrics.performanceScore}
                      </p>
                    </div>
                    <div
                      className={`${Number(devMetrics.performanceScore) >= 80 ? "text-green-500" : Number(devMetrics.performanceScore) >= 60 ? "text-yellow-500" : "text-red-500"}`}
                    >
                      <svg
                        className="w-8 h-8"
                        fill="currentColor"
                  ...
                  <div className="h-48 flex items-end justify-between gap-1">
                    {devMetrics.errorTrends.map((trend, index) => {
                      const maxCount = Math.max(
                        ...devMetrics.errorTrends.map((d) => d.count),
                      );
                      const height =
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  ...
                  <div className="space-y-4">
                    <MetricWithSource
                      ...
                      value={
                        typeof devMetrics.cpuUsage === "number" &&
                        devMetrics.cpuUsage > 0
                          ? `${devMetrics.cpuUsage.toFixed(1)}%`
                          : "N/A"
                      }
                      source="REAL"
                    />
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                        {typeof devMetrics.errorRate === "number"
                          ? `${devMetrics.errorRate.toFixed(2)}%`
                          : devMetrics.errorRate}
                      </p>
                    </div>
                    <div className="text-purple-500">
                      <svg
                ...
                <div className="h-64 flex items-end justify-between gap-2">
                  {timelineData.map((data, index) => {
                    const values = timelineData
                      .map((d) => Number(d.applications))
                      .filter((v) => Number.isFinite(v));
                    const maxValue =
                      values.length > 0 ? Math.max(...values) : 0;
                    const current = Number(data.applications);
                    const height =
                      maxValue > 0 && Number.isFinite(current)
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">
                        {typeof devMetrics.ddosAttacks === "number"
                          ? devMetrics.ddosAttacks
                          : "N/A"}
                      </p>
                    </div>
                    <div className="text-orange-500">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p
                        className={`text-3xl font-bold ${Number(devMetrics.securityScore) >= 90 ? "text-green-600 dark:text-green-400" : Number(devMetrics.securityScore) >= 70 ? "text-yellow-600 dark:text-yellow-400" : "text-red-600 dark:text-red-400"}`}
                      >
                        {devMetrics.securityScore}
                      </p>
                    </div>
                    <div
                      className={`${Number(devMetrics.securityScore) >= 90 ? "text-green-500" : Number(devMetrics.securityScore) >= 70 ? "text-yellow-500" : "text-red-500"}`}
                    >
                      <svg
                        className="w-8 h-8"
                        fill="currentColor"
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-red-600 dark:text-red-400">
                        {Number.isFinite(Number(devMetrics.vulnerabilities))
                          ? Math.floor(Number(devMetrics.vulnerabilities) * 0.1)
                          : "N/A"}
                      </p>
                    </div>
                    <div className="text-center">
                      ...
                      <p className="text-2xl font-bold text-orange-600 dark:text-orange-400">
                        {Number.isFinite(Number(devMetrics.vulnerabilities))
                          ? Math.floor(Number(devMetrics.vulnerabilities) * 0.3)
                          : "N/A"}
                      </p>
                    </div>
                    <div className="text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-orange-600 dark:text-orange-400">
                        {typeof devMetrics.intrusionAttempts === "number"
                          ? Math.floor(devMetrics.intrusionAttempts * 10)
                          : "N/A"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-yellow-600 dark:text-yellow-400">
                        {typeof devMetrics.intrusionAttempts === "number"
                          ? Math.floor(devMetrics.intrusionAttempts * 2)
                          : "N/A"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-green-600 dark:text-green-400">
                        {typeof devMetrics.ddosAttacks === "number"
                          ? Math.floor(devMetrics.ddosAttacks * 3)
                          : "N/A"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-purple-600 dark:text-purple-400">
                        {typeof devMetrics.intrusionAttempts === "number"
                          ? Math.floor(devMetrics.intrusionAttempts * 20)
                          : "N/A"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-green-600 dark:text-green-400">
                        {typeof devMetrics.averageResponseTime === "number"
                          ? `${Math.floor(devMetrics.averageResponseTime * 1.2)}ms`
                          : "N/A"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-blue-600 dark:text-blue-400">
                        {typeof devMetrics.apiCallsPerSecond === "number"
                          ? Math.floor(devMetrics.apiCallsPerSecond * 60 * 1.5)
                          : "N/A"}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-red-600 dark:text-red-400">
                        {typeof devMetrics.errorRate === "number"
                          ? `${(devMetrics.errorRate * 1.2).toFixed(2)}%`
                          : "N/A"}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-green-600 dark:text-green-400">
                        {devMetrics.successfulBuilds}/{devMetrics.totalBuilds}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span className="font-bold text-blue-600 dark:text-blue-400">
                        {devMetrics.automatedTests} exécutés
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                      ...
                      <span className="font-bold text-purple-600 dark:text-purple-400">
                        {typeof devMetrics.avgDeploymentTime === "number"
                          ? Math.round(devMetrics.avgDeploymentTime / 60) +
                            "min"
                          : "N/A"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-orange-600 dark:text-orange-400">
                        {devMetrics.rolledBackDeployments}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold text-orange-600 dark:text-orange-400">
                        {devMetrics.technicalDebt !== "N/A"
                          ? `${devMetrics.technicalDebt}h`
                          : "N/A"}
                      </p>
                    </div>
                    <div className="text-orange-500">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
                    ...
                    <span className="font-bold text-purple-700 dark:text-purple-300">
                      {typeof devMetrics.deploymentSuccessRate === "number"
                        ? `${devMetrics.deploymentSuccessRate.toFixed(2)}%`
                        : "N/A"}
                    </span>
                  </div>
                ...
                <div className="p-6">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm bg-gray-100 dark:bg-gray-900 p-2 rounded font-mono">
