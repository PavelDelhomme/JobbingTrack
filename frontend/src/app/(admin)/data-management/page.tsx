...
export default function DataManagementPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "browse" | "export" | "import" | "operations" | "tests"
  >("browse");
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      if (items.length === 0) {
        ...
        console.log(
          `✅ ${items.length} enregistrements chargés pour ${selectedTable}`,
        );
  ...
  const deleteRow = async (id: string) => {
    if (
      !confirm(
        "⚠️ Êtes-vous sûr de vouloir supprimer cet enregistrement ?\n\nCette action est irréversible.",
      )
    )
      return;
  ...
  const bulkDelete = async () => {
    if (
      !confirm(
        "⚠️ ATTENTION : Voulez-vous vraiment effectuer une suppression en masse ?",
      )
    )
      return;

    alert(
      "Fonctionnalité de suppression en masse à implémenter avec sélection de lignes",
    );
  };
  ...
  const runDBTests = async () => {
    ...
    await runSingleTest(0, async () => {
      const response = await fetch(
        "http://localhost:8080/api/v1/admin/test-db/connection",
        {
    ...
    for (let i = 0; i < services.length; i++) {
      await runSingleTest(i + 1, async () => {
        const response = await fetch(
          `http://localhost:8080/api/v1/admin/test-db/schema/${services[i]}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
    ...
    await runSingleTest(5, async () => {
      const response = await fetch(
        "http://localhost:8080/api/v1/admin/test-db/migration-test",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const runSingleDBTest = async (index: number) => {
    const testFunctions = [
      ...
      async () => {
        const response = await fetch(
          "http://localhost:8080/api/v1/admin/test-db/schema/auth",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      async () => {
        const response = await fetch(
          "http://localhost:8080/api/v1/admin/test-db/schema/application",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      async () => {
        const response = await fetch(
          "http://localhost:8080/api/v1/admin/test-db/schema/call",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      async () => {
        const response = await fetch(
          "http://localhost:8080/api/v1/admin/test-db/schema/notification",
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
        ...
        <div className="mb-4 md:mb-8">
          ...
          <div className="border-b border-gray-200 dark:border-gray-700 overflow-x-auto">
            <nav className="-mb-px flex space-x-4 sm:space-x-6 md:space-x-8">
              {["browse", "export", "import", "operations", "tests"].map(
                (tab) => (
                  <button
                    key={tab}
                  ...
                  <div className="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {tableData && tableData.rows.length > 0 && (
                        <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400 mt-1">
                          {tableData.total} enregistrement
                          {tableData.total > 1 ? "s" : ""} •{" "}
                          {tableData.columns.length} colonne
                    ...
                    {tableData && tableData.rows.length > 0 && (
                      <div className="text-xs text-gray-500 dark:text-gray-400">
                        ...
                        {Math.min(page * limit, tableData.total)} sur{" "}
                        {tableData.total}
                      </div>
                    )}
                  </div>
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-gray-600 dark:text-gray-400 mt-1">
                    Exportez vos données dans différents formats avec sélection
                    précise
                  </p>
                </div>
                <AdvancedDataExporter
              ...
              {tableData && tableData.rows.length > 0 && (
                <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                  <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                    📊 Aperçu des données - {tableData.total} enregistrement
                    {tableData.total > 1 ? "s" : ""}
                  </h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
                      <div className="text-lg font-bold text-green-600 dark:text-green-400">
                        {Math.round((tableData.rows.length / 1024) * 100) / 100}{" "}
                        KB
                      </div>
                      <div className="text-gray-600 dark:text-gray-400">
                        Taille estimée
                    ...
                    <div className="text-center p-3 bg-orange-50 dark:bg-orange-900/20 rounded-lg">
                      <div className="text-lg font-bold text-orange-600 dark:text-orange-400">
                        {
                          tableData.rows.filter((r) => r.is_active !== false)
                            .length
                        }
                      </div>
                      <div className="text-gray-600 dark:text-gray-400">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
                  <p className="text-sm text-yellow-800 dark:text-yellow-200">
                    ⚠️ <strong>Attention :</strong> L'import de données peut
                    écraser des enregistrements existants. Assurez-vous d'avoir
                    une sauvegarde.
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-gray-600 dark:text-gray-400 mt-1">
                      Vérification de la connexion PostgreSQL, des schémas
                      Prisma et des migrations
                    </p>
                  </div>
                  <button
                    onClick={runDBTests}
                ...
                <div className="mt-6 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                  <p className="text-sm text-blue-800 dark:text-blue-200">
                    💡 <strong>Note :</strong> Ces tests vérifient que la base
                    de données et tous les schémas Prisma sont correctement
                    configurés. Cliquez sur "▶️ Lancer tous les tests" pour une
                    vérification complète, ou sur le bouton ▶️ de chaque test
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-2xl">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                Fonctionnalité à implémenter : formulaire dynamique basé sur le
                schéma de la table
              </p>
        ...
        <AdvancedEditModal
          ...
          onSave={async (updatedData) => {
            ...
            switch (selectedTable) {
              ...
              default:
                throw new Error(
                  `Table ${selectedTable} non supportée pour la modification`,
                );
            }
