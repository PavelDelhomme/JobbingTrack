      ...
      if (!data?.services?.length) {
        try {
          ...
          if (res2.ok) {
            ...
            const servicesList =
              json.servicesList ||
              (json.services ? Object.values(json.services) : []);
            const containers =
              json.containers && typeof json.containers === "object"
                ? Object.entries(json.containers)
                : [];
            if (servicesList.length > 0) {
              data = { services: mapAggToServices(servicesList) };
            } else if (containers.length > 0) {
              data = {
                services: mapAggToServices(
                  containers.map(([name, m]: [string, any]) => ({
                    ...
                    metrics:
                      ...
                      (m.cpu
                        ? {
                            cpu_percent: m.cpu.percentage || 0,
                            memory_percent: m.memory?.percentage || 0,
                            memory_usage_mb: m.memory?.usage || 0,
                            pids: null,
                          }
                        : null),
                  })),
      ...
      if (data && data.services) {
        ...
        if (loadedServices.length > 0) {
          if (isInitial) {
            ...
            const runningServices = loadedServices.filter(
              (s: Service) => s.is_running,
            );
          ...
          {services.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-gray-400 dark:text-gray-500 mb-4">
                Les services peuvent prendre quelques instants à charger.
                Vérifiez que le service metrics-aggregator est démarré.
              </p>
              <button
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-100">
                        {service.metrics ? (
                          <div className="flex items-center gap-1">
                            ...
                            {service.metrics.memory_usage_mb.toFixed(0)} MB (
                            {service.metrics.memory_percent.toFixed(1)}%)
                          </div>
                        ) : (
                          "-"
                        )}
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          {(() => {
                            ...
                            const isCritical =
                              CRITICAL_SERVICES.includes(serviceName);
                              ...
                              return (
                                <button
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    if (isCritical) {
                                      alert(
                                        `⚠️ Ce service est critique et ne peut pas être démarré depuis l'interface pour des raisons de sécurité.\n\nService: ${serviceName}\n\nUtilisez la ligne de commande si vous devez absolument le démarrer.`,
                                      );
                                      return;
                                    }
                                    ...
                                    try {
                                      const response = await fetch(
                                        `${METRICS_URL}/api/v1/docker/service/${serviceName}/start`,
                                        {
                                          method: "POST",
                                          headers: {
                                      ...
                                      if (data.success) {
                                        alert(
                                          `✅ Service ${serviceName} démarré avec succès !`,
                                        );
                                        setTimeout(
                                          () => loadServices(false),
                                          1000,
                                      ...
                                      if (
                                        ...
                                        alert(
                                          "Erreur de connexion : Le service metrics-aggregator pourrait être indisponible. Vérifiez qu'il est démarré.",
                                        );
                                      } else {
                                        console.error(
                                          "Erreur démarrage service:",
                                          error,
                                        );
                                        alert(
                                          `Erreur lors du démarrage du service: ${error.message || "Erreur inconnue"}`,
                                        );
                                      }
                                    }
                                  }}
                                  title={
                                    isCritical
                                      ? "Service critique - Démarrage désactivé"
                                      : "Démarrer le service"
                                  }
                                  disabled={isCritical}
                                >
                              ...
                              <>
                                <button
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    if (isCritical) {
                                      alert(
                                        `⚠️ Ce service est critique et ne peut pas être redémarré depuis l'interface pour des raisons de sécurité.\n\nService: ${serviceName}\n\nUtilisez la ligne de commande si vous devez absolument le redémarrer.`,
                                      );
                                      return;
                                    }
                                    ...
                                    try {
                                      const response = await fetch(
                                        `${METRICS_URL}/api/v1/docker/service/${serviceName}/restart`,
                                        {
                                          method: "POST",
                                          headers: {
                                            "Content-Type": "application/json",
                                      ...
                                      if (
                                        ...
                                        console.error(
                                          "Erreur redémarrage service:",
                                          error,
                                        );
                                        alert(
                                          `Erreur lors du redémarrage du service: ${error.message || "Erreur inconnue"}`,
                                        );
                                      }
                                    }
                                  }}
                                  title={
                                    isCritical
                                      ? "Service critique - Redémarrage désactivé"
                                ...
                                <button
                                  ...
                                  onClick={async (e) => {
                                    ...
                                    if (isCritical) {
                                      alert(
                                        `⚠️ Ce service est critique et ne peut pas être arrêté depuis l'interface pour des raisons de sécurité.\n\nService: ${serviceName}\n\nUtilisez la ligne de commande si vous devez absolument l'arrêter.`,
                                      );
                                      return;
                                    }
                                    ...
                                    try {
                                      const response = await fetch(
                                        `${METRICS_URL}/api/v1/docker/service/${serviceName}/stop`,
                                        {
                                          method: "POST",
                                          headers: {
                                            "Content-Type": "application/json",
                                      ...
                                      if (data.success) {
                                        alert(
                                          `✅ Service ${serviceName} arrêté avec succès !`,
                                        );
                                        setTimeout(
                                          () => loadServices(false),
                                          1000,
                                      ...
                                      if (
                                        ...
                                        console.error(
                                          "Erreur arrêt service:",
                                          error,
                                        );
                                        alert(
                                          `Erreur lors de l'arrêt du service: ${error.message || "Erreur inconnue"}`,
                                        );
                                      }
                                    }
                                  }}
                                  title={
                                    isCritical
                                      ? "Service critique - Arrêt désactivé"
                                      : "Arrêter le service"
                                  }
                                  disabled={isCritical}
                                >
