...
jest.mock("next/dynamic", () => ({
  __esModule: true,
  default: () => {
    const { MonitoringServiceHistoryCharts } = jest.requireActual(
      "@/components/monitoring/MonitoringServiceHistoryCharts",
    );
...
jest.mock("@/lib/services/centralMetricsService", () => ({
  centralMetricsService: {
    ...
    getAggregatorMetrics: jest
      .fn()
      .mockRejectedValue(new Error("aggregator unavailable in test")),
  },
}));
...
const mockServiceLogs = {
  ...
  errorLines: ["[ERROR] Failed to connect to database"],
};
...
describe("ServiceDetailPage", () => {
  ...
  describe("Monitoring détail (A1 — disque hôte, Block I/O, réutilisation API)", () => {
    it("affiche Block I/O conteneur, disque hôte et encart réutilisation", async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(/Disque hôte \(contexte\)/i),
        ).toBeInTheDocument();
        expect(
          screen.getByText(/Réutilisation monitoring/i),
        ).toBeInTheDocument();
      });
      await waitFor(() => {
  ...
  describe("Cartes de métriques", () => {
    it("devrait afficher la métrique CPU", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getAllByText(/Utilisation CPU/i).length,
    ...
    it("devrait afficher le nombre de processus actifs", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Processus \/ tâches \(PIDs\)/i),
        ).toBeInTheDocument();
    ...
    it("devrait afficher le trafic réseau avec RX et TX", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Trafic cumulé interface/i),
        ).toBeInTheDocument();
  ...
  describe("Historique des performances", () => {
    it("devrait afficher la section historique", async () => {
      ...
      expect(
        await screen.findByRole("heading", {
          level: 2,
          name: /Historique des Performances/i,
        }),
      ).toBeInTheDocument();
    });
    ...
    it("devrait afficher le nombre de points de données", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /\d+ points \(fichiers agrégateur \+ session courante\)/i,
          ),
        ).toBeInTheDocument();
      });
    });
    ...
    it("devrait afficher un message quand pas d'historique", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            /Aucun historique de performance disponible pour ce service/i,
          ),
        ).toBeInTheDocument();
      });
    });
  ...
  describe("Logs en temps réel", () => {
    it("devrait afficher la section logs", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Logs du Service \(Temps Réel\)/i),
        ).toBeInTheDocument();
      });
    });
    ...
    it("devrait afficher les lignes de logs", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Server started on port 8001/i),
        ).toBeInTheDocument();
        expect(
          screen.getByText(/High memory usage detected/i),
        ).toBeInTheDocument();
      });
    });
    ...
    it("devrait afficher un message quand pas de logs", async () => {
          ...
          return Promise.resolve({
            ...
            json: () =>
              Promise.resolve({
                system: {
                  disk: [
                    { mount: "/", usage_percent: 10, used: 1, total: 100 },
                  ],
                },
              }),
          } as Response);
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/Aucun log disponible pour ce service/i),
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe("Cas spéciaux", () => {
    it("devrait gérer un service avec le préfixe jobbingtrack-", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(/jobbingtrack-auth-service/i),
        ).toBeInTheDocument();
      });
    });
    ...
    it("devrait gérer un service unhealthy", async () => {
      ...
      expect(
        await screen.findByText(/Service non disponible/i, undefined, {
          timeout: 5000,
        }),
      ).toBeInTheDocument();
      expect(
        await screen.findByText(/Docker: unhealthy/i, undefined, {
          timeout: 3000,
        }),
      ).toBeInTheDocument();
    });
