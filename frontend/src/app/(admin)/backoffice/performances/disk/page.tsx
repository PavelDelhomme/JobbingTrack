  ...
  const chartData = useMemo(() => {
    ...
    return rows
      .map((raw) => {
        ...
        const tsIso =
          normalizeMetricTimestampToIso(r.timestamp as string) ||
          String(r.timestamp ?? "");
        const timeMs = metricRowToTimeMs(r, tsIso);
        const usageRaw = r.diskUsagePercent ?? r.disk_usage_percent;
        const usage =
          usageRaw != null && Number.isFinite(Number(usageRaw))
            ? Number(usageRaw)
            : NaN;
        const used =
          pickGb(
    ...
    <AdminLayout>
      <div className="p-6 mx-auto max-w-6xl space-y-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Historique snapshots disque sur les dernières 24h (usage %, volume
          utilisé/total). Pour le détail Block I/O par service, utiliser la page
          détail service.
        </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {latest && Number.isFinite(latest.usage)
              ...
              <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {latest && Number.isFinite(latest.used)
                    ? `${latest.used.toFixed(1)} GB`
                    : "—"}
                </p>
              </div>
              <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                  {latest && Number.isFinite(latest.total)
                    ? `${latest.total.toFixed(1)} GB`
                    : "—"}
                </p>
              </div>
            </div>
