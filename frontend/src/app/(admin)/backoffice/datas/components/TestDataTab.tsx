...
export default function TestDataTab() {
  ...
  const tagLikely = async () => {
    ...
    if (
      !confirm(
        'Marquer comme données de test (isTestData) les comptes évidents (@jobbingtrack.test, user*@jobbingtrack.com, TEST_USER_EMAIL, etc.) et les entités liées, plus les enregistrements contenant [TEST_DATA_TAG: dans les notes ?\n\nLes comptes admin et PROTECTED_USER_EMAILS ne sont pas modifiés.'
      )
    ) {
      return
    }
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-gray-600 dark:text-gray-400">
          ...
          <strong>{summary?.protectedAdminEmail || 'admin@jobbingtrack.com'}</strong>
