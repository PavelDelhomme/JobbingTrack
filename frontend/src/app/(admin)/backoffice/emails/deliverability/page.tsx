...
export default function EmailDeliverabilityPage() {
  ...
  const handleTestDNS = async () => {
    if (!domain || !domain.trim()) {
      setDnsResults({
        ...
        mx: {
          status: "error",
          records: [],
          error: "Veuillez entrer un domaine à tester",
        },
        spf: {
          status: "error",
          record: null,
    ...
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/v1/emails/test-dns?domain=${encodeURIComponent(domain.trim())}`,
        {
  ...
  const handleSendTestEmail = async (
    emailType: "test" | "reset" | "verification" = "test",
  ) => {
    if (!testEmail) {
      setSendResult({
        success: false,
