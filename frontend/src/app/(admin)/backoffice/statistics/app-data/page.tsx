    ...
    <AdminLayout>
      <div className="p-6 mx-auto max-w-5xl space-y-6">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          Données issues de <code className="text-xs">GET /api/v1/statistics</code> et{' '}
          <code className="text-xs">/api/v1/statistics/timeline</code> (gateway). Complète la vue d’ensemble sans la
          dupliquer.
        </p>
        ...
        {loading ? (
          ...
          <p className="text-sm text-amber-700 dark:text-amber-300">Aucune statistique renvoyée par l’API.</p>
        ) : (
          <>
            ...
            {chartRows.length > 1 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={chartRows} margin={{ top: 8, right: 16, left: 0, bottom: 8 }}>
                      ...
                      <Tooltip
                        ...
                        labelFormatter={(_, p) => {
                          const ts = (p as { payload?: { label?: string } }[])?.[0]?.payload?.label
                          return ts ?? '—'
                        }}
                      />
                      <Line type="monotone" dataKey="applications" name="Candidatures" stroke="#7c3aed" dot={false} />
                      <Line type="monotone" dataKey="users" name="Utilisateurs" stroke="#2563eb" dot={false} />
                      <Line type="monotone" dataKey="companies" name="Entreprises" stroke="#059669" dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Pas assez de points timeline pour tracer un graphique (collecte ou rôle API).
              </p>
            )}
