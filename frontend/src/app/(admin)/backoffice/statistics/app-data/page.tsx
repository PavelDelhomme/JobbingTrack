  ...
  return (
    ...
    >
      {loading ? (
        ...
        >
          Aucune statistique renvoyée par l’API. À traiter : brancher les séries
          métier utiles (candidatures, utilisateurs actifs, entreprises,
          contacts) et afficher un état vide explicite.
        </div>
      ) : (
        <>
          ...
          {chartRows.length > 1 ? (
            ...
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-5 text-sm text-blue-950 dark:border-blue-900/60 dark:bg-blue-950/30 dark:text-blue-100">
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
                <SnapshotMetric
                  label="Candidatures"
                  value={chartRows[0].applications}
                />
                <SnapshotMetric
                  label="Utilisateurs"
                  value={chartRows[0].users}
                />
                <SnapshotMetric
                  label="Entreprises"
                  value={chartRows[0].companies}
                />
                <SnapshotMetric
                  label="Contacts"
                  value={chartRows[0].contacts}
                />
                <SnapshotMetric
                  label="Entretiens"
                  value={chartRows[0].interviews}
                />
                <SnapshotMetric label="Appels" value={chartRows[0].calls} />
                <SnapshotMetric
                  label="Relances"
                  value={chartRows[0].followups}
                />
                <SnapshotMetric
                  label="Événements"
                  value={chartRows[0].events}
                />
              </div>
