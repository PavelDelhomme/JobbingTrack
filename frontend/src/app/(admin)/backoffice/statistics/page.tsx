        ...
        const historicalStats: Statistics = {
          system: {
            ...
            availability: parseFloat(
              String(lastMetric.availability_percent || 100),
            ),
            totalRequests: 0,
          ...
          performance: {
            averageResponseTime: parseFloat(
              String(lastMetric.response_time_avg || 0),
            ),
    ...
    try {
      ...
      const systemStats = {
        ...
        memory: {
          ...
          average: parseFloat(
            String((metricsStats as any)?.memory?.avg || "0"),
          ),
        ...
        network: {
          totalRx: parseFloat(
            String(metrics?.system?.network?.total_rx_mb || "0"),
          ),
          totalTx: parseFloat(
            String(metrics?.system?.network?.total_tx_mb || "0"),
          ),
          avgRx: parseFloat(
            String((metricsStats as any)?.network?.rx_mb_avg || "0"),
          ),
          avgTx: parseFloat(
            String((metricsStats as any)?.network?.tx_mb_avg || "0"),
          ),
        },
        availability: parseFloat(
          String(metrics?.health?.availability_percent || "100"),
        ),
        totalRequests: parseInt(
          String((metricsStats as any)?.requests?.total || "0"),
        ),
        totalErrors: parseInt(
          String((metricsStats as any)?.errors?.total || "0"),
        ),
      };
      ...
      try {
        ...
        appStats = await cacheManager.get<ApplicationStatistics>(
          appStatsCacheKey,
          { ttl: 30000 },
        ); // Cache 30 secondes
        ...
        const appStatsCacheKey = "statistics_app_stats";
        appStats =
          (await cacheManager.get<ApplicationStatistics>(appStatsCacheKey)) ||
          null;
      }

      // Calculer le temps de réponse moyen depuis les services
      const servicesWithResponseTime = servicesArray.filter(
        (s) => s.responseTime > 0,
      );
      ...
      const mockAppStats = {
        ...
        performance: {
          ...
          successRate:
            100 - parseFloat((metricsStats as any)?.errors?.rate || "0.0"),
          ...
          <div>
            ...
            <ul className="space-y-2 text-gray-900 dark:text-gray-100">
              ...
              <li className="flex justify-between items-baseline">
                <span>Contacts</span>
                <span className="text-xl font-bold">
                  {(stats.contacts?.total ?? 0).toLocaleString()}
                </span>
              </li>
            </ul>
          </div>
              ...
              <table className="w-full text-sm text-left text-gray-700 dark:text-gray-300">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-gray-600">
                    ...
                    {stats.applications.today !== undefined &&
                      stats.applications.today !== null && (
                        <th className="py-2 px-2 font-medium text-center">
                          Aujourd&apos;hui
                        </th>
                      )}
                ...
                <tbody>
                  ...
                  {(stats.companies?.thisWeek !== undefined ||
                    stats.companies?.thisMonth !== undefined) && (
                    <tr className="border-b border-gray-100 dark:border-gray-700">
                      <td className="py-2 pr-4">Nouvelles entreprises</td>
                      <td className="py-2 px-2 text-center font-semibold">
                        {stats.companies.thisWeek ?? 0}
                      </td>
                      <td className="py-2 px-2 text-center font-semibold">
                  ...
                  {(stats.contacts?.thisWeek !== undefined ||
                    stats.contacts?.thisMonth !== undefined) && (
                    <tr className="border-b border-gray-100 dark:border-gray-700">
      ...
      <DashboardLayoutRegion variant="section">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2 text-center">
            Les graphiques temporels seront disponibles une fois l'historique
            collecté
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <RechartsPieChart>
              <Pie
                data={Object.entries(stats.applications.byStatus || {}).map(
                  ([status, count]) => ({
                    name: status,
                    value: count,
                  }),
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart
              data={Object.entries(stats.companies.byIndustry || {}).map(
                ([industry, count]) => ({
                  name:
                    industry.length > 15
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <ResponsiveContainer width="100%" height={150}>
                <RechartsPieChart>
                  <Pie
                    data={Object.entries(stats.applications.byType).map(
                    ...
                    label
                  >
                    {Object.entries(stats.applications.byType).map(
                      (entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={PIE_COLORS[index % PIE_COLORS.length]}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
            Max: {stats.system.cpu.max.toFixed(1)}% | Min:{" "}
            {stats.system.cpu.min.toFixed(1)}%
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
          ...
          <div className="mt-2 text-xs text-gray-500 dark:text-gray-400">
            Max: {stats.system.memory.max.toFixed(1)}% | Min:{" "}
            {stats.system.memory.min.toFixed(1)}%
          </div>
        </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                📊 Détails:{" "}
                {
                  stats.services.find((s: any) => s.name === selectedService)
                    ?.displayName
                }
              </h3>
              <button
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800 dark:text-blue-300">
              Les données réseau sont collectées depuis les métriques Docker et
              représentent le trafic total des conteneurs. RX = Réception
              (Download), TX = Transmission (Upload).
            </p>
          </div>
        </div>
      </div>
...
const LogsTab = memo(function LogsTab({
  ...
  const errorStatsArray = Object.values(errorStatsByService).map(
    (stat: any) => ({
      ...
      <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-800 dark:text-blue-300">
              Pour consulter les logs détaillés de chaque service, rendez-vous
              dans la section
...
function StatCard({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
      <div className="flex items-center justify-between mb-2">
        <div
          className={`p-2 rounded-lg ${colors[color as keyof typeof colors] ?? colors.blue}`}
        >
          {icon}
        </div>
