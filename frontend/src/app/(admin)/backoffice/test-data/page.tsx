...
export default function TestDataPage() {
  ...
  const [customConfig, setCustomConfig] = useState(
    PRESETS.find((p) => p.name === "standard")!.config,
  );
  ...
  const clearTestData = async (onlyTestData: boolean = true) => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text:
          error.response?.data?.error || "Erreur lors du nettoyage des données",
      });
    } finally {
      setLoading(false);
    }
              ...
              >
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-600 dark:text-gray-400">
                      {preset.config.companies}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <FileText className="h-3 w-3 text-gray-400" />
                    <span className="text-gray-600 dark:text-gray-400">
                      {preset.config.applications}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
              ...
              {Object.entries(customConfig).map(([key, value]) => (
                <div key={key}>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                    {key === "users" && (
                      <>
                        <Users className="h-4 w-4 inline mr-1" /> Utilisateurs
                      </>
                    )}
                    {key === "companies" && (
                      <>
                    ...
                    {key === "contacts" && (
                      <>
                        <Users className="h-4 w-4 inline mr-1" /> Contacts
                      </>
                    )}
                    {key === "interviews" && (
                      <>
                        <Calendar className="h-4 w-4 inline mr-1" /> Entretiens
                      </>
                    )}
                    {key === "followups" && (
                      <>
                        <Mail className="h-4 w-4 inline mr-1" /> Relances
                      </>
                    )}
                    {key === "calls" && (
                      <>
                        <Phone className="h-4 w-4 inline mr-1" /> Appels
                      </>
                    )}
                    {key === "events" && (
                      <>
                        <Clock className="h-4 w-4 inline mr-1" /> Événements
                      </>
                    )}
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-gray-300 cursor-pointer">
              ...
              />
              Volumes équilibrés (plus de contacts / relances / entretiens par
              rapport aux entreprises — recommandé pour les stats)
            </label>
            <button
              onClick={() =>
                generateTestData(
          ...
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm text-blue-700 dark:text-blue-300">
              <div>
                ...
                {showCustomConfig
                  ? customConfig.users
                  : selectedPresetData.config.users}{" "}
                utilisateurs
              </div>
              <div>
                🏢{" "}
                {showCustomConfig
                  ? customConfig.companies
                  : selectedPresetData.config.companies}{" "}
                entreprises
              </div>
              <div>
                📝{" "}
                {showCustomConfig
                  ? customConfig.applications
                  : selectedPresetData.config.applications}{" "}
                candidatures
              </div>
              <div>
                👤{" "}
                {showCustomConfig
                  ...
                  : selectedPresetData.config.contacts}{" "}
                contacts
              </div>
              <div>
                🎤{" "}
                {showCustomConfig
                  ? customConfig.interviews
                  : selectedPresetData.config.interviews}{" "}
                entretiens
              </div>
              <div>
                📧{" "}
                {showCustomConfig
                  ? customConfig.followups
                  : selectedPresetData.config.followups}{" "}
                relances
              </div>
              <div>
                📞{" "}
                {showCustomConfig
                  ? customConfig.calls
                  : selectedPresetData.config.calls}{" "}
                appels
              </div>
              <div>
                📅{" "}
                {showCustomConfig
                  ? customConfig.events
                  : selectedPresetData.config.events}{" "}
                événements
              </div>
            </div>
            <p className="text-xs text-blue-600 dark:text-blue-400 mt-3">
              ✨ Les événements seront automatiquement créés et liés aux
              candidatures, entretiens, relances et appels
            </p>
          </div>
        )}
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">
                Les données générées sont fictives et destinées uniquement aux
                environnements de développement et de test. Ne pas utiliser en
                production. Toutes les données générées sont taguées pour
