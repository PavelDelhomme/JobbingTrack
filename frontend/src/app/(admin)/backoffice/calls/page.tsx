...
import { Phone, Search, Plus, Edit, Calendar, RefreshCw } from 'lucide-react';
...
export default function CallsPage() {
  ...
  const loadCalls = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/api/v1/calls`, {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Gestion des Appels</h1>
            <p className="text-gray-600 dark:text-gray-400 mt-1">Gérez vos appels et suivis téléphoniques</p>
          </div>
          <button
            onClick={() => router.push('/backoffice/calls/new')}
                ...
                {filteredCalls.map((call) => (
                  <tr key={call.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                    <td className="px-6 py-4">
                      <div className="text-sm font-medium text-gray-900 dark:text-gray-100">{call.subject || 'N/A'}</div>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <button
                        onClick={() => router.push(`/backoffice/calls/${call.id}`)}
