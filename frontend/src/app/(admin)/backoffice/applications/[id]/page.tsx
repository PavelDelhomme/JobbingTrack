...
export default function BackofficeApplicationDetailPage() {
  ...
  const currentStatus = typeof application?.status === 'object' && application?.status !== null && 'code' in application.status
    ...
    : application ? String(application.status) : ''
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            {application.description && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Description du poste</h2>
          ...
          <div className="space-y-6">
            {application.company && (
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
                <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">🏢 Entreprise</h2>
                <p className="font-medium text-gray-900 dark:text-gray-100">{application.company.name}</p>
                {application.company.industry && <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">📊 {application.company.industry}</p>}
                {application.company.location && <p className="text-sm text-gray-600 dark:text-gray-400">📍 {application.company.location}</p>}
                {application.company.website && (
                  <a href={application.company.website} target="_blank" rel="noopener noreferrer" className="text-sm text-blue-600 dark:text-blue-400 mt-1 block">🔗 Site web</a>
                )}
              </div>
            )}
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6">
              <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Détails</h2>
              <div className="space-y-2 text-sm">
                {application.location && <DetailRow label="Localisation" value={application.location} />}
                {application.salary && <DetailRow label="Salaire" value={application.salary} />}
                {application.jobUrl && (
                  <div className="pt-2">
                    <a href={application.jobUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 dark:text-blue-400">🔗 Voir l'offre</a>
                  </div>
                )}
              </div>
            </div>
