...
export default function ServicesPage() {
  ...
  const REAL_SERVICES = [
    ...
    {
      name: "Auth Service",
      url: "http://localhost:3001/api/v1/auth/health",
      port: 3001,
      serviceType: "auth",
    },
    {
      name: "Application Service",
      url: "http://localhost:3002/api/v1/applications/health",
      port: 3002,
      serviceType: "application",
    },
    {
      name: "Company Service",
      url: "http://localhost:3003/api/v1/companies/health",
      port: 3003,
      serviceType: "company",
    },
    {
      name: "Contact Service",
      url: "http://localhost:3004/api/v1/contacts/health",
      port: 3004,
      serviceType: "contact",
    },
    {
      name: "Interview Service",
      url: "http://localhost:3005/api/v1/interviews/health",
      port: 3005,
    ...
    },
    {
      ...
      url: "http://localhost:3007/api/v1/dashboard/health",
      port: 3007,
      serviceType: "dashboard",
    },
    {
      name: "Call Service",
      url: "http://localhost:3008/api/v1/calls/health",
      port: 3008,
      serviceType: "call",
    },
    {
      name: "Event Service",
      url: "http://localhost:3009/api/v1/events/health",
      port: 3009,
      serviceType: "event",
    },
    {
      name: "FollowUp Service",
      url: "http://localhost:3010/api/v1/followups/health",
      port: 3010,
      serviceType: "followup",
    },
    {
      name: "Profile Service",
      url: "http://localhost:3011/api/v1/profile/health",
      port: 3011,
      serviceType: "profile",
    },
    {
      name: "Workflow Service",
      url: "http://localhost:3013/api/v1/workflow/health",
      port: 3013,
      serviceType: "workflow",
    },
    {
      name: "Metrics Aggregator",
      url: "http://localhost:8014/api/v1/health",
      port: 8014,
      serviceType: "metrics",
    },
    {
  ...
  const loadMaintenances = async () => {
    try {
      ...
      console.warn(
        "Service de maintenance non disponible, utilisation de valeurs par défaut",
      );
      setMaintenances({});
    }
  };
    ...
    if (metrics && metrics.services) {
      const updatedServices = REAL_SERVICES.map((service) => {
        ...
        const serviceMetrics =
          serviceKey && metrics.services
            ? metrics.services[serviceKey as string]
            : null;
        ...
        if (serviceMetrics && serviceMetrics.health) {
          return {
            ...
            status:
              serviceMetrics.health.status === "online"
                ? ("online" as const)
                : serviceMetrics.health.status === "offline"
                  ? ("offline" as const)
                  : ("online" as const), // Assume online si status non défini
            responseTime: serviceMetrics.health.responseTime || "N/A",
            version:
              serviceMetrics.health.version ||
              serviceMetrics.version ||
              "1.0.0",
  ...
  const averageResponseTime =
    services
      ...
      .reduce((acc, s) => acc + (s.responseTime as number), 0) /
    (services.filter(
      (s) => s.responseTime && typeof s.responseTime === "number",
    ).length || 1);
  ...
  const fetchServiceLogs = async (service: ServiceStatus) => {
    try {
      if (metrics && metrics.services) {
        ...
        const serviceKey = Object.keys(metrics.services || {}).find(
          (key: string) =>
        ...
        if (
          ...
          if (serviceData.health?.status) {
            logs.push(
              `[${new Date().toISOString()}] INFO: Service ${service.name} ${serviceData.health.status}`,
            );
          }

          if (serviceData.lastCheck) {
            logs.push(
              `[${serviceData.lastCheck}] INFO: Dernière vérification effectuée`,
            );
          }
          ...
          if (serviceData.metrics?.memory) {
            ...
            logs.push(
              `[${new Date().toISOString()}] INFO: Mémoire utilisée: ${mem.percentage}% (${typeof mem.usage === "number" ? Math.round(mem.usage / 1024 / 1024) : "N/A"}MB)`,
            );
          }
          ...
          if (serviceData.metrics?.cpu) {
            ...
            logs.push(
              `[${new Date().toISOString()}] INFO: CPU utilisé: ${cpu.percentage}%`,
            );
          }

          if (logs.length === 0) {
            logs.push(
              `[${new Date().toISOString()}] INFO: Service ${service.name} en cours de surveillance`,
            );
          }
          ...
          setServiceLogs([
            `[${new Date().toISOString()}] INFO: Service ${service.name} - Métriques non disponibles`,
          ]);
        }
      } else {
        setServiceLogs([
          `[${new Date().toISOString()}] INFO: Service ${service.name} - En attente de données`,
        ]);
      }
    } catch (error) {
  ...
  const activateMaintenance = async (service: ServiceStatus) => {
    try {
      ...
      console.warn(
        "Service de maintenance non disponible, fonction désactivée temporairement",
      );
  ...
  const getServiceMetrics = (service: ServiceStatus) => {
    ...
    if (metrics?.services) {
      ...
      if (serviceMetrics && serviceMetrics.metrics) {
        ...
        return {
          type: "service",
          data: {
            memory: serviceMetrics.metrics.memory || {
              usage: "N/A",
              limit: "N/A",
              percentage: "N/A",
            },
            cpu: serviceMetrics.metrics.cpu || {
              usage: "N/A",
              system: "N/A",
              percentage: "N/A",
            },
    ...
    if (metrics?.containers && !Array.isArray(metrics.containers)) {
      ...
      const containerName = Object.keys(metrics.containers).find(
        (containerKey) => {
          ...
          return (
            containerLower.includes(serviceLower) ||
        ...
        <div className="mb-4 md:mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4 md:mb-6">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-3 sm:p-4">
              ...
              <p className="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 dark:text-gray-100">
                {Number.isFinite(averageResponseTime)
                  ? `${Math.round(averageResponseTime as number)}ms`
                  : "N/A"}
              </p>
            </div>
          </div>
                  ...
                  <div className="p-4 sm:p-6">
                    ...
                    <div className="space-y-2">
                      ...
                      {(() => {
                        ...
                        if (
                          serviceMetrics &&
                          serviceMetrics.data.memory.percentage !== "N/A"
                        ) {
                          return (
              ...
              <div
                className={`p-6 ${
                  selectedService.status === "online"
                    ...
                    : selectedService.status === "testing"
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm opacity-90">
                        Service • Port {selectedService.port} •{" "}
                        {selectedService.status === "online"
                          ? "En ligne"
                          : selectedService.status === "testing"
                            ? "Test en cours"
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-medium text-gray-900 dark:text-gray-100 font-mono text-xs break-all">
                            {selectedService.url}
                          </span>
                        </div>
                        <div className="flex justify-between items-center">
                      ...
                      {(() => {
                        ...
                        if (serviceMetrics) {
                          ...
                          return (
                            <div className="space-y-3">
                              ...
                              {metrics.network &&
                                (metrics.network.rx_bytes !== "N/A" ||
                                  metrics.network.tx_bytes !== "N/A") && (
                        ...
                        return (
                          <div className="text-sm text-gray-500 dark:text-gray-400">
                            Métriques du service non disponibles pour{" "}
                            {selectedService.name}
                            <br />
                            <button
                              onClick={() =>
                                fetchServiceDetailedMetrics(selectedService)
                              }
