...
export default function SecurityTestsPage() {
  ...
  const pollSecurityJob = async (
    ...
    for (;;) {
      ...
      if (!response.ok) {
        throw new Error(
          data.error || `Suivi job impossible: ${response.status}`,
        );
      }
      lastResult = data;
  ...
  const runAppSecurityTests = async () => {
    ...
    setReportId(null);
    addLog(
      "🚀 Lancement tests sécurité applicatifs (XSS, SQLi, auth, headers…)…",
    );
  ...
  const runCveScan = async () => {
    ...
    setReportId(null);
    addLog(
      "🧬 Lancement scan CVE dépendances (npm, Rust, Flutter, Docker si activé)…",
    );
