...
export default function UserStatsContent() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${API_URL}/api/v1/auth/users?limit=200`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
  ...
  const fetchUserStats = (userId: string) => {
    ...
    const token =
      typeof window !== "undefined" ? localStorage.getItem("token") : null;
    axios
      .get(`${API_URL}/api/v1/analytics/stats/${userId}?days=${days}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-gray-600 dark:text-gray-400">
          Statistiques globales et par utilisateur (sessions, événements,
          données)
        </p>
      </div>
      ...
      <section className="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-sm overflow-hidden">
        ...
        {!loadingGlobal && globalStats && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 p-6">
            ...
            <StatTile
              label="Candidatures"
              value={globalStats.applications?.total ?? 0}
              color="purple"
            />
            <StatTile
              label="Entreprises"
              value={globalStats.companies?.total ?? 0}
              color="green"
            />
            <StatTile
              label="Contacts"
              value={globalStats.contacts?.total ?? 0}
              color="amber"
            />
            <StatTile
              label="Entretiens"
              value={globalStats.interviews?.total ?? 0}
              color="cyan"
            />
          </div>
        )}
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            {usersList.map((u) => (
              ...
              >
                ...
                {selectedUserId === u.id &&
                  (userStatsCache[u.id] || loadingUserStats === u.id) && (
...
function StatTile({
  ...
  return (
    <div
      className={`rounded-lg border border-gray-200 dark:border-gray-600 p-3 ${small ? "bg-white dark:bg-gray-800" : "bg-gray-50 dark:bg-gray-800/50"}`}
    >
      <p
        className={`text-gray-500 dark:text-gray-400 ${small ? "text-xs" : "text-sm"}`}
      >
        {label}
      </p>
      <p
        className={`font-bold ${colorClasses[color]} ${small ? "text-lg" : "text-2xl"} mt-0.5`}
      >
        {value}
      </p>
    </div>
