      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-200 dark:border-gray-700">
          ...
          {searchHistory.length > 0 && !searchQuery && (
            <div className="mt-4">
              <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Recherches récentes :</p>
        ...
        {hasResults && (
          <div className="space-y-4">
            ...
            {results.map((moduleResults) => {
              if (!moduleResults.results || moduleResults.results.length === 0) return null;
              
              const config = MODULE_CONFIG[moduleResults.module as keyof typeof MODULE_CONFIG];
              if (!config) return null;

              const Icon = config.icon;

              return (
                <div key={moduleResults.module} className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                  <div className={`bg-${config.color}-50 dark:bg-${config.color}-900/20 border-b border-${config.color}-200 dark:border-${config.color}-800 px-6 py-3`}>
                    <h3 className={`text-base font-medium text-${config.color}-900 dark:text-${config.color}-100 flex items-center gap-2`}>
                        ...
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                              {result.companyName || result.email || result.sector || result.type || 'Détails non disponibles'}
                            </p>
                          </div>
        ...
        {!loading && !hasResults && searchQuery.length >= 2 && results.length > 0 && (
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-900/20 dark:to-purple-900/20 rounded-lg p-6 border border-blue-200 dark:border-blue-800">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 text-sm">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="font-medium text-gray-900 dark:text-gray-100">Recherche multi-modules</p>
                <p className="text-gray-600 dark:text-gray-400">Recherchez dans tous vos modules simultanément</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Filter className="h-5 w-5 text-purple-600 flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-gray-900 dark:text-gray-100">Filtres intelligents</p>
                <p className="text-gray-600 dark:text-gray-400">Sélectionnez les modules à rechercher</p>
              </div>
            </div>
            <div className="flex items-start gap-2">
              <Clock className="h-5 w-5 text-green-600 flex-shrink-0 mt-0.5" />
              <div>
                <p className="font-medium text-gray-900 dark:text-gray-100">Historique</p>
                <p className="text-gray-600 dark:text-gray-400">Accédez à vos recherches récentes</p>
              </div>
            </div>
          </div>
        </div>
