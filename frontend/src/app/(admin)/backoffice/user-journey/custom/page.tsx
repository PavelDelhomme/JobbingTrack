...
const AVAILABLE_STEPS = [
  ...
  },
  {
    ...
    description: "Validation de l'email après inscription",
    icon: "📧",
  },
  {
    id: "login",
  ...
  {
    id: "application_with_company",
    name: "Candidature avec Entreprise",
    description: "Création candidature avec création entreprise",
    icon: "📝",
  },
  // Candidatures
  {
    ...
    description: "Créer des candidatures de test",
    icon: "📄",
  },
  {
  ...
  },
  {
    ...
    description: "Vérification/mise à jour du statut candidature",
    icon: "📊",
  },
  {
    id: "application_rejected",
    name: "Candidature Rejetée",
    description: "Marquer candidature comme rejetée après entretien",
    icon: "❌",
  },
  // Contacts
  {
    id: "create_contacts",
    name: "Créer Contacts",
    description: "Créer des contacts recruteurs",
    icon: "👥",
  },
  {
    id: "update_contacts",
  ...
  {
    id: "contact_to_application",
    name: "Contact à Candidature",
    description: "Ajout d'un contact à une candidature",
    icon: "📇",
  },
  // Suivi candidature
  {
    id: "followup",
    name: "Relance",
    description: "Ajout d'une relance à une candidature",
    icon: "📞",
  },
  {
    id: "interview",
    name: "Entretien",
    description: "Ajout d'un entretien à une candidature",
    icon: "📅",
  },
  {
    id: "call_company",
    name: "Appel Entreprise",
    description: "Enregistrement d'un appel avec l'entreprise",
    icon: "☎️",
  },
  {
    id: "call_contact",
    name: "Appel Contact",
    description: "Enregistrement d'un appel avec un contact",
    icon: "📱",
  },
  // Calendrier & événements
  {
    ...
    description: "Créer des événements au calendrier",
    icon: "📆",
  },
  {
    id: "view_calendar",
  ...
  // Dashboard & notifications
  {
    ...
    description: "Consulter le dashboard et les statistiques",
    icon: "📈",
  },
  {
  ...
  // Mobile vision (section 9 FONCTIONNALITES.md)
  {
    ...
    description: "Consulter le dashboard mobile : stats, entretiens, relances",
    icon: "📊",
  },
  {
    id: "search_hub",
    name: "Hub Recherche (6 tabs)",
    description:
      "Naviguer les 6 onglets : candidatures, contacts, entreprises, relances, appels, entretiens",
    icon: "🔍",
  },
  {
    ...
    description:
      "Consulter le détail candidature : timeline, entretiens, relances liés",
    icon: "📋",
  },
  {
    id: "archive_restore",
    name: "Archivage & Restauration",
    description: "Archiver → masquer → désarchiver → supprimer → restaurer",
    icon: "🗑️",
  },
  {
    id: "password_reset",
    name: "Reset Mot de Passe",
    description: "Demande reset → email MailHog → token → nouveau password",
    icon: "🔑",
  },
  {
...
export default function CustomJourneyPage() {
  ...
  const updateStepOptions = (stepId: string, options: Record<string, any>) => {
    setSteps(
      steps.map((s) =>
        s.id === stepId ? { ...s, options: { ...s.options, ...options } } : s,
      ),
    );
  };
      ...
      setResults([
        {
          ...
          name: "Configuration",
          status: "error",
          message: "Selectionnez un appareil ADB pour les etapes mobiles",
        },
      ]);
      return;
    }
    ...
    for (const step of steps) {
      ...
      try {
        if (step.isMobile && adb) {
          const msg = await executeMobileAction(
            step.stepId,
            step.options || {},
            adb,
          );
          ...
          const response = await fetch("/api/user-journey/custom", {
            ...
            body: JSON.stringify({
              name: journeyName,
              userMode,
              steps: [{ step: step.stepId, options: step.options || {} }],
            }),
          });
      ...
      try {
        ...
        const reportData = {
          ...
          summary: {
            ...
            warningCount: allResults.filter((r) => r.status === "warning")
              .length,
            skippedCount: allResults.filter((r) => r.status === "skipped")
              .length,
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {userMode === "user" && (
                      <Badge className="ml-auto bg-blue-500">Actif</Badge>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                    Crée un compte test (rôle USER). Simule l&apos;usage réel de
                    l&apos;app mobile.
                  </p>
                </button>
                <button
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {userMode === "admin" && (
                      <Badge className="ml-auto bg-orange-500">Actif</Badge>
                    )}
                  </div>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="flex items-end">
                  <div
                    className={`px-3 py-2 rounded-lg text-sm ${controllerOk && selectedDevice ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300" : "bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300"}`}
                  >
                    {controllerOk && selectedDevice
                      ? "Pret pour les tests mobiles"
                      : "Configurez le controleur + appareil"}
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              {showMobileSteps && (
                <CardContent className="space-y-2">
                  ...
                  <div className="space-y-1.5 max-h-[350px] overflow-y-auto">
                    {MOBILE_ACTIONS.filter(
                      (a) =>
                        mobileFilter === "all" || a.category === mobileFilter,
                    ).map((action) => (
                      <div
                        key={action.id}
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => addStep(action.id, true)}
                          disabled={isRunning}
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="text-base">
                Votre Parcours ({steps.length} etapes)
              </CardTitle>
            </CardHeader>
            <CardContent>
              {steps.length === 0 ? (
                <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                  ...
                  <p className="text-sm mt-2">
                    Ajoutez des etapes API ou des actions mobiles depuis la
                    colonne de gauche
                  </p>
                </div>
              ) : (
                <div className="space-y-2">
                  {steps.map((step, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-medium text-gray-500 dark:text-gray-500 w-6">
                            {index + 1}
                          </span>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="font-medium text-sm text-gray-900 dark:text-white flex items-center gap-1.5">
                              ...
                              {isM && (
                                <span className="text-[10px] px-1 py-0.5 rounded bg-indigo-200 dark:bg-indigo-800 text-indigo-700 dark:text-indigo-300">
                                  Mobile
                                </span>
                              )}
                            </div>
