...
export default function DeploymentsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "deployments" | "rollbacks" | "metrics"
  >("overview");
  ...
  const loadDeploymentData = async () => {
    ...
    try {
      const response = await axios.get(
        `${API_URL}/api/v1/deployments/metrics/analytics?days=30`,
        {
      ...
      console.error(
        "Erreur lors du chargement des données de déploiement:",
        error,
      );
    } finally {
      setLoading(false);
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-green-600 dark:text-green-400">
                      {metrics.overview.successfulDeployments}/
                      {metrics.overview.totalDeployments}
                    </p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">
                      {(100 - metrics.performance.avgErrorRate * 100).toFixed(
                        2,
                      )}
                      %
                    </p>
                  ...
                  >
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-t-lg relative">
                      <div
                        ...
                        style={{
                          height: `${(trend.deployment_count / Math.max(...metrics.trends.map((t) => t.deployment_count))) * 100}%`,
                        }}
                      ></div>
                    </div>
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${getStatusColor(deployment.status)}`}
                      >
                        {deployment.status}
                      </span>
                  ...
                  {deployment.rollbackReason && (
                    <div className="mt-3 p-3 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded">
                      <p className="text-sm text-orange-800 dark:text-orange-400">
                        <strong>Raison du rollback:</strong>{" "}
                        {deployment.rollbackReason}
                      </p>
                    </div>
                  )}
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              ...
              <p className="text-sm mt-2">
                Les rollbacks seront affichés ici une fois implémentés
              </p>
            </div>
          </div>
        )}
