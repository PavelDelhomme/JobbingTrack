...
export default function SecurityAlertsPage() {
  ...
  const loadSecurityAlerts = async () => {
    ...
    try {
      ...
      console.error(
        "Erreur lors du chargement des alertes de sécurité:",
        error,
      );
      ...
      const mockAlerts = Array.from({ length: 20 }, (_, i) => ({
        ...
        category: ["intrusion", "vulnerability", "ddos", "authentication"][
          Math.floor(Math.random() * 4)
        ],
        ...
        metadata: {
          ...
          affectedSystems: ["api-gateway", "auth-service", "database"][
            Math.floor(Math.random() * 3)
          ],
          attackVector: ["SQL Injection", "XSS", "Brute Force", "DDoS"][
            Math.floor(Math.random() * 4)
          ],
        },
      }));
  ...
  const applyFilters = () => {
    ...
    if (filters.acknowledged !== "") {
      ...
      filtered = filtered.filter(
        (alert) => alert.isAcknowledged === acknowledged,
      );
    }
  ...
  const acknowledgeAlert = async (alertId: string) => {
    try {
      await axios.patch(
        `${API_URL}/api/v1/alerts/${alertId}/acknowledge`,
        {
  ...
  const resolveAlert = async (alertId: string) => {
    try {
      await axios.patch(
        `${API_URL}/api/v1/alerts/${alertId}/resolve`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("acknowledged", e.target.value)
                }
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {filteredAlerts.length} alertes affichées sur {alerts.length}{" "}
              total
            </div>
            <div className="flex gap-4 text-sm">
              ...
              <span className="text-orange-600 dark:text-orange-400">
                {filteredAlerts.filter((a) => a.level === "high").length}{" "}
                élevées
              </span>
              <span className="text-yellow-600 dark:text-yellow-400">
                {filteredAlerts.filter((a) => a.level === "medium").length}{" "}
                moyennes
              </span>
              <span className="text-green-600 dark:text-green-400">
                {
                  filteredAlerts.filter(
                    (a) => !a.isAcknowledged && !a.resolvedAt,
                  ).length
                }{" "}
                non traitées
              </span>
          ...
          {filteredAlerts.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span
                      className={`px-3 py-1 text-sm rounded-full font-medium ${getLevelColor(alert.level)}`}
                    >
                      {alert.level.toUpperCase()}
                    </span>
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getCategoryColor(alert.category)}`}
                    >
                      {alert.category}
                    </span>
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(alert)}`}
                    >
                      {getStatusText(alert)}
                    </span>
                  </div>
                ...
                {alert.isAcknowledged && alert.acknowledgedBy && (
                  <div className="text-sm text-blue-600 dark:text-blue-400 mb-4">
                    ✅ Acquittée par {alert.acknowledgedBy} le{" "}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Aucune alerte de sécurité trouvée avec les critères actuels
              </p>
            </div>
          )}
        </div>
