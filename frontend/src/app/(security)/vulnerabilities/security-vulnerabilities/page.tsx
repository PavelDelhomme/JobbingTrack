...
export default function SecurityVulnerabilitiesPage() {
  ...
  const loadVulnerabilities = async () => {
    ...
    try {
      ...
      const mockVulnerabilities = Array.from({ length: 30 }, (_, i) => ({
        ...
        cveId:
          Math.random() > 0.5
            ? `CVE-2023-${10000 + Math.floor(Math.random() * 9000)}`
            : undefined,
        ...
        remediation:
          Math.random() > 0.5
            ? `Mettre à jour vers la version ${Math.floor(Math.random() * 10)}.${Math.floor(Math.random() * 10)}.${Math.floor(Math.random() * 10)}`
            : undefined,
        tags: [
          ["web", "server", "database", "authentication", "authorization"][
  ...
  const applyFilters = () => {
    ...
    if (filters.component) {
      filtered = filtered.filter(
        (vuln) => vuln.affectedComponent === filters.component,
      );
    }
  ...
  const updateVulnerabilityStatus = async (
    ...
    try {
      await axios.patch(
        `${API_URL}/api/v1/vulnerabilities/${vulnerabilityId}`,
        {
          status: newStatus,
          resolvedAt:
      ...
      console.error(
        "Erreur lors de la mise à jour de la vulnérabilité:",
        error,
      );
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Gestion et suivi des vulnérabilités de sécurité découvertes dans le
            système
          </p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {filteredVulnerabilities.length} vulnérabilités affichées sur{" "}
            ...
            <div className="flex gap-4 text-sm">
              ...
              <span className="text-orange-600 dark:text-orange-400">
                {
                  filteredVulnerabilities.filter((v) => v.severity === "high")
                    .length
                }{" "}
                élevées
              </span>
              <span className="text-yellow-600 dark:text-yellow-400">
                {
                  filteredVulnerabilities.filter((v) => v.severity === "medium")
                    .length
                }{" "}
                moyennes
              </span>
              <span className="text-red-600 dark:text-red-400">
                {
                  filteredVulnerabilities.filter((v) => v.status === "open")
                    .length
                }{" "}
                ouvertes
              </span>
          ...
          {filteredVulnerabilities.length > 0 ? (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span
                      className={`px-3 py-1 text-sm rounded-full font-medium ${getSeverityColor(vulnerability.severity)}`}
                    >
                      {vulnerability.severity.toUpperCase()}
                    </span>
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getStatusColor(vulnerability.status)}`}
                    >
                      {vulnerability.status.replace("_", " ")}
                    </span>
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getComponentColor(vulnerability.affectedComponent)}`}
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
                  ...
                  {vulnerability.cvssScore && (
                    <div className="text-sm">
                      ...
                      <div className="mt-1 font-mono">
                        {vulnerability.cvssScore.toFixed(1)}/10
                      </div>
                    </div>
                  )}
                  ...
                  <div className="text-sm">
                    ...
                    <div className="mt-1">
                      <span
                        className={`px-2 py-1 text-xs rounded-full ${getStatusColor(vulnerability.status)}`}
                      >
                        {vulnerability.status.replace("_", " ")}
                      </span>
                    </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Aucune vulnérabilité trouvée avec les critères actuels
              </p>
            </div>
          )}
        </div>
