...
export default function SecurityDDoSPage() {
  ...
  const loadDDoSAttacks = async () => {
    ...
    try {
      ...
      const mockAttacks = Array.from({ length: 25 }, (_, i) => ({
        ...
        mitigationTime: Math.random() > 0.5 ? Math.floor(Math.random() * 300) + 30 : undefined,
        metadata: {
  ...
  const applyFilters = () => {
    ...
    if (filters.attackType) {
      filtered = filtered.filter(attack => attack.attackType === filters.attackType)
    }
    ...
    if (filters.search) {
      ...
      filtered = filtered.filter(attack =>
        ...
        attack.countries.some(country => country.toLowerCase().includes(searchTerm)) ||
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Surveillance et analyse des attaques par déni de service distribué (DDoS)
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => handleFilterChange('mitigated', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {filteredAttacks.length} attaques affichées sur {attacks.length} total
            </div>
            <div className="flex gap-4 text-sm">
              <span className="text-red-600 dark:text-red-400">
                {filteredAttacks.filter(a => a.attackType === 'VOLUMETRIC').length} volumétriques
              </span>
              <span className="text-orange-600 dark:text-orange-400">
                {filteredAttacks.filter(a => a.attackType === 'PROTOCOL').length} protocole
              </span>
              <span className="text-purple-600 dark:text-purple-400">
                {filteredAttacks.filter(a => a.attackType === 'APPLICATION').length} application
              </span>
        ...
        <div className="space-y-4">
          {filteredAttacks.length > 0 ? filteredAttacks.map((attack) => (
            <div key={attack.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <span className={`px-3 py-1 text-sm rounded-full font-medium ${getAttackTypeColor(attack.attackType)}`}>
                    {attack.attackType}
                  </span>
                  {attack.isMitigated ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                <div className="text-sm">
                  ...
                  <div className="font-mono text-xs mt-1">{attack.targetEndpoint}</div>
                </div>

                <div className="text-sm">
                  <strong className="text-gray-900 dark:text-gray-100">Durée:</strong>
                  <div className="mt-1">{formatDuration(attack.duration)}</div>
                </div>

                <div className="text-sm">
                  <strong className="text-gray-900 dark:text-gray-100">Requêtes:</strong>
                  <div className="mt-1">
                ...
                <div className="text-sm">
                  ...
                  <div className="mt-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs">{attack.sourceIPs.length} IPs</span>
              ...
              {attack.metadata && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  <div>
                    <strong className="text-gray-900 dark:text-gray-100">Bande passante:</strong>
                    <div className="text-gray-600 dark:text-gray-400">{attack.metadata.bandwidth}</div>
                  </div>
                  <div>
                    <strong className="text-gray-900 dark:text-gray-100">Paquets/sec:</strong>
                    <div className="text-gray-600 dark:text-gray-400">{attack.metadata.packetsPerSecond.toLocaleString()}</div>
                  </div>
                  <div>
                    <strong className="text-gray-900 dark:text-gray-100">Vecteur:</strong>
                    <div className="text-gray-600 dark:text-gray-400">{attack.metadata.attackVector}</div>
                  </div>
                </div>
              )}
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
              <svg className="w-12 h-12 mx-auto mb-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 01-1.414-1.414L6.586 13H9a1 1 0 010 2H7a1 1 0 01-1-1V5a1 1 0 011-1h8a1 1 0 011 1v8a1 1 0 01-1 1H9a1 1 0 010-2h2.414l1.293 1.293a1 1 0 001.414-1.414L12.414 11H15a2 2 0 002-2V5a2 2 0 00-2-2H5z" />
              </svg>
              <p className="text-gray-500 dark:text-gray-400">Aucune attaque DDoS trouvée avec les critères actuels</p>
            </div>
          )}
        </div>
      </div>
