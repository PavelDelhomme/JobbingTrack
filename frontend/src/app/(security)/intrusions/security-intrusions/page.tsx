...
export default function SecurityIntrusionsPage() {
  ...
  const loadIntrusionAttempts = async () => {
    ...
    try {
      ...
      console.error(
        "Erreur lors du chargement des tentatives d'intrusion:",
        error,
      );
      ...
      const mockIntrusions = Array.from({ length: 50 }, (_, i) => ({
        ...
        country: ["US", "CN", "RU", "FR", "DE", "BR"][
          Math.floor(Math.random() * 6)
        ],
        city: ["New York", "Beijing", "Moscow", "Paris", "Berlin", "São Paulo"][
          Math.floor(Math.random() * 6)
        ],
        attackType: [
          "SQL_INJECTION",
        ...
        blockReason:
          Math.random() > 0.5 ? "Suspicious pattern detected" : undefined,
        metadata: {
  ...
  const applyFilters = () => {
    ...
    if (filters.attackType) {
      filtered = filtered.filter(
        (intrusion) => intrusion.attackType === filters.attackType,
      );
    }
    ...
    if (filters.blocked !== "") {
      ...
      filtered = filtered.filter(
        (intrusion) => intrusion.isBlocked === blocked,
      );
    }
    ...
    if (filters.riskScore) {
      ...
      filtered = filtered.filter(
        (intrusion) => intrusion.riskScore >= minScore,
      );
    }
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 dark:text-gray-400">
            Surveillance et analyse des tentatives d'intrusion détectées par le
            système de sécurité
          </p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  handleFilterChange("riskScore", e.target.value)
                }
                className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
              >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-4 mb-6">
          <div className="flex items-center justify-between">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {filteredIntrusions.length} tentatives affichées sur{" "}
              {intrusions.length} total
            </div>
            <div className="flex gap-4 text-sm">
              <span className="text-red-600 dark:text-red-400">
                {filteredIntrusions.filter((i) => i.riskScore >= 80).length}{" "}
                critiques
              </span>
              <span className="text-orange-600 dark:text-orange-400">
                {
                  filteredIntrusions.filter(
              ...
              <span className="text-yellow-600 dark:text-yellow-400">
                {
                  filteredIntrusions.filter(
                    (i) => i.riskScore >= 40 && i.riskScore < 60,
                  ).length
                }{" "}
                moyennes
              </span>
        ...
        <div className="space-y-4">
          {filteredIntrusions.length > 0 ? (
            filteredIntrusions.map((intrusion) => (
              <div
                key={intrusion.id}
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getAttackTypeColor(intrusion.attackType)}`}
                    >
                      {intrusion.attackType.replace("_", " ")}
                    </span>
                    <span
                      className={`px-2 py-1 text-xs rounded-full font-medium ${getRiskColor(intrusion.riskScore)}`}
                    >
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {intrusion.country && (
                        <span className="text-lg">
                          {getCountryFlag(intrusion.country)}
                        </span>
                      )}
                      {intrusion.country && (
                ...
                {intrusion.metadata && (
                  <div className="text-xs text-gray-500 dark:text-gray-400">
                    ...
                    {(intrusion.metadata.confidence * 100).toFixed(1)}% •
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg p-8 text-center">
              ...
              <p className="text-gray-500 dark:text-gray-400">
                Aucune tentative d'intrusion trouvée avec les critères actuels
              </p>
            </div>
          )}
        </div>
