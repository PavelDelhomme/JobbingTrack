...
# ============================================================================
# Script de test de l'interface d'administration Playwright
# ============================================================================
# Ce script teste l'interface d'administration pour créer et gérer
# des utilisateurs de test et exécuter des tests
# ============================================================================
...
echo -e "${BLUE}🧪 Test de l'interface d'administration Playwright${NC}"
echo "=================================================="
...
echo -e "${BLUE}🎭 Test de l'interface d'administration...${NC}"
...
if curl -s http://localhost:3000/health > /dev/null; then
    ...
    USER_EMAIL="testuser_admin_$(date +%s)@jobbingtrack.test"
    ...
    if echo "$CREATE_RESPONSE" | grep -q "success"; then
        ...
        echo -e "${YELLOW}⚠️ Impossible de créer l'utilisateur via API${NC}"
    fi
    ...
    if echo "$LIST_RESPONSE" | grep -q "success"; then
        ...
        echo -e "${YELLOW}⚠️ Impossible de lister les utilisateurs${NC}"
    fi
    ...
    if echo "$DELETE_RESPONSE" | grep -q "success"; then
        ...
        echo -e "${YELLOW}⚠️ Impossible de supprimer l'utilisateur${NC}"
    fi
else
    echo -e "${YELLOW}⚠️ API Gateway non accessible${NC}"
fi

echo -e "${GREEN}🎉 Tests de l'interface d'administration terminés !${NC}"
...
echo "   npm run test:e2e:impersonation   # Tests d'authentification impersonnée"
echo ""
echo -e "${YELLOW}💡 Fonctionnalités d'administration :${NC}"
echo "   - Création d'utilisateurs de test"
echo "   - Gestion des rôles utilisateur"
echo "   - Exécution de tests depuis l'interface"
echo "   - Visualisation des résultats en temps réel"
echo "   - Rapports de tests automatiques"
echo ""
echo -e "${GREEN}🎯 Interface d'administration fonctionnelle !${NC}"
