...
const menu = {
  ...
  options: [
    ...
    { id: '12', label: '📱 Tests sur différents appareils', command: 'devices' },
    { id: '13', label: '🎬 Mode UI interactif (Playwright UI)', command: 'ui' },
    { id: '14', label: '🐛 Mode Debug (step by step)', command: 'debug' },
    { id: '15', label: '📸 Captures d\'écran uniquement', command: 'screenshots' },
...
function displayMenu() {
  ...
  console.log(colorize('║  📱 TESTS MOBILE INTERACTIFS - JobbingTrack            ║', 'cyan'));
  console.log(colorize('╚════════════════════════════════════════════════════════╝', 'cyan'));
  console.log();
...
function handleChoice(choice) {
  ...
  switch (option.command) {
    ...
    case 'dashboard':
      ...
      break;
    case 'ux':
      runTests('mobile-complete-journey.spec.ts', null, null, '-g "12. 🎨|13. 📱"');
      break;
    case 'performance':
      runTests('mobile-complete-journey.spec.ts', null, null, '-g "Performance"');
      break;
    case 'devices':
    ...
    case 'debug':
      ...
      break;
    case 'screenshots':
      runTests('mobile-complete-journey.spec.ts', null, 'screenshots', '-g "15. 📸"');
      break;
    case 'report':
      console.log(colorize('📊 Ouverture du rapport HTML...', 'green'));
      execSync('cd frontend && npx playwright show-report playwright-report-mobile', { stdio: 'inherit' });
      askContinue();
      return;
    case 'exit':
...
function main() {
  ...
  if (!checkServices()) {
    ...
    console.log(colorize('💡 Ou le frontend avec: cd frontend && npm run dev', 'yellow'));
    console.log();
    rl.question(colorize('Appuyez sur Entrée pour continuer...', 'yellow'), () => {
      main();
    });
    return;
  }
