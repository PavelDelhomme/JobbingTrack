...
test.describe('👑 Fonctionnalités Administrateur Avancées', () => {
  ...
  test('devrait permettre la gestion complète des utilisateurs', async ({ page }) => {
    ...
    await page.fill('input[name="email"]', 'nouveau.recruteur@jobbingtrack.test');
  ...
  test('devrait permettre l\'audit et la conformité RGPD', async ({ page }) => {
    ...
    // Traiter la demande
    await page.locator('tr').filter({ hasText: 'redacted@example.invalid' }).locator('button').filter({ hasText: /Traiter/ }).click();
