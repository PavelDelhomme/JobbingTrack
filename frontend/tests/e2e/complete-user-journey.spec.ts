...
test.describe("Parcours utilisateur complet - JobbingTrack", () => {
  test("Parcours complet: Inscription → Connexion → Création candidature → Gestion complète", async ({
    ...
    const registerButton = page
      .locator("text=/S'inscrire|Inscription|Créer un compte/i")
      .first();
    ...
    const createCompanyButton = page
      .locator(
        'button:has-text("Créer"), button:has-text("Ajouter"), button:has-text("Nouvelle")',
      )
      .first();
    ...
    if (await descriptionField.isVisible().catch(() => false)) {
      await descriptionField.fill(
        "Entreprise spécialisée en développement web et mobile",
      );
    }
    ...
    await page.fill(
      'input[name="title"], input[name="position"]',
      "Développeur Full Stack Senior",
    );

    // Sélectionner l'entreprise
    const companySelect = page.locator(
      'select[name="companyId"], input[name="company"]',
    );
    if (await companySelect.isVisible().catch(() => false)) {
      const isSelect = await companySelect.evaluate(
    ...
    // Type de contrat
    const contractTypeField = page.locator(
      'select[name="contractType"], input[name="contractType"]',
    );
    if (await contractTypeField.isVisible().catch(() => false)) {
      const isSelect = await contractTypeField.evaluate(
        (el) => el.tagName === "SELECT",
      );
      if (isSelect) {
    ...
    const applicationRow = page
      .locator("text=Développeur Full Stack Senior")
      .locator("..");
    await applicationRow
      .locator(
        'button:has-text("Modifier"), button:has-text("Éditer"), [aria-label*="dit"]',
      )
      .first()
      .click();
    ...
    // Créer un entretien
    const createInterviewButton = page
      .locator(
        'button:has-text("Créer"), button:has-text("Ajouter"), button:has-text("Nouvel")',
      )
      .first();
    ...
    const scheduledAtField = page.locator(
      'input[name="scheduledAt"], input[type="datetime-local"]',
    );
    ...
    // Créer une relance
    const createFollowUpButton = page
      .locator(
        'button:has-text("Créer"), button:has-text("Ajouter"), button:has-text("Nouvelle")',
      )
      .first();
    ...
    if (await createFollowUpButton.isVisible().catch(() => false)) {
      ...
      const scheduledDateField = page.locator(
        'input[name="scheduledDate"], input[type="datetime-local"]',
      );
      ...
      console.log(
        "⚠️ Page de relances non disponible (feature en cours de développement)",
      );
    }
    ...
    const statsCards = page.locator(
      '[data-testid="metrics-cards"], .stats-card, .metric-card',
    );
    ...
    const exportButton = page.locator(
      'button:has-text("Exporter"), button:has-text("Export")',
    );
    ...
    const logoutButton = page
      .locator(
        'button:has-text("Déconnexion"), button:has-text("Logout"), [aria-label="Logout"]',
      )
      .first();
  ...
  test("Parcours: Création automatique d'entreprise lors d'une candidature", async ({
    page,
  }) => {
