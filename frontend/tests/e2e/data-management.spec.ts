...
test.describe('💾 Gestion des Données - Tests Complets', () => {
  ...
  test('devrait permettre l\'export avancé des données', async ({ page }) => {
    ...
    await expect(page.locator('button').filter({ hasText: 'JSON' })).toHaveClass(/bg-green-500/);
  ...
  test('devrait permettre la modification avancée des données', async ({ page }) => {
    ...
    await page.locator('text=Utilisateurs').click();
    await page.waitForSelector('text=user1@test.com');

    // Cliquer sur le bouton d'édition
    await page.locator('button').filter({ has: page.locator('[data-testid="edit-button"]') }).first().click();
    ...
    await expect(page.locator('input[value="user1@test.com"]')).toBeVisible();
    ...
    await expect(page.locator('text=Modifier l\'enregistrement')).not.toBeVisible();
  });
  ...
  test('devrait permettre les tests de base de données', async ({ page }) => {
    ...
    await page.locator('button').filter({ hasText: 'Lancer tous les tests' }).click();
  ...
  test('devrait permettre l\'import de données', async ({ page }) => {
    ...
    await expect(page.locator('text=Cliquez pour sélectionner un fichier')).toBeVisible();
  ...
  test('devrait permettre les opérations en masse', async ({ page }) => {
    ...
    await expect(page.locator('text=Synchronisation des données')).toBeVisible();
  });
