...
test.describe('👔 Suivi intérim (backoffice)', () => {
  ...
  test('la page Suivi intérim charge sans erreur', async ({ page }) => {
    ...
    const heading = page.getByRole('heading', { name: /Suivi intérim/i }).or(
      page.locator('h2').filter({ hasText: /Suivi intérim|Boîtes d'intérim/i })
    );
  ...
  test('la page affiche soit des agences soit le message invitant à en créer', async ({ page }) => {
    await page.goto('/b4ck0ff1ce/suivi-interim');
    await page.waitForLoadState('domcontentloaded');
    await expect(page.locator('nav').first()).toBeVisible({ timeout: 25000 });
    const heading = page.getByRole('heading', { name: /Suivi intérim/i }).or(
      page.locator('h2').filter({ hasText: /Suivi intérim/i })
    );
    await expect(heading.first()).toBeVisible({ timeout: 20000 });
    const bodyText = await page.locator('body').textContent() ?? '';
    const hasAgencies = /Randstad|Manpower|Boîte d'intérim|Candidatures liées/i.test(bodyText);
    const hasEmptyMessage = /Aucune boîte d'intérim|Créez une entreprise.*Boîte d'intérim/i.test(bodyText);
    expect(hasAgencies || hasEmptyMessage).toBe(true);
  });

  test('après génération de données de test, la page peut afficher des agences', async ({ page, request }) => {
    ...
    const genRes = await request.post(`${API_URL}/api/v1/admin/generate-test-data`, {
...
test.describe('🖥️ Pages Backoffice Suivi intérim', () => {
  ...
  test('la page Suivi intérim est accessible depuis le menu', async ({ page }) => {
    ...
    const suiviLink = page.locator('a[href="/b4ck0ff1ce/suivi-interim"]').first();
    if (!(await suiviLink.isVisible().catch(() => false))) {
      const dataExpander = page.locator('a[href="/b4ck0ff1ce/datas"] + button[aria-label="Expander les sous-items"]').first();
