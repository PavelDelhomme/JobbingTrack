...
test.describe("Inscription + vérification email (Email Monitor)", () => {
  ...
  test("inscription des 3 comptes puis vérification dans Email Monitor", async ({
    ...
    const listOrEmpty = page
      .locator("text=Emails Envoyés")
      .first()
      .or(page.locator("text=Aucun email trouvé").first());
    ...
    const hasListOrEmpty =
      bodyText.includes("Aucun email trouvé") ||
  ...
  test("Email Monitor affiche les filtres et le type Vérification", async ({
    page,
  }) => {
    ...
    expect(bodyText.toLowerCase()).toMatch(
      /email monitor|type|vérification|verification|aucun email/i,
    );
  });
});
