...
test.describe('📱 Application Mobile Flutter', () => {
  ...
  test('✅ Application mobile se charge correctement', async ({ page }) => {
    ...
    await expect(page.locator('text=JobbingTrack')).toBeVisible({ timeout: 10000 });

    // Vérifier la présence du logo
    await expect(page.locator('text=Suivez vos candidatures facilement')).toBeVisible();
  });
  ...
  test('🔐 Connexion utilisateur mobile', async ({ page }) => {
    ...
    await expect(page.locator('text=Gérez vos candidatures en un coup d\'œil')).toBeVisible();
  });
...
test.describe('🔗 Intégration API Mobile', () => {
  ...
  test('📋 Récupération des données utilisateur mobile', async ({ request }) => {
    // Se connecter d'abord
