...
test.describe('🎭 Tests d\'impersonnalisation - Interface Utilisateur (admin)', () => {
  ...
  test('🧪 Test de différents rôles utilisateur', async ({ page }) => {
    ...
    const testUsers = [
      { email: `user_basic_${Date.now()}@jobbingtrack.com`, role: 'USER', expectedAccess: 'Candidatures' },
      { email: `user_manager_${Date.now()}@jobbingtrack.com`, role: 'MANAGER', expectedAccess: 'Candidatures' },
      { email: `user_hr_${Date.now()}@jobbingtrack.com`, role: 'HR', expectedAccess: 'Candidatures' }
    ];
