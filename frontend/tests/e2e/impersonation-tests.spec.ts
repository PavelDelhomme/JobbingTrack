...
test.describe('🎭 Tests d\'impersonnalisation - Interface Utilisateur', () => {
  test('👤 Création d\'utilisateur de test via interface', async ({ page }) => {
    ...
    await expect(page.locator('text=Utilisateur créé')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('👑 Impersonnalisation administrateur', async ({ page }) => {
    ...
    await expect(page.locator('text=Backoffice')).toBeVisible({ timeout: 10000 });
  });
  ...
  test('🧪 Test de différents rôles utilisateur', async ({ page }) => {
    ...
    const testUsers = [
      { email: `user_basic_${Date.now()}@jobbingtrack.test`, role: 'USER', expectedAccess: 'Candidatures' },
      { email: `user_manager_${Date.now()}@jobbingtrack.test`, role: 'MANAGER', expectedAccess: 'Candidatures' },
      { email: `user_hr_${Date.now()}@jobbingtrack.test`, role: 'HR', expectedAccess: 'Candidatures' }
    ];
    ...
    for (const testUser of testUsers) {
      ...
      await expect(page.locator(`text=${testUser.expectedAccess}`)).toBeVisible();
  ...
  test('🗑️ Suppression d\'utilisateur de test', async ({ page }) => {
    ...
    await expect(page.locator('button:has-text("Créer Utilisateur Test")')).toBeVisible();
  });
});
...
test.describe('🎭 Tests d\'impersonnalisation avancée', () => {
  test('🔄 Changement rapide d\'utilisateur', async ({ page }) => {
    ...
    for (let i = 0; i < 3; i++) {
      await page.click('button:has-text("Créer Utilisateur Test")');
      const email = `quickuser_${Date.now()}_${i}@jobbingtrack.test`;
  ...
  test('📊 Test de données utilisateur spécifiques', async ({ page }) => {
    ...
    await page.click('button:has-text("Créer Utilisateur Test")');
    const specificEmail = `specific_${Date.now()}@jobbingtrack.test`;
