...
test.describe("Backoffice — Émulateur mobile", () => {
  ...
  test("affiche la section inscription + vérification email et tous les parcours", async ({
    page,
  }) => {
    await expect(
      page.locator("text=Inscription + vérification email").first(),
    ).toBeVisible({ timeout: 10000 });
    await expect(page.locator("text=Tous les parcours").first()).toBeVisible({
      timeout: 8000,
    });
    const anyScenario = page
      .locator("button")
      .filter({
        hasText:
          /Inscription \+ vérif\. email \(Gmail\)|Login rapide|Inscription compl/,
      })
      .first();
  ...
  test("affiche le bouton Lancer le parcours", async ({ page }) => {
    await expect(
      page.getByRole("button", { name: /Lancer le parcours/i }).first(),
    ).toBeVisible({ timeout: 10000 });
  });

  test("bouton Lancer le parcours désactivé ou message si aucun appareil", async ({
    page,
  }) => {
    const btn = page
      .getByRole("button", { name: /Lancer le parcours/i })
      .first();
    await expect(btn).toBeVisible({ timeout: 8000 });
    // Soit le bouton est disabled (pas d'appareil), soit un message demande de sélectionner un appareil
    const disabled = await btn.isDisabled();
    const hasDeviceMessage = await page
      .locator("text=Selectionnez un appareil")
      .isVisible();
    expect(disabled || hasDeviceMessage).toBeTruthy();
  });

  test("affiche la liste des parcours (tous les parcours ou filtre)", async ({
    page,
  }) => {
    await expect(page.locator("text=Tous les parcours").first()).toBeVisible({
      timeout: 8000,
    ...
    const anyScenario = page
      ...
      .filter({
        hasText:
          /Login|Inscription|Parcours|Reset|Navigation|CRUD|Verification/,
      })
      .first();
    await expect(anyScenario).toBeVisible({ timeout: 5000 });
  });

  test("parcours inscription + vérif. email (Gmail, Proton, BlueMail) et autres parcours visibles", async ({
    page,
  }) => {
    await expect(
      page.locator("text=Inscription + vérification email").first(),
    ...
    await page
      .locator("text=Tous les parcours")
      .first()
    ...
    expect(
      visibleCount,
      "Au moins 4 parcours (Gmail/Proton/BlueMail, Reset, Login, Parcours complet) doivent être visibles",
    ).toBeGreaterThanOrEqual(4);
  });

  test("affiche un statut contrôleur (connecté ou injoignable)", async ({
    page,
  }) => {
    await page.waitForTimeout(3000);
    const hasConnecte = await page
      .getByText(/Controleur emulateur connecte/)
      .first()
      .isVisible()
      .catch(() => false);
    const hasInjoignable = await page
      .getByText("injoignable")
      .first()
      .isVisible()
      .catch(() => false);
...
test.describe("Backoffice — Émulateur mobile (interface complète, boutons et sections)", () => {
  ...
  test("bouton Vérifier le contrôleur visible et cliquable", async ({
    page,
  }) => {
  ...
  test("section contrôleur : soit Démarrer le contrôleur soit Build/Installer/Lancer/Désinstaller", async ({
    page,
  }) => {
    await page.waitForTimeout(3500);
    const startController = page
      .getByRole("button", { name: /Démarrer le contrôleur/i })
      .first();
    ...
    expect(
      hasStart || (hasBuild && hasInstall && hasLaunchOnly && hasUninstall),
    ).toBeTruthy();
  });

  test("boutons Build APK, Installer et lancer, Lancer seulement, Désinstaller présents quand contrôleur connecté", async ({
    page,
  }) => {
    await page.waitForTimeout(3500);
    const installBtn = page.getByTestId("btn-install-run");
    ...
    const buildOrForce = page
      .getByTestId("btn-build-apk")
      .or(page.getByTestId("btn-force-rebuild"));
  ...
  test("section Nettoyer un compte test visible quand contrôleur connecté", async ({
    page,
  }) => {
    await page.waitForTimeout(3500);
    ...
    if (visible) {
      await expect(
        page.getByText(/Nettoyer un compte test/i).first(),
      ).toBeVisible();
    }
  });

  test("Demarrer AVD et Flutter run présents quand contrôleur connecté", async ({
    page,
  }) => {
    await page.waitForTimeout(3500);
    ...
    await expect(
      page.getByRole("button", { name: /Flutter run/i }).first(),
    ).toBeVisible();
  });
});
...
test.describe("Backoffice — Émulateur mobile (parcours avec données)", () => {
  ...
  test("sélection du parcours complet avec données", async ({ page }) => {
    await expect(
      page
        .locator('button:has-text("Parcours complet (avec données)")')
        .first(),
    ).toBeVisible({ timeout: 10000 });
    await page
      .locator('button:has-text("Parcours complet (avec données)")')
    ...
    await page.waitForTimeout(500);
    await expect(
      page.locator("text=Génère données de test").first(),
  ...
  test("sélection du parcours Inscription complète", async ({ page }) => {
    ...
    const inscriptionBtn = page
      .getByRole("button", {
        name: /Inscription compl|Inscription \(désactivée/,
      })
      .first();
    ...
    await page.waitForTimeout(500);
    const anchor = page
      .locator("text=Déconnexion")
      .or(page.locator("text=Deconnexion"))
      .or(page.locator("text=étapes"))
      .or(page.locator("text=etapes"))
      .or(page.locator('[data-testid="run-journey-btn"]'))
      .first();
  ...
  test("sélection du parcours Archives & Corbeille", async ({ page }) => {
    await page
      .locator('button:has-text("Archives & Corbeille")')
      .first()
      .click();
    await page.waitForTimeout(500);
    await expect(page.locator("text=Archives").first()).toBeVisible({
      timeout: 3000,
    });
  });
});
...
test.describe("Backoffice — Émulateur mobile (exécution via système de scénarios)", () => {
  ...
  test("lance un parcours inscription + vérif. email (Gmail) via le système de scénarios", async ({
    page,
  }) => {
    await page
      .locator("text=Inscription + vérification email")
      .first()
      .scrollIntoViewIfNeeded();
    await page.waitForTimeout(300);
    const gmailBtn = page
      .getByRole("button", { name: /Vérif\. email \(Gmail\)/ })
      .first();
    ...
    const successOrError = page
      .locator('[data-testid="step-success"], [data-testid="step-error"]')
      .first();
    await expect(successOrError).toBeVisible({ timeout: 60000 });
    const hasSuccess =
      (await page.locator('[data-testid="step-success"]').count()) > 0;
