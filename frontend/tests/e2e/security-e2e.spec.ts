...
test.describe("🛡️ Sécurité – Protection XSS", () => {
  test("injection XSS dans le champ recherche entreprises est neutralisée", async ({
    page,
  }) => {
    ...
    if (await searchInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      expect(hasRawXss, "Le HTML ne doit pas contenir le script XSS brut").toBe(
        false,
      );
    }
  });

  test("injection XSS dans la recherche globale est neutralisée", async ({
    page,
  }) => {
    ...
    if (await searchInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      expect(
        hasActiveOnErrorTag,
        "Le HTML ne doit pas contenir une balise active avec onerror",
      ).toBe(false);
    }
  });
  ...
  test("injection XSS via API est rejetée ou nettoyée", async ({ page }) => {
    ...
    if (res.ok) {
      ...
      expect(
        returnedName,
        "API doit rejeter ou renvoyer un nom nettoyé (sans <script>)",
      ).not.toContain("<script>");

      const id =
        data?.id || data?._id || data?.company?.id || data?.company?._id;
      if (id) {
...
test.describe("🛡️ Sécurité – Protection SQL Injection", () => {
  ...
  test("injection SQL dans la recherche ne fonctionne pas", async ({
    page,
  }) => {
  ...
  test("injection NoSQL dans la recherche ne fonctionne pas", async ({
    page,
  }) => {
    const res = await apiFetch(page, "POST", "/api/v1/auth/login", {
      email: { $gt: "" },
