...
test.describe('🔐 Tests de Sécurité Avancés', () => {
  ...
  test('devrait protéger contre les attaques par injection JWT', async ({ page }) => {
    ...
    await page.route('**/api/v1/auth/login', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          // Token JWT malformé
          token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjEiLCJlbWFpbCI6ImFkbWluQGpvYmJpbmd0cmFjay5jb20iLCJmaXJzdE5hbWUiOiJBZG1pbiIsImxhc3ROYW1lIjoiSm9iYmluZ1RyYWNrIiwicm9sZSI6IlNVUEVSX0FETUlOIn0.invalid_signature'
        })
      });
    });
  ...
  test('devrait protéger contre les attaques CSRF sophistiquées', async ({ page }) => {
    ...
    await page.route('**/api/v1/companies', async route => {
      ...
      expect(headers['origin'] || headers['referer']).toContain('localhost:3000');
    ...
    await expect(page.locator('text=Entreprise Malveillante')).not.toBeVisible();
  });
  ...
  test('devrait sécuriser les téléchargements et exports de données', async ({ page }) => {
    ...
    await page.route('**/api/v1/reports/export', async route => {
      ...
      expect(['application/pdf', 'text/csv', 'application/vnd.ms-excel']).toContain(contentType);
    ...
    // Se connecter avec un utilisateur non autorisé
    await page.route('**/api/v1/auth/login', async route => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
    ...
    await expect(page.locator('button').filter({ hasText: /Exporter/ })).not.toBeVisible();
  });
  ...
  test('devrait protéger contre les attaques par timing', async ({ page }) => {
    ...
    const variance = timingResults.reduce((acc, time) => acc + Math.pow(time - avgTime, 2), 0) / timingResults.length;
    ...
    console.log(`⏱️ Timing attack test: avg=${avgTime}ms, variance=${variance}`);
  });
  ...
  test('devrait sécuriser les communications WebSocket et temps réel', async ({ page }) => {
    ...
    const wsConnection = await page.evaluate(async () => {
      try {
        const ws = new WebSocket('ws://localhost:3000/api/v1/notifications/websocket');
  ...
  test('devrait maintenir la sécurité lors des mises à jour système', async ({ page }) => {
    ...
    await page.locator('button').filter({ hasText: /Mettre à jour/ }).click();
