...
test.describe("Backoffice – Utilisateurs (filtre test, nettoyage, abonnement)", () => {
  ...
  test('affiche le filtre "Utilisateurs de test" et le bouton Nettoyer', async ({
    page,
  }) => {
    await expect(
    ...
    const filterSelect = page
      .locator("select")
      .filter({ has: page.locator('option[value="test"]') });
    await expect(filterSelect).toBeVisible({ timeout: 5000 });
    const cleanButton = page.getByRole("button", {
      name: /Nettoyer les utilisateurs de test/i,
    });
    await expect(cleanButton).toBeVisible({ timeout: 5000 });
  });

  test("détail utilisateur affiche la section Abonnement & facturation", async ({
    page,
  }) => {
    ...
    if (await firstRow.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const billingLink = page.getByRole("link", {
        name: /Voir \/ gérer l'abonnement/i,
      });
