...
test.describe("🔄 Workflow Complet - Gestion des Candidatures (admin)", () => {
  ...
  test("devrait permettre un workflow complet de recrutement", async ({
    page,
  }) => {
    ...
    await page.fill(
      ...
      "Startup innovante en développement web",
    );
    ...
    await page.fill('input[name="email"]', "jean.dupont@techstart.com");
    ...
    await page.fill(
      ...
      "React, Node.js, 5+ ans expérience",
    );
    await page
      .locator('select[name="companyId"]')
      .selectOption({ label: "TechStart Solutions" });
    ...
    await page.fill(
      'input[name="title"]',
      "Entretien technique - Dev Full Stack",
    );
    ...
    await page
      .locator('select[name="applicationId"]')
      .selectOption({ label: "Développeur Full Stack Senior" });
    await page
      .locator('select[name="interviewerId"]')
      .selectOption({ label: "Jean Dupont" });
    await page.locator('button[type="submit"]').click();
    ...
    await page.fill(
      'textarea[name="notes"]',
      "Candidat très compétent, bonne expérience React",
    );
    await page.fill(
      'input[name="nextSteps"]',
      "Proposition d'embauche à préparer",
    );
    await page
      .locator('select[name="status"]')
      .selectOption("INTERVIEW_COMPLETED");
    await page.locator('button[type="submit"]').click();
    ...
    await page
      .locator("button")
      .filter({ hasText: /Modifier|Éditer/ })
      .first()
      .click();
  ...
  test("devrait permettre la gestion complète du pipeline de recrutement", async ({
    page,
  }) => {
    await page.locator("text=Applications").click();
  ...
  test("devrait gérer les refus et archivages correctement", async ({
    page,
  }) => {
    await page.locator("text=Applications").click();
    ...
    await page
      .locator("button")
      .filter({ hasText: /Actions|Bulk/ })
      .click();
    await page
      .locator("button")
      .filter({ hasText: /Refuser/ })
      .click();
  ...
  test("devrait permettre la gestion des relances et communications", async ({
    page,
  }) => {
    await page.locator("text=Suivis").click();
    ...
    await page
      .locator("button")
      .filter({ hasText: /Relance/ })
      .click();
    await page.fill(
      'textarea[name="message"]',
      "Bonjour, nous souhaitons planifier un entretien...",
    );
  ...
  test("devrait fournir des métriques et rapports détaillés", async ({
    page,
  }) => {
