...
test.describe("🔒 Tests de Sécurité Avancés", () => {
  test("devrait empêcher l'accès non autorisé aux routes admin", async ({
    page,
    request,
  }) => {
    ...
    const url = page.url();
    expect(url.includes("/b4ck0ff1ce")).toBeFalsy();
  });

  test("devrait gérer correctement les sessions et timeouts", async ({
    page,
  }) => {
    await page.goto("/b4ck0ff1ce");
  ...
  test("devrait gérer correctement les permissions granulaires", async ({
    page,
  }) => {
    await page.goto("/b4ck0ff1ce");
    ...
    await page.goto("/b4ck0ff1ce/applications");
    await expect(
      page.locator("button").filter({ hasText: /Supprimer/ }),
    ).not.toBeVisible();
  });
  ...
  test("devrait protéger contre les attaques de taux (rate limiting)", async ({
    page,
  }) => {
    await page.goto("/login");
  ...
  test("devrait sécuriser les mots de passe et données sensibles", async ({
    page,
  }) => {
    await page.goto("/login");
  ...
  test("devrait gérer les erreurs de sécurité de manière appropriée", async ({
    page,
  }) => {
    // Essayer d'accéder à une URL malformée
    await page.goto("/b4ck0ff1ce/%3Cscript%3Ealert(1)%3C/script%3E");
  ...
  test("devrait maintenir la sécurité lors des changements de rôle", async ({
    page,
  }) => {
    await page.goto("/b4ck0ff1ce");
    ...
    await page
      .locator("tr")
      .filter({ hasText: "Admin JobbingTrack" })
      .locator("button")
      .filter({ hasText: /Modifier/ })
      .click();
