...
test.describe("📱 Tests Mobile - Parcours Utilisateur Complet", () => {
  ...
  test("2. 🔐 Connexion - Authentification mobile", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/Dashboard|Tableau de bord/i"),
    ).toBeVisible();
  });
  ...
  test("3. 🏢 Création d'Entreprise - Mobile", async ({ page }) => {
    ...
    const addButton = page
      .locator(
        'button:has-text("Nouveau"), button:has-text("Ajouter"), button:has-text("+")',
      )
      .first();
    ...
    await page.fill(
      'input[name="website"], input[placeholder*="site"]',
    ...
    // Soumettre
    await page.click(
      'button[type="submit"], button:has-text("Créer"), button:has-text("Enregistrer")',
    );

    // Vérifier la création
    await expect(
      page.locator("text=/Test Company Mobile|succès|success/i"),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("4. 📋 Création de Candidature - Mobile", async ({ page }) => {
    ...
    // Remplir le formulaire
    await page.fill(
      'input[name="position"], input[placeholder*="poste"]',
      "Développeur Mobile",
    );
    await page.fill(
      'input[name="company"], select[name="company"]',
      "Test Company",
    );
    await page.fill(
      'input[name="platform"], select[name="platform"]',
      "LinkedIn",
    );
    ...
    // Vérifier la création
    await expect(page.locator("text=/Développeur Mobile|succès/i")).toBeVisible(
      { timeout: 10000 },
    );
  });
  ...
  test("5. 👥 Création de Contact - Mobile", async ({ page }) => {
    ...
    // Vérifier la création
    await expect(page.locator("text=/Jean Dupont|succès/i")).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("6. 📞 Création d'Appel - Mobile", async ({ page }) => {
    ...
    // Vérifier la création
    await expect(page.locator("text=/succès|success|Appel créé/i")).toBeVisible(
      { timeout: 10000 },
    );
  });
  ...
  test("7. 📅 Création d'Entretien - Mobile", async ({ page }) => {
    ...
    // Remplir le formulaire
    await page.fill(
      'input[name="application"], select[name="application"]',
      "Développeur Mobile",
    );
    await page.fill(
    ...
    // Vérifier la création
    await expect(
      page.locator("text=/succès|success|Entretien créé/i"),
    ).toBeVisible({ timeout: 10000 });
  });
  ...
  test("8. 🔔 Création de Relance - Mobile", async ({ page }) => {
    ...
    // Vérifier la création
    await expect(
      page.locator("text=/succès|success|Relance créée/i"),
    ).toBeVisible({ timeout: 10000 });
  });

  test("9. 🔔 Notifications - Affichage et interaction mobile", async ({
    page,
  }) => {
    await loginUser(
      page,
    ...
    const notificationIcon = page
      .locator(
        'button[aria-label*="notification"], button:has-text("🔔"), [data-testid="notifications"]',
      )
      .first();
    ...
    if (await notificationIcon.isVisible()) {
      ...
      await expect(
        page.locator("text=/Notification|Aucune notification/i"),
      ).toBeVisible();
    }
  });
  ...
  test("10. 🔍 Recherche - Fonctionnalité mobile", async ({ page }) => {
    ...
    const searchInput = page
      .locator(
        'input[type="search"], input[placeholder*="recherche"], input[placeholder*="search"]',
      )
      .first();
    ...
    if (await searchInput.isVisible()) {
      ...
      await expect(page.locator("text=/résultat|result|test/i")).toBeVisible({
        timeout: 5000,
      });
    }
  });
  ...
  test("12. 🎨 Navigation Mobile - Menu hamburger", async ({ page }) => {
    ...
    const menuButton = page
      .locator(
        'button[aria-label*="menu"], button:has-text("☰"), [data-testid="menu-toggle"]',
      )
      .first();
  ...
  test("14. 🔄 Synchronisation Offline - Mobile", async ({ page, context }) => {
    ...
    const offlineIndicator = page.locator(
      "text=/hors ligne|offline|synchronisation/i",
    );
  ...
  test("15. 📸 Capture d'écran - Vérification visuelle mobile", async ({
    page,
  }) => {
    await loginUser(
      page,
...
test.describe("📱 Tests Mobile - Performance et UX", () => {
  ...
  test("Responsive - Adaptation différentes tailles d'écran", async ({
    page,
  }) => {
