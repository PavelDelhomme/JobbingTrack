...
test.describe("🔗 API Critiques - Tests d'intégration", () => {
  ...
  test("devrait tester la connectivité avec tous les services backend", async ({
    page,
  }) => {
  ...
  test("devrait gérer les erreurs 404 pour les routes inexistantes", async ({
    page,
  }) => {
  ...
  test("devrait tester la validation des données d'entrée", async ({
    page,
  }) => {
  ...
  test("devrait tester la gestion des headers de sécurité", async ({
    page,
  }) => {
  ...
  test("devrait tester la pagination des données", async ({ page }) => {
    ...
    const response = await page.request.get(
      "/api/v1/applications?page=1&limit=10",
    );
    expect(response.ok()).toBeTruthy();
  ...
  test("devrait tester les filtres et tri des données", async ({ page }) => {
    ...
    const response = await page.request.get(
      "/api/v1/applications?status=active&sort=createdAt&order=desc",
    );
    expect(response.ok()).toBeTruthy();
  ...
  test("devrait tester l'export de données", async ({ page }) => {
    ...
    const response = await page.request.get(
      "/api/v1/applications/export?format=csv",
    );
    expect(response.ok()).toBeTruthy();
