...
test.describe("📤📥 Export/Import Avancé - Tests Complets (admin)", () => {
  ...
  test("devrait permettre l'export CSV complet avec données complexes", async ({
    page,
  }) => {
  ...
  test("devrait permettre l'export JSON avec sélection multiple", async ({
    page,
  }) => {
    await page.goto("/backoffice/data-management");
  ...
  test("devrait tester les performances de l'export avec de gros volumes", async ({
    page,
  }) => {
    await page.goto("/backoffice/data-management");
  ...
  test("devrait permettre l'export dans les deux pages (Analytics et Data Management)", async ({
    page,
  }) => {
    // Test Analytics
    await page.goto("/backoffice/analytics");
    ...
    const analyticsExportButton = page
      .locator("button")
      .filter({ hasText: "Exporter" })
      .first();
    const dataExportButton = page
      .locator("button")
      .filter({ hasText: "Exporter" })
      .last();
  ...
  test("devrait maintenir la cohérence du design entre les pages", async ({
    page,
  }) => {
