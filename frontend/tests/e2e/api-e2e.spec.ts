...
test.describe("🔑 API – Authentification", () => {
  ...
  test("login avec identifiants invalides retourne 401", async ({
    request,
  }) => {
    const res = await apiFetch(request, "POST", "/api/v1/auth/login", {
...
test.describe("🏢 API – CRUD Entreprises", () => {
  ...
  test("GET /api/v1/companies retourne une liste", async ({ request }) => {
    ...
    if (res.status === 200)
      expect(
        Array.isArray(res.data) || (res.data && typeof res.data === "object"),
      ).toBe(true);
  });
  ...
  test("POST /api/v1/companies crée une entreprise", async ({ request }) => {
    ...
    createdCompanyId = (data?.id ||
      ...
      (data as any)?.company?._id) as string;
  });

  test("DELETE /api/v1/companies/:id supprime l'entreprise", async ({
    request,
  }) => {
    if (!createdCompanyId) {
      const name = `E2EAPI_TempDel_${Date.now()}`;
      const createRes = await apiFetch(request, "POST", "/api/v1/companies", {
        name,
      });
    ...
    if (createdCompanyId) {
      const res = await apiFetch(
        request,
        "DELETE",
        `/api/v1/companies/${createdCompanyId}`,
      );
...
test.describe("📝 API – Candidatures", () => {
  test("GET /api/v1/applications retourne des candidatures", async ({
    request,
  }) => {
...
test.describe("🔍 API – Recherche", () => {
  test("GET /api/v1/search?q=test retourne des résultats", async ({
    request,
  }) => {
...
test.describe("📊 API – Dashboard & Analytics", () => {
  test("GET /api/v1/dashboard retourne les données du tableau de bord", async ({
    request,
  }) => {
  ...
  test("GET /api/v1/dashboard/statistics retourne les stats", async ({
    request,
  }) => {
...
test.describe("⚙️ API – Admin", () => {
  test("GET /api/v1/admin/services retourne la liste des services", async ({
    request,
  }) => {
  ...
  test("GET /api/v1/admin/monitoring/system retourne les infos système", async ({
    request,
  }) => {
    const res = await apiFetch(
      request,
...
test.describe("📬 API – Emails", () => {
  ...
  test("GET /api/v1/emails/templates retourne les templates", async ({
    request,
  }) => {
