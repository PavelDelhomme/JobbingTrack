...
| Menaces historiques/lab triées avant purge | Inventaire dry-run `network_threats` distinguant lab/test, IP privées Docker, vraies menaces externes, statut bloqué/non bloqué | local/preprod | [ ] | Constat porteur 21/05 : page Menaces encore chargée en historiques `10.0.0.x`, `198.51.100.42`, `172.19.x`/`172.20.x`. Dry-run `scripts/security/security-threats-inventory-dry-run.cjs --limit=20` : `lab_test=228`, `private_network=11`. Ne pas supprimer sans rapport et validation. |
