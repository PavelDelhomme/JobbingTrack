...
| Performance disque alignée et Block I/O exploitable | `/b4ck0ff1ce/performances/disk` : design cohérent avec Performances, période sélectionnable, courbes usage/volume, Block I/O lecture/écriture et débit Mo/min visibles si données présentes | local | [ ] | Reprise technique 20/05 ; à valider navigateur avec `container_metrics_snapshots.blockReadBytes/blockWriteBytes` alimentés. |
...
| Compression emails/logs/métriques sans perte | Plan validé : payloads anciens compressés/archivés, metadata et index consultables, restauration possible, aucune purge non validée | local/preprod | [ ] | Chantier à implémenter après diagnostic taille par table et validation porteur. |
