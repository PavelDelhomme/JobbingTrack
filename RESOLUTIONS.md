...
- **Reply-To** : Configuré `SMTP_REPLY_TO=noreply@jobbingtrack.com` (affichage « répondre à » en jobbingtrack.com ; pas de boîte active sur ce domaine = pas de réponse possible). Ajout des headers `Auto-Submitted: auto-generated` et `X-Auto-Response-Suppress: All` pour indiquer que c’est un message automatique.
- **Config SMTP** : Compte OVH noreply@maily.ovh (MX Plan maily.ovh actif) ; affichage expéditeur `JobbingTrack <noreply@jobbingtrack.com>`. DNS jobbingtrack.com : MX, SPF OK ; DKIM non configuré (optionnel).
