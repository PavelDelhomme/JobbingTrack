...
const char* get_procfs_path(void) {
    ...
    if (env && env[0] != '\0') {
        snprintf(proc_path_buf, sizeof(proc_path_buf), "%s", env);
        return proc_path_buf;
    }
    ...
    *free = 0;
    ...
    while (fgets(line, sizeof(line), fp)) {
        if (strncmp(line, "MemTotal:", 9) == 0) {
            if (sscanf(line, "MemTotal: %lu", total) != 1) *total = 0;
        } else if (strncmp(line, "MemAvailable:", 13) == 0) {
            if (sscanf(line, "MemAvailable: %lu", free) != 1) *free = 0;
            break;
        }
    }
    ...
    if (fgets(line, sizeof(line), fp)) {
        if (sscanf(line, "cpu %llu %llu %llu %llu %llu %llu %llu %llu",
            ...
            if (last_total > 0 && last_idle > 0) {
                unsigned long long total_diff = total >= last_total ? total - last_total : 0;
                unsigned long long idle_diff = total_idle >= last_idle ? total_idle - last_idle : 0;
