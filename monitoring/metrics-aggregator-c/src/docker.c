...
int docker_list_containers(DockerMetrics *containers, int max_count) {
    ...
    while (fgets(line, sizeof(line), fp) && count < max_count) {
        ...
        if (sscanf(line, "%127s\t%255s\t%63[^\n]", id, name, status) == 3) {
            copy_string(containers[count].container_id, sizeof(containers[count].container_id), id);
            copy_string(containers[count].container_name, sizeof(containers[count].container_name), name);
            copy_string(containers[count].status, sizeof(containers[count].status), status);
