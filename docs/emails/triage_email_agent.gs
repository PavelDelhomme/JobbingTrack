 ...
 * JobbingTrack — configuration locale via proprietes Apps Script
 ...
 *    Exécuter en tant que : Moi (compte Google qui lit la boite de recherche)
...
// ============================================================
var CONFIG = {
  // Ne pas versionner l'adresse reelle : definir TRIAGE_AGENT_EMAIL dans les proprietes du script.
  email: "",
  ...
  // Ne pas versionner de PIN/token : definir JBT_AGENT_PIN dans les proprietes du script.
  jobbingtrackPin: "",
...
function proprieteScript(nom, fallback) {
  try {
    var value = PropertiesService.getScriptProperties().getProperty(nom);
...
function emailAgent() {
  var configured = proprieteScript("TRIAGE_AGENT_EMAIL", CONFIG.email);
...
function destinataireRecap() {
  var recipient = proprieteScript("TRIAGE_DIGEST_RECIPIENT", "");
  if (recipient) return recipient;
  var fallback = emailAgent();
  if (fallback) return fallback;
  throw new Error("Configurer TRIAGE_DIGEST_RECIPIENT dans les proprietes Apps Script.");
}

function pinJobbingTrack() {
  return proprieteScript("JBT_AGENT_PIN", CONFIG.jobbingtrackPin);
}
...
function lireEmailsEmploi() {
  ...
  var agent = emailAgent();
  var query = "after:" + dateStr + " -in:spam -in:trash" + (agent ? " -from:" + agent : "");
...
function jbtAuthentifier() {
  try {
    var pin = pinJobbingTrack();
    if (!pin) return null;
    var r = UrlFetchApp.fetch(CONFIG.jobbingtrackApiBase + "/auth/login", {
      method: "post", contentType: "application/json",
      payload: JSON.stringify({ pin: pin }), muteHttpExceptions: true
    });
...
function envoyerRecapitulatif(analyses, tachesCreees, tachesEnRetard, actionsJBT, calendrier, taskListId, offresAuto) {
  ...
  GmailApp.sendEmail(destinataireRecap(), "Triage Emploi — " + dateEnFrancais(aujourd_hui), "", { htmlBody: html });
...
================================================================
1. Aller sur https://script.google.com avec le compte Google configure pour la recherche d'emploi
2. Ouvrir le projet → remplacer TOUT Code.gs par ce fichier
3. Services (+) > Tasks API v1 > identifiant : TasksAPI (si deja fait, skip)
4. Proprietes du script a creer :
   - TRIAGE_AGENT_EMAIL : compte Gmail lu par l'agent
   - TRIAGE_DIGEST_RECIPIENT : destinataire du recap
   - JBT_AGENT_PIN : PIN/token JBT si l'ecriture auto est activee
5. DEPLOYER LE WEB APP (pour les boutons d'action dans le mail) :
   - Cliquer "Déployer" > "Nouveau déploiement"
   - Type : "Application Web"
   - Exécuter en tant que : "Moi" (compte Google configure)
...
6. Executer installerDeclencheur
7. Tester : executer lancerTriageQuotidien
