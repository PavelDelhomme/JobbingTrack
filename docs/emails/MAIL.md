...
Pour **OVH avec jobbingtrack.com** (compte créé dans le MX Plan, ex. `noreply@jobbingtrack.test`) :
...
- **Créer le compte** : dans l’espace OVH, MX Plan jobbingtrack.com → créer l’adresse (ex. `noreply@jobbingtrack.test`) et son mot de passe.
...
1. Créer le compte email OVH (ex. `noreply@jobbingtrack.test`) dans le MX Plan jobbingtrack.com.
