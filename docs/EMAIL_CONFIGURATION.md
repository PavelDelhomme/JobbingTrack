...
Vous avez un compte email `redacted@example.invalid` et vous voulez que les emails apparaissent comme venant de `noreply@jobbingtrack.test`.
...
# Utilisez votre compte redacted@example.invalid pour envoyer
# mais les emails apparaîtront comme venant de noreply@jobbingtrack.test
...
**Important** : 
- `SMTP_USER` = votre compte email réel sur maily.ovh (`redacted@example.invalid`)
- `SMTP_FROM` = l'adresse qui apparaîtra dans l'email (`noreply@jobbingtrack.test`)
- Le serveur SMTP OVH acceptera l'envoi car vous vous authentifiez avec `redacted@example.invalid`
- Les destinataires verront `noreply@jobbingtrack.test` comme expéditeur
...
- Soit utiliser `redacted@example.invalid` comme `SMTP_FROM` (temporaire)
- Soit configurer le domaine `jobbingtrack.com` plus tard

**Si vous avez le domaine `jobbingtrack.com`**, pour que les emails envoyés depuis `noreply@jobbingtrack.test` via le serveur SMTP de `maily.ovh` soient acceptés, configurez le SPF :
