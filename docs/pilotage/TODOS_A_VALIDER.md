...
| Mobile | **Mobile — releases OTA** | local | (1) Démarrer contrôleur : `bash scripts/mobile/setup/restart-emulator-controller.sh`. (2) **Mobile → releases OTA** → **Lancer le build APK** → **Télécharger** ou **Publier canal dev**. (3) Optionnel : **Installer sur appareil ADB**. (4) Promote prod si OK. | **[ ] EN COURS** | **Preuve agent 17/06** : build UI intégré page releases ; download + publish one-click ; Portainer = upload manuel seulement. **Reste porteur** : `OK Mobile releases OTA backoffice`. | |
