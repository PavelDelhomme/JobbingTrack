...
| Mobile | **Mobile — releases OTA** | local | (1) **`docker compose up -d emulator-controller`**. (2) **`https://jobbingtrack.localhost:5443/backoffice/mobile/releases`** — étapes 1–2 OK porteur 06/07 (install appareil). (3) **Étape 3** : bouton « Publier sur canal dev » ou formulaire ci-dessous. (4) Samsung : MAJ OTA. (5) **Étape 5** promote prod si OK. | **[ ] EN COURS** | **Preuve agent 07/07** : fix smoke OTA `loginAdminToken` destructuring → GET releases **HTTP 200** build **12** ; auteur **admin@jobbingtrack.com**. Upload smoke stub rejeté **500** attendu (APK trop petit — utiliser APK Flutter réel pour publish). **Porteur 06/07** : install OK. **Reste porteur** : valider backoffice + drawer + OTA Samsung → `OK Mobile releases OTA backoffice`. | |
