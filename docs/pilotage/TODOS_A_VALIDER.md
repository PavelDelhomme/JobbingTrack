...
| Mobile | **Mobile — releases OTA** | local | (1) **`docker compose up -d emulator-controller`**. (2) **`https://jobbingtrack.localhost:5443/backoffice/mobile/releases`** — étapes 1–2 OK porteur 06/07 (install appareil). (3) **Étape 3** : bouton « Publier sur canal dev » ou formulaire ci-dessous. (4) Samsung : MAJ OTA. (5) **Étape 5** promote prod si OK. | **[ ] EN COURS** | **Preuve agent 06/07** : merge `feat/mobile-etape2-porteur-feedback` → **`dev`** (`f64ae057`) ; ADB USB + git/SDK build ; polling 3 s ; smoke OTA API **OK**. **Porteur 06/07** : étape 2 install OK. **Agent 06/07 soir** : publish dev **build 5** (`publish-built`) ; Samsung **4→5** OTA OK (dialog, download ~179 Mo, « Application installée »). **Reste porteur** : valider backoffice étape 3–5 visuellement → `OK Mobile releases OTA backoffice`. **Ne pas promouvoir prod** tant que prod = smoke 17 o (build 3). | |
