...
| Mobile | **Mobile — releases OTA** | local | (1) **`docker compose up -d emulator-controller`** (une fois ; sinon bootstrap auto au chargement page). (2) **`https://jobbingtrack.localhost:5443/backoffice/mobile/releases`** — **Étape 1** build APK (ou APK déjà prêt) → **Étape 2** téléphone USB (débogage) : sélection appareil + version installée → **Installer**. (3) **Étape 3** publier canal **dev**. (4) Samsung : MAJ OTA (Paramètres ou prompt app). (5) **Étape 5** promote prod si OK. | **[ ] EN COURS** | **Preuve agent 03/07** : contrôleur Docker actif ; proxy HTTPS `:5443` health/bootstrap/apk-info **200** ; nom APK `jobbingtrack-v1.0.0+1-debug.apk` ; smoke OTA API + Playwright panel sans « Diagnostic API (404) » ; Jest releases **8/8**. **Reste porteur** : parcours Samsung complet → `OK Mobile releases OTA backoffice`. | |
