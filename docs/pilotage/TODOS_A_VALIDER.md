...
| Mobile | **Mobile — releases OTA** | local | (1) Terminal : `bash scripts/mobile/setup/restart-emulator-controller.sh`. (2) **Mobile → releases OTA** — **Étape 1** build → **Étape 2** sélection appareil ADB (version installée visible) → install. (3) **Étape 3** publier canal **dev**. (4) Promote prod si OK. | **[ ] EN COURS** | **Preuve agent 17/06** : proxy same-origin `/api/emulator-proxy` (plus d’appels directs `:5055` navigateur) ; détection ADB auto + version installée ; parcours 5 étapes UI. **Reste porteur** : `OK Mobile releases OTA backoffice`. | |
