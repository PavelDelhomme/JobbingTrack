...
| Mobile | **Mobile — releases OTA** | local | (1) Menu **Mobile → Mobile — releases OTA** — **pas** de 404. (2) **État serveur OTA** visible (stockage + PUBLIC_API_URL). (3) Upload APK debug canal **dev** (bouton **Suggestion** version/build). (4) Lien **Télécharger l’APK** + URL OTA `/mobile/releases/latest`. (5) Optionnel : **Promouvoir dev → production**. | **[ ] EN COURS** | **Preuve agent 17/06** : middleware `mobileApkUpload` **manquant** (upload impossible) — **créé** ; titres onglet corrigés (MutationObserver) ; panel enrichi (suggestion build, liens relatifs). **Reste porteur** : upload APK test → `OK Mobile releases OTA backoffice` ou KO + capture. |
