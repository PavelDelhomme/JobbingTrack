...
|------|----------------|---------------|-------------------|
| 07/09/2026 | Backoffice **BO-OVERVIEW-KPI-MEM-01** — KPI santé cliquables + budget mémoire JT 8 Go + deploy auto | local + préprod + **prod** | OK porteur : plus de ~101 Go ; recreate via `force-refresh-jt-services.sh` ; images `:latest` sans merge massif `dev`→`main`. |
