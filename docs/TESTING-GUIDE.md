...
test.describe('Authentication', () => {
  test('should login successfully', async ({ page }) => {
    ...
    await page.fill('[data-testid="email-input"]', 'redacted@example.invalid');
...
describe('Database Migrations', () => {
  ...
  test('should handle data migration correctly', async () => {
    ...
    await db.query(`
      ...
      VALUES ('redacted@example.invalid', 'Test User')
    `);

    // Vérifier la migration
    const result = await db.query('SELECT * FROM users WHERE email = $1', ['redacted@example.invalid']);
...
2. **Structure AAA (Arrange, Act, Assert)**
   ...
   test('should create user', async () => {
     // Arrange
     const userData = { email: 'redacted@example.invalid', password: 'password123' };
