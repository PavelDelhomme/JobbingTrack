...
**Dernière mise à jour** : 18 mai 2026 — **validation BDD complète + CI/CD + scripts metrics** : les anciens bruits Postgres `column Application.isTestData does not exist` et `relation log_collector_logs does not exist` sont résolus sur la base Docker active après `make db-push-all`; les colonnes critiques `Application.isTestData`, `Application.isArchived`, `Application.thankYouEmailSentAt` et `Company.isTestData` sont présentes. `make test-database` exécute maintenant la suite DB complète en local avec `DATABASE_URL` explicite et passe : **61 tables**, **121 contraintes**, intégrité OK, config Postgres/docs/Prisma OK, base temporaire `prisma db push` + `init-key-tables.sql` OK (**59 tables**, **58 FK**, **269 index**) avec `log_collector_logs` vérifiée. Le parcours API archive/corbeille candidature passe **21/21**, et les logs Postgres post-push/tests ne montrent plus `isTestData`, `log_collector_logs`, `does not exist`, `FATAL` ou `PANIC`. Les avertissements GitHub Actions Node 20 ont été traités côté workflows (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`, `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`). `scripts/testing/verify-all-metrics.sh` ne pointe plus vers les anciens ports `8014`/`3000` et passe en réel sur la stack active (**52/52**, `METRICS_URL` 5004, gateway 5002). **Reste actif** : sécurité B11/B15/B14, scans P0 à trier, ports prod trop exposés, alertes SMTP/UI admin, et durcissement prod ; voir `TODOS.md`, `STATUS.md` et `PLAN.md`.
...
**GitHub Actions — Trivy / Node 20 (12–13/05, maj 21/05)** : l'erreur `Unable to resolve action aquasecurity/trivy-action@0.28.0` venait d'une référence sans préfixe `v` et a été corrigée avec `aquasecurity/trivy-action@v0.36.0`. Les workflows projet utilisent des actions compatibles runtime Node 24 (`actions/checkout@v6.0.2`, `actions/setup-node@v6.4.0`, `actions/upload-artifact@v7.0.1`) et `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`. **21/05** : `security-audit.yml` ne dépend plus de `gitleaks/gitleaks-action@v2.3.9` (action Node 20) ; il installe le binaire Gitleaks `v8.30.1` puis exécute `scripts/security/secrets-scan.sh`. L'upload SARIF Trivy passe aussi de `github/codeql-action/upload-sarif@v3` à `@v4`. Validation GitHub : run **Security Audit `26234510539` succès** ; Gitleaks conserve l'artefact redacted mais ne bloque pas sur la dette historique brute (`GITLEAKS_FINDINGS_EXIT_CODE=0`), à trier séparément dans `docs/security/STATS.md`.

**Dependabot — alertes historiques vs checkout réel (12/05)** : les alertes GitHub `mobile/package-lock.json` ne correspondent pas au dépôt courant (`mobile/` est Flutter et ne contient pas de `package-lock.json`). Si elles restent ouvertes, vérifier la branche scannée par Dependabot ou un ancien lockfile supprimé. Côté surfaces présentes, les audits runtime npm sont verts pour `frontend`, `api-gateway`, `application-service`, `auth-service`, `deployment-service`, `workflow-service` et `security-service`; `security-service` ne dépend plus de `geoip-lite`/`ip-address`, la géolocalisation IP étant neutralisée derrière `lookupGeoIp` en attendant un fournisseur maintenu.

**Auth-service Jest hors stack DB (12/05)** : corrigé. `backend/auth-service/tests/setup.js` respecte `TEST_DATABASE_URL`/`DATABASE_URL` puis reconstruit une URL locale avec `POSTGRES_PORT=5000` par défaut; le test d'inscription utilise désormais un payload volontairement invalide. Validation : `backend/auth-service npm test -- --runInBand` = **18/18**.
...
- **Validation prod `.env` dédiée** : `make env-validate-prod` ne doit plus servir à juger le `.env` de développement ; utiliser `ENV_FILE=.env.production make env-validate-prod` pour un vrai fichier prod non versionné et `make env-validate-prod-example` pour le modèle versionné. `ALLOWED_ORIGINS` prod doit contenir uniquement des origines `https://` publiques ; `DEV_AUTH_BYPASS_TOKEN` doit rester vide/absent.
- **Variables `NEXT_PUBLIC_*` sensibles** : tout mot de passe/token dans `NEXT_PUBLIC_*` est exposé au navigateur/build. Correctif partiel 12/05 : `NEXT_PUBLIC_MOBILE_TEST_USER_PASSWORD` retiré ; le mot de passe mobile doit passer par `MOBILE_TEST_USER_PASSWORD` côté runner ou une session locale non versionnée.
...
- **Statistics log-stats — `container_logs` vs `log_collector_logs`** : `container_logs` est l’historique conteneurs de l’ancien pipeline metrics-aggregator / collecteur legacy ; `log_collector_logs` est le flux Docker alimenté par **`log-collector-rs`**. Voir `container_logs` non nul ne veut donc pas dire que Rust est absent ; vérifier plutôt que `log_collector_logs` est non nul et que `log-collector-rs` est healthy.
- **Statistics log-stats — `system_events` / `service_network_history` à 0** : avant le correctif 20/05, ces tables pouvaient rester vides alors que les métriques/logs principaux étaient OK. Elles sont désormais alimentées par `metrics-aggregator` : événements sur observation/transition de statut service, et historique réseau basé sur les probes santé services. Validation attendue : `node scripts/ops/smoke-persistence-stats.cjs` avec `events > 0` et `serviceNetwork > 0`.
- **Vue d’ensemble backoffice — carte « Sessions actives » (22/06/2026)** : la carte hub `/backoffice` affiche le nombre de sessions récentes (`/api/v1/auth/sessions/active`, fenêtre 30 min) mais le sous-titre utilise le **length** d’une requête `users?limit=10` (souvent **10**), et le lien ouvre **`/backoffice/users?status=active`** qui liste les **comptes activés** (~100 en local, majoritairement E2E/tests) — **pas** les sessions affichées. Ce n’est pas un simple décalage de cache : définitions métier différentes + pollution comptes test. Corriger avant prod (voir **`docs/pilotage/TODOS.md`** et gate **`docs/production/A_VALIDER_AVANT_PRODUCTION.md`**).
- **Backoffice fiche utilisateur — renvoi email vérif (22/06/2026)** : action **Renvoyer email de vérification** sur **`/backoffice/users/[id]`** peut afficher `AxiosError: Network Error` / `ERR_NETWORK_CHANGED` alors que le gateway reçoit le `POST`. Test agent : endpoint **200** en ~873 ms via gateway HTTP direct — backend fonctionnel ; suspect : timeout/proxy HTTPS, **Fast Refresh** Next.js en dev, ou latence SMTP. UI : messages `alert()` peu explicites. Route sans `requireAdmin` dans `users.routes.js`. Reset MDP admin : UI sur **`/auth/forgot-password`** au lieu de **`/users/:id/send-password-reset`**. Voir **`docs/pilotage/TODOS.md`** § actions admin.
...
| **Script « Tests API Backend » — `Status: 000` + corps JSON incohérent** | **`scripts/testing/test-api-specific.sh`** | **`curl` `000`** = pas de réponse HTTP ; le JSON affiché venait souvent d’un **corps `/tmp/response.txt` réutilisé** entre appels | **Corrigé (17/04)** : **`mktemp`** par requête + normalisation **`api-gateway` → 127.0.0.1** en tête de script |
...
| **auth-service unhealthy / gateway `ECONNREFUSED :3001` + Prisma « Authentication failed… credentials… not valid » alors qu’un `psql` « dans Postgres » semble OK** | Postgres + `.env` | Un `docker exec jobbingtrack-postgres psql -h localhost …` **sans** `PGPASSWORD` peut réussir (**trust / socket local**) sans prouver que le mot de passe du **`.env`** est celui du rôle pour l’auth **TCP** (`postgres:5432`) utilisée par Prisma depuis **auth-service** | Vérifier le **vrai** test TCP : `docker run --rm --network <réseau_stack> -e PGPASSWORD=… postgres:15 psql -h postgres -U jobbingtrack -d jobbingtrack -c 'select 1'`. Si échec : **`ALTER USER jobbingtrack WITH PASSWORD`** aligné sur **`POSTGRES_PASSWORD`** du `.env` (guillemets dollar PostgreSQL si besoin), puis **`docker restart jobbingtrack-auth-service`**. Le volume Postgres conserve le secret initial : changer seulement le `.env` sans **ALTER** ou sans recréer le volume ne suffit pas. |
| **`FATAL: password authentication failed for user "jobbingtrack"` en rafale (~15 s) sans action login** | **metrics-aggregator** (Prisma + collecteur `METRICS_COLLECTION_INTERVAL_NORMAL` défaut **15 s**) + autres clients TCP | Souvent **mot de passe avec caractères spéciaux** dans `DATABASE_URL` interpolé par Compose **sans** encodage URL (`@ : # + %`…), ou **mot de passe du volume Postgres** ≠ **`POSTGRES_PASSWORD`** du `.env` | **Encodage** : depuis le code, `applySafeDatabaseUrl` (voir `src/utils/applySafeDatabaseUrl.js`) + variables **`POSTGRES_*`** sur le service **`jobbingtrack-metrics-aggregator`** dans **`docker-compose.yml`**. **Désalignement volume** : tester TCP `psql -h postgres` depuis un conteneur sur le réseau stack avec `PGPASSWORD` = valeur `.env` ; si échec, **`ALTER USER jobbingtrack WITH PASSWORD`** aligné sur `.env` (sans tout effacer), puis **`docker compose … up -d --force-recreate jobbingtrack-metrics-aggregator`**. |
| **`FATAL: password authentication failed…` en rafale après changement `.env` / `ALTER USER`** | Tous les services avec `DATABASE_URL` | Le mot de passe est **figé dans l’env du conteneur** à sa création ; healthchecks / fond tournent sans action UI | **`make recreate`** (profils **`full` + `monitoring`**) ou équivalent `docker compose -f docker-compose.yml -f docker-compose.monitoring.yml --profile full --profile monitoring up -d --force-recreate`. Vérification : empreinte du mot de passe dans `docker inspect <service>` (`DATABASE_URL`) vs **`POSTGRES_PASSWORD`**. |
| **`FATAL: password authentication failed…` en rafale après `make recreate`** | Postgres volume vs `.env` | Un **`--force-recreate`** sur **postgres** (ex. via autre cible) peut laisser le **volume** avec l’ancien mot de passe du rôle alors que les conteneurs ont le **nouveau** `DATABASE_URL` | Réaligner le rôle : **`ALTER USER jobbingtrack WITH PASSWORD`** = **`POSTGRES_PASSWORD`** du `.env`, puis **`make recreate`** à nouveau **sans** recréer postgres seul de façon incohérente. **`make recreate`** inclut notamment **metrics-aggregator**, **monitoring-agent-rs**, **log-collector-rs** (Rust). Legacy **C** : **`make monitoring-c-up`** (profil **`monitoring-c`**) ; pas dans le parcours `full`/`monitoring` par défaut. |
...
| ~~Login HTTPS dev — URL API `:5002` en TLS ou 401 admin~~ | **18/05** : front → `https://api.jobbingtrack.localhost:5443` (`ports.config.ts`, `api.ts`) ; resync admin via `create-admin-user.sh` (Prisma + env, sans interpolation shell) ; doc **`docs/troubleshooting/TROUBLESHOOTING_LOGIN.md`**. |
...
### Verification
- `scripts/testing/test-api-specific.sh` : 51/51 passes apres correctifs.
