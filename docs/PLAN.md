...
| A1h | **Statistics & vue d’ensemble backoffice — fiabilité monitoring global** : corriger les KPI **services** (ex. majorité « hors ligne » à tort sur **`/backoffice/statistics`**) ; refaire graphes **erreur dans le temps** et **disponibilité dans le temps** ; réaligner onglets **Sécurité** et **Logs** de Statistiques sur **`aggregated_logs`** / **log-collector-rs** / contrats sécurité paginés ; harmoniser **`/backoffice`** (vue d’ensemble) avec la même source métriques que **`/backoffice/services`** ; audit des **~73** pages `backoffice/**` non encore revues (HTTPS centralisé, persistance, empty states). Détail cases : **`TODOS.md`** § **Chantier Statistiques & backoffice** | Partiel (20/05/2026) | Oui — KPI services Statistics + `/backoffice` et filtres log-stats validés porteur ; reste tests conteneurs arrêtés/dégradés + taux d’erreur dans le temps | `statistics/page.tsx`, `statistics/security/page.tsx`, `statistics/log-stats/page.tsx`, `backoffice/page.tsx`, `centralMetricsService.ts`, `metrics-aggregator-service` |

**Note (priorisation)** : poursuivre **A1–A2** et **priorités immédiates** (`TODOS.md` : corrélation logs, central logging, sécurité) en premier ; **A1h** + fin **A5** sur **Statistics / vue d’ensemble** dès que le socle collecte/persiste de façon fiable.
...
| I3 | **Connexion email lecture seule** : OAuth Gmail et boîte candidatures configurés hors Git, scopes minimaux, tokens chiffrés, révocation visible | À faire | Non | Worker planifié ; ne pas envoyer/supprimer d’email au départ |
...
| I7 | **Google Calendar + vue calendrier agrégée** : créer tâches/relances/événements, entretiens, job dating, salons, rappels et calendrier unifié | À faire | Non | Calendar obligatoire pour entretiens/job dating/salons ; confirmation avant tout envoi externe |
...
- **Statistics & `/backoffice` (lot A1h)** : compteurs services et courbes **erreur / disponibilité** non fiables en vue d’ensemble (symptôme : ~22 « offline », 1 « sain ») — voir **`TODOS.md`** § **Chantier Statistiques & backoffice**.
