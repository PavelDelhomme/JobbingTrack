...
**Dernière révision du plan** : 11 mai 2026 — **B15** ajouté et outillé partiellement : matrice tests sécurité offensifs contrôlés, workflow `.github/workflows/security-audit.yml`, cibles `make security-*`, rapports locaux `reports/security/`, contrainte performance explicite (runtime léger gateway/public uniquement, pas d’analyse du trafic inter-conteneurs). **B11** : socle alertes email critiques backend en place (`SecurityAlert` `critical/high` → `notification-service`) ; reste UI admin réauth/audit, anti-flap et service down. **Blocage prod scan** : `docker-compose.prod.yml` seul n’est pas scannable de façon fiable (`auth-service` sans `image`/`build`) ; corriger la config ou documenter la commande compose fusionnée. Historique conservé : **6–7 mai** B14/A3 corrélation, logs-watch, rechargements axios, budget ressources ; voir `STATUS.md` et `TODOS.md`.
...
| B5 | WAF + `make security-live-check` : auth firewall/WAF sur security-service (JWT ou `X-Internal-Secret`) ; scripts alignés ; **rebuild** image si deps (`jsonwebtoken`) ; volume `src` ; `FIREWALL_BASE_URL` / `AUTH_GATEWAY_URL` ; **`user: 0:0` + NET_ADMIN** pour **iptables** en dev | À renforcer | Non | `security-service/server.js`, `scripts/security/*.sh`, `docker-compose.yml` ; **RESOLUTIONS.md** § 8 avril 2026. **11/05 partiel** : bypass WAF interne configurable (`WAF_INTERNAL_BYPASS_*`) pour localhost/subnets Docker + tests externe bloqué / interne autorisé ; **reste** : valider CIDR remote host/reverse proxy du serveur et logs séparés prod |
...
| B8 | **Vue backoffice « investigation / post-incident »** : croisement **menaces WAF**, **audit B7**, **logs techniques** (A2), **IPs** ; export **horodaté** (CSV/JSON) avec **hash** ou empreinte pour **chaîne de conservation** légale / interne ; pas de promesse « preuve judiciaire » sans dispositif certifié — viser **traçabilité opérationnelle** forte | Partiel (11/05/2026) | Non | `frontend/.../backoffice/security/**` ; APIs agrégation ; doc procédure dans **`docs/security/`** (à créer) : qui consulte, où stocker l’export. **Fait partiel** : fiche menace enrichie attacker/target/application/network, logs 24h, DDoS/intrusions liés, fallback `network_connections`; **reste** : IP intelligence ASN/VPN/proxy/Tor, payload samples, comptes impactés fiables, export signé |
...
| B10 | **UX backoffice — outils sécurité** : reprendre **`/backoffice/security`** et sous-pages (**firewall**, **analyse**, **réseau**, etc.) pour un ensemble **réellement utilisable** : lisibilité (hiérarchie visuelle, tableaux), **métriques cohérentes** avec les contrats API (libellés, unités, fenêtres temporelles), **empty states** honnêtes, **mesurabilité** (ce qui est compté / exclu), préparation des emplacements pour la vue **investigation (B8)** ; **vue d’ensemble** : éviter qu’un même **`network_threat_detected`** apparaisse à la fois comme **menace** et comme **log CRITICAL** (filtrage incidents **22/04**) ; **score** : aujourd’hui pondération locale **`securityScoreWeights`** (`localStorage`, **`page.tsx`**) — à documenter / option serveur | Partiel (11/05/2026) | Non | `frontend/src/app/(admin)/backoffice/security/**` ; **`centralMetricsService`** / clients API ; tests manuels checklist dans **`TODOS.md`** ; **`ERRORS.md`** § boot **`up-full`** / intrusion. **11/05** : filtres menaces responsives, auto-refresh silencieux, cartes Analyse cliquables vers menaces/firewall filtrés ; **reste** : page réseau “IP source à surveiller” détaillable, seuils expliqués, WAF on/off testable proprement |
| B11 | **Alertes email sécurité et disponibilité** : envoyer un email pour les événements réellement critiques (SecurityAlert `critical/high`, CVE critique/haute, DDoS, intrusion sévère, firewall majeur, service/conteneur critique `down`) sans spammer ; la configuration admin doit demander réauth + audit. | Partiel (11/05/2026) | Non | **Fait** : `security-service` → `notification-service` via route interne `X-Internal-Secret`; variables `SECURITY_ALERT_EMAIL(S)`, `SECURITY_ALERT_EMAIL_LEVELS`, `SECURITY_ALERT_EMAIL_ENABLED`; logs `EmailLog`; tests ciblés security/notification. **Reste** : UI admin réauth/audit, anti-flap/dédup, service down metrics-aggregator, digest `high` |
