...
**Dernière révision du plan** : 12 mai 2026 — **B15** ajouté et outillé partiellement : matrice tests sécurité offensifs contrôlés, workflow `.github/workflows/security-audit.yml`, cibles `make security-*`, rapports locaux `reports/security/`, contrainte performance explicite (runtime léger gateway/public uniquement, pas d’analyse du trafic inter-conteneurs). **12/05** : validation applicative sécurité/backoffice renforcée et verte (`security-service` **15/15**, frontend Jest **86/86**, Playwright sécurité **16/16**, lint/type-check OK), vues sécurité stabilisées contre les données partielles et auto-refresh concurrents, fiche menace renforcée via `network_connections`, et audit config `.env` outillé (`make env-validate`). **B11** : socle alertes email critiques backend en place (`SecurityAlert` `critical/high` → `notification-service`) et alertes disponibilité `SERVICE_DOWN` branchées depuis le metrics-aggregator avec dédup ; reste UI admin réauth/audit, digest et validation SMTP/MailHog. **B14** : retrait de fallbacks dangereux renforcé : scripts env sous `scripts/env/`, inventaire `scripts/`, `.env.production.example`, validation prod dédiée via `ENV_FILE`, `ALLOWED_ORIGINS` HTTPS-only, CORS gateway aligné sur l'env, WAF pilotable via Make, metrics auth dev par proxy serveur sans clé `NEXT_PUBLIC_*`, et HTTPS dev avec CA locale navigateur + proxy Nginx. **Scan prod** : cible compose fusionnée corrigée ; reste durcissement des ports publiés. Historique conservé : **6–7 mai** B14/A3 corrélation, logs-watch, rechargements axios, budget ressources ; voir `STATUS.md` et `TODOS.md`.
