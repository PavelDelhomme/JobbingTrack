...
- **Admin** : `admin@jobbingtrack.test` / `password123` (ou `ADMIN_EMAIL` / `ADMIN_PASSWORD`)
...
2. Lancer le seed à la main :  
   `docker exec -e ADMIN_EMAIL=admin@jobbingtrack.test -e ADMIN_PASSWORD=password123 jobbingtrack-auth-service npx prisma db seed`
