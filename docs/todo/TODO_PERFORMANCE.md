...
- [ ] Réduire l'overhead de `monitoring-c` (fork/exec répétés) avec collecte plus native. **07/05 partiel** : suppression des forks `docker inspect` / `docker port` / `curl` pour les health checks (URL connues + libcurl) ; reste `docker stats`/`docker ps` et parallélisation.
