...
- [x] Réduire l'overhead de `monitoring-c` (fork/exec répétés) avec collecte plus native. **07/05** : inventaire conteneurs via Docker socket Unix sans shell, métriques CPU/mémoire/réseau via cgroups/proc, health checks via libcurl multi parallèle ; plus aucun `popen` dans `monitoring-c`.
