...
**Dernière mise à jour** : 15 mai 2026 — **Politique env stricte (amorcée)** : sentinelle globale `__JT_ENV_INCOMPLETE__`, module `config/jt-env-policy.cjs`, doc `docs/configuration/STRICT_ENV.md`, validation renforcée dans `scripts/env/env-validate-runtime.cjs`, boot **api-gateway** via `src/bootstrap/strictGatewayEnv.js` et service **api-gateway** dans `docker-compose.yml` sans substitutions silencieuses sur les clés critiques (`:?`, Redis réseau Docker). **Organisation scripts** : helpers bypass tests/WAF regroupés sous **`scripts/env/`** (`dev-test-bypass-fetch.cjs`, `dev-test-bypass-curl.inc.sh`), suppression du dossier **`scripts/dev/`**, index **`scripts/env/README.md`** à jour. Le déploiement env strict sur les autres services / compose reste planifié dans **`docs/TODOS.md`**. **13/05 (inchangé par ailleurs)** — **BDD validée + CI/CD corrigé + scripts metrics** : `make db-push-all` relancé sur la stack Docker, puis `make test-database` réaligné pour exécuter localement la suite complète DB avec `DATABASE_URL` hôte explicite. Résultats : connexion OK, **61 tables**, **121 contraintes**, intégrité OK, configuration Postgres/docs/Prisma OK, `prisma db push` sur base temporaire OK (**59 tables**, **58 FK**, **269 index**), table `log_collector_logs` présente, colonnes critiques présentes (`Application.isTestData`, `Application.isArchived`, `Application.thankYouEmailSentAt`, `Company.isTestData`), et parcours API archive/corbeille candidature OK (**21/21**). Logs Postgres après push/tests : aucune erreur `isTestData`, `log_collector_logs`, `does not exist`, `FATAL` ou `PANIC`. CI/CD : workflows GitHub migrés hors actions Node 20 dépréciées (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`) avec `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`; validation DB CI via `DATABASE_URL` de test, `prisma db push` et format Prisma non bloquant. **13/05 complément** : `backend/package-lock.json` resynchronisé avec `backend/package.json` après l'échec `npm ci` (`eslint-plugin-import`, `eslint-plugin-n`, `eslint-plugin-promise` manquants), et `ci-cd.yml` rend bloquants lint/type-check/tests/build critiques au lieu de les masquer par `|| echo`. **Scripts metrics** : `scripts/testing/verify-all-metrics.sh` modernisé et validé en réel sur la stack active (**52/52**, sortie `0`). **Accès admin post-tests** : IP Docker/proxy `172.19.0.1` débloquée, WAF dev sans bannissement durable d’IP privée, URL admin publique **`/b4ck0ff1ce`**. **Hygiène racine** : rapports générés sortis de Git (`backend-performance-reports/`, `diagnostic-reports/`, `test-results*`), sorties attendues sous `reports/performance/`, `reports/diagnostics/` et `tests/results/`; anciens prototypes Python `services/`, `mobile-native-app/` et `backend/src/services/cache.service.js` archivés sous `docs/archive/`; script personnel sensible `create-pavel-user.sh` et ancien serveur `test-runner-service.js` supprimés du suivi Git.
...
**Secrets / PostgreSQL / doc env (15/05)** : `scripts/env/env-generate-secrets.cjs --with-postgres` réécrit **`DATABASE_URL` pour l’hôte** (Prisma, scripts) à partir de `POSTGRES_USER`, `POSTGRES_DB`, `POSTGRES_PORT` et **`POSTGRES_CLIENT_HOST`** (défaut `localhost`) ; **`docker-compose.yml`** continue d’injecter `...@postgres:5432/...` dans les conteneurs. **`.env.example`** : clarification **`EMULATOR_CONTROLLER_URL`** (hôte vs `host.docker.internal` dans le conteneur `frontend`), ports collecteurs **Rust** (`LOG_COLLECTOR_RS_PORT`) vs **legacy C** (`LOG_COLLECTOR_C_PORT` / `LOG_COLLECTOR_C_LEGACY_PORT` sans conflit de bind), **`SECURITY_CRITICAL_SERVICES`** inclut **`jobbingtrack-monitoring-agent-rs`** et **`jobbingtrack-log-collector-rs`** (retirer les noms C du CSV si le conteneur n’existe pas dans ton merge Compose). **`AUDIT_SEC_PROJECT.md`** rangé sous **`docs/security/`** (liens mis à jour). Chantier **WAF reverse-proxy dédié** : étapes dans **`docs/security/ACTIVATION_WAF.md`** § chantier WAF edge ; pas encore de service Compose séparé.

**Audit documentation / validation porteur (13/05)** : ajout de **`docs/security/AUDIT_SEC_PROJECT.md`** (risques sécurité projet et suites P0), **`A_VALIDER_VERIFIER.md`** (registre porteur avant prod), **`BRANCHES.md`** (conventions branches/commits) et **`monitoring/RAPPORT_MONITORING_GOOD_PRACTICE_GO_AND_C.md`**. Point clarifié : la réécriture complète de `monitoring-c` selon les règles C demandées n'a pas été faite ; la trajectoire reste Rust, avec benchmark long post-bascule Rust à faire au gate préprod.

**Rapports sécurité backoffice (13/05)** : les routes **`frontend/src/app/api/test-reports/*`** listent, affichent, téléchargent et suppriment désormais les rapports générés sous **`reports/security/**`** et **`tests/results/security/**`** quand un répertoire contient `summary.md`, `summary.json` ou `report.html`. La page **`/backoffice/test-reports`** connaît le type `security`. À valider par le porteur : présence de la catégorie **Sécurité**, lecture du résumé et téléchargement lisible sur un vrai rapport local/preprod.

**Tri P0 sécurité (13/05)** : `docs/security/STATS.md` contient maintenant un tri initial priorisé des résultats `gitleaks`/Trivy déjà consignés. Limite importante : les dossiers datés `reports/security/secrets-20260511-142026/` et `tests/results/security/cve-20260511-162025/` ne sont pas présents dans le workspace actuel, donc le tri finding par finding reste à faire après récupération des artefacts CI ou régénération locale autorisée.
...
**Fluidité backoffice (11/05)** : premier correctif transverse lancé côté frontend : préchargement explicite des routes internes du menu admin/raccourcis et `loading.tsx` global sous `/backoffice/**` pour rendre les transitions immédiatement visibles. Suite à faire : profiler les pages lourdes, lazy-loader les blocs charts/tests/rapports, réduire polling/animations coûteuses, et parcourir toutes les pages admin.

**Audit documentation complet (11/05)** : nouveau plan **`operations/DOCUMENTATION_AUDIT_PLAN.md`** pour vérifier tous les dossiers docs demandés, les fichiers racine `docs/*`, `docs/generate-pdfs.js`, `docs/pdf-style.css`, `docs/pdfs/` et `monitoring/MIGRATION_RUST.md` avant restructuration. Règle : fusionner et déplacer par petits lots, avec liens/index mis à jour immédiatement.

**Validation sécurité/backoffice (12/05)** : `frontend npm run type-check`, `frontend npm run lint`, lints Cursor et `git diff --check` OK. `backend/security-service` OK (**15/15**), Jest frontend OK (**86/86**) et Playwright sécurité applicatif via frontend `5003` + gateway `5002` OK (**16/16**) sur XSS, SQL/NoSQL injection, auth/JWT, headers et protections d’attaque. Correctif important : les tests `security-service` utilisent maintenant la base Docker locale via `POSTGRES_PORT` (`5000` par défaut) au lieu de forcer `localhost:5432`. Redis runtime vérifié : conteneur healthy, `PING` OK, clés `intrusion:*` et `blocked_ip:*` présentes. **Suite alertes disponibilité** : tests ciblés `service-availability-alerts`, `security-cve-alerts`, `server` OK (**10/10**) ; le lint CLI `security-service` reste bloqué par la config ESLint backend existante (`@typescript-eslint/recommended` introuvable), sans diagnostic Cursor sur les fichiers modifiés.

**Audit `.env` / `.env.example` (12/05)** : `.env` et `.env.example` sont alignés en clés après ajout explicite de `ENABLE_METRICS_AUTH` et remplacement de `NEXT_PUBLIC_MOBILE_TEST_USER_PASSWORD` par `MOBILE_TEST_USER_PASSWORD`. `make env-validate` vérifie sans afficher les valeurs les variables critiques, placeholders/secrets faibles, SMTP si alertes activées, exposition de secrets en `NEXT_PUBLIC_*`, et contraintes prod (`ALLOWED_ORIGINS` HTTPS-only, WAF actif, auth metrics active, bypass dev absent). Résultat local dev après `make env-generate-secrets` : **OK sans avertissement**. La validation prod ne cible plus le `.env` dev par défaut : utiliser `ENV_FILE=.env.production make env-validate-prod`; le modèle versionné se vérifie avec `make env-validate-prod-example`. `ENABLE_METRICS_AUTH=true` est maintenant la valeur dev attendue, avec clé injectée uniquement côté gateway/route Next serveur.

**HTTPS dev (12/05)** : le flux navigateur local passe désormais par `docker-compose.https.yml` et `production/nginx/dev-https/default.conf`. `make dev-https-install-ca` génère une CA locale hors Git (`.local/dev-certs/`) et l’installe dans les magasins NSS disponibles ; `make dev-https-up` expose le front et la gateway sur `https://jobbingtrack.localhost:5443` et `https://api.jobbingtrack.localhost:5443`. Vérifié avec `curl --cacert` sur `/health` frontend et gateway, puis en smoke navigateur réel sur `/backoffice/security`, `/backoffice/security/threats` et `/backoffice/services` avec JWT/cookie admin réel. HSTS strict reste réservé aux vrais domaines préprod/prod.

**HTTPS dev / `up-full` (15/05)** : par défaut, **`make up-full`** enchaîne la génération des certificats locaux et **`make dev-https-up`** (désactivation : **`UP_FULL_HTTPS=0`**). Les invocations Compose du monitoring incluent **`docker-compose.https.yml`** pour éviter l’avertissement **« orphan »** sur **`jobbingtrack-dev-https-proxy`**. Le profil **`full`** a été retiré du service HTTPS : seul le profil **`https`** le démarre. Critère outillage côté hôte après **`make dev-https-install-ca`** : **`curl` vers `https://jobbingtrack.localhost:5443/login` en 200 sans `--cacert`** ⇒ chaîne TLS + CA **système** OK ; si le navigateur refuse encore le certificat, privilégier import PEM ou navigateur non Flatpak/Snap (**`docs/operations/DEV_HTTPS.md`**). Les logs Redis « intrusion detector » au shutdown passent en **warn** pour réduire le bruit. **Suite 15/05** : Nginx sert un **`jobbingtrack-dev-fullchain.pem`** (feuille + CA OpenSSL) ; après **`trust anchor`**, le script tente **`trust extract-compat`** ; redémarrer le proxy TLS pour prendre en compte le fullchain.

**HTTPS dev / connexion backoffice (14/05)** : l'URL locale officielle reste `https://jobbingtrack.localhost:5443/login` ; une faute de domaine comme `jobbingtrck.localhost` n'est pas couverte par le certificat et doit rester non sécurisée. Le générateur OpenSSL `scripts/ops/dev-https-certs.sh` produit maintenant une vraie CA locale (`basicConstraints=CA:true`, `keyUsage=keyCertSign,cRLSign`) et remplace l'ancienne entrée NSS avant réinstallation. Validation stricte après régénération et redémarrage du proxy HTTPS : frontend `/health` `200`, gateway `/health` `200`, login API HTTPS `200`, profil HTTPS `200`, rôle `SUPER_ADMIN`. Côté frontend navigateur, les pages/composants/hooks lisent désormais l'API via `FRONTEND_URLS.api` ; seules la configuration centralisée et les routes serveur Next gardent des variables d'environnement directes pour les URLs internes.
...
**Dependabot / npm supply-chain (12/05)** : première passe réelle effectuée sur les manifestes présents. Corrections appliquées : `frontend` (`next` 16.2.6, `axios` 1.16.0, `postcss` 8.5.14, `jspdf` 4.2.1, overrides runtime pour `postcss`/`glob`), backend (`axios` 1.16.0, `nodemailer` 8.0.7, `express` 4.22.2, `jsonwebtoken` 9.0.3, lockfiles hors workspace synchronisés). Audits runtime verts : `frontend`, `api-gateway`, `application-service`, `auth-service`, `deployment-service`, `workflow-service`, puis `security-service` après retrait de `geoip-lite`. Compat Next 16 corrigée : `useSearchParams` de `/verify-email` enveloppé par `Suspense`, route handlers catch-all alignés sur `params: Promise<...>`, `next.config.js` nettoyé des options invalides. Tests : `frontend npm run type-check`, `npm run lint` et `npm run build` OK avec warnings historiques (`middleware` à renommer `proxy`, Browserslist, trace NFT `test-reports/schedule`). `api-gateway` Jest **27/27**, `application-service` Jest **3/3**, `auth-service` Jest **18/18**, `security-service` Jest **18/18**. La géolocalisation IP est désormais un provider neutre (`lookupGeoIp`) à brancher plus tard sur un fournisseur maintenu ASN/VPN/proxy/Tor.
...
**Scripts — restructuration progressive (12/05)** : plusieurs lots sûrs déplacent les scripts racine vers des dossiers métier sans casser les commandes Make/CI/routes : `env/`, `setup/`, `db/`, `docker/`, `reports/`, `performance/`, `ops/`, `health/`, `testing/`, `utils/`. Validations : `bash -n` sur les scripts déplacés, `make -n setup-ports`, `make -n test-mobile-report`, `make -n status-watch`, `make -n logs-watch`, `make -n menu`, `make -n fix-all`, `make -n git-checkout`, `make -n test-performance-backend`, puis `make scripts-inventory`. État actuel : **122 scripts**, **60 actifs**, **0 script classé racine**, **0 sans référence automatique** après audit documenté dans `docs/scripts/NON_REFERENCED_SCRIPTS_AUDIT.md`; les scripts racine de test/Playwright/rapports restants ont été rangés sous `scripts/testing/`, `scripts/monitoring/`, `scripts/performance/`, `scripts/reports/` et `scripts/setup/`; `scripts/testing/test-reset-password.sh` est modernisé pour la gateway actuelle et la clé metrics optionnelle ; validation réelle stack active OK sur `POST /api/v1/auth/forgot-password` après correction auth-service (fallback `User.resetToken/resetTokenExpiry` quand le modèle Prisma `PasswordResetToken` n'existe pas). `scripts/testing/verify-all-metrics.sh` est aussi modernisé pour `METRICS_URL`/`API_GATEWAY_URL`, les ports exposés actuels et l'auth metrics optionnelle ; validation réelle stack active : **52/52 réussis**, **0 avertissement**. Les scripts restants à traiter sont classés legacy/manuel avant archivage progressif.
...
**Smoke services via gateway (12/05)** : `tests/services/smoke-gateway-services.js` vérifie le chemin prod-like `API_GATEWAY_URL` pour les services exposés (`auth`, `applications`, `companies`, `contacts`, `interviews`, `calls`, `events`, `followups`, `notifications`, `dashboard`, `profile`, `workflows`, `security`). Les routes protégées valident `401/403` comme joignables/auth requise ; `profile-service` cible `/api/v1/profile/me` pour éviter le faux `404` de la racine. Branché dans `scripts/run-all-tests-with-reports.sh` sous `Smoke Gateway Services`. Validation réelle locale : `API_GATEWAY_URL=http://127.0.0.1:5002 /usr/bin/node tests/services/smoke-gateway-services.js` = **14/14 succès**.

**Smokes infra non-gateway (13/05)** : les exceptions F3 sont maintenant couvertes par scripts dédiés sous `tests/services/`. `test-metrics-aggregator.js` vérifie `/health`, le refus sans `X-API-Key` (`401`), l’accès métriques avec clé (`200`) et `/api/v1/docker/services/all` avec clé (`200`) ; validation réelle : **4/4** sur `127.0.0.1:5004`. `test-deployment-service.js` vérifie `/health`, `/health/detailed` et `/api/v1/deployments` ; validation réelle : **3/3** sur `127.0.0.1:5018`. Les deux scripts sont repris automatiquement par `scripts/run-all-tests-with-reports.sh`.

**Suite complète agrégée (13/05)** : l’équivalent direct de `make tests` a été lancé sans `make` via `bash scripts/run-all-tests-with-reports.sh` avec `API_GATEWAY_URL=http://127.0.0.1:5002`, `PLAYWRIGHT_BASE_URL=http://localhost:5003`, Playwright en smoke et `PERF_LIGHT=1`. Résultat final : **436/436 tests réussis**, **1 ignoré**, sortie `0`, rapport local `tests/results/20260513-002408/`. Correctifs nécessaires avant le vert : runner `.env` non exécutant + `exit 1` en cas d’échec, seed auth masqué, alignement `TEST_ADMIN_*`/`ADMIN_*`, JWT admin pour firewall/WAF, routes applicatives `/:id/status*` avant `/:id`, normalisation du statut candidature, et colonnes/tables critiques ajoutées au flux `db-push-all`, dont `Application.isTestData` et `log_collector_logs`.
