...
- **Security Audit GitHub / Node 20** : `security-audit.yml` retire `gitleaks/gitleaks-action@v2.3.9` (runtime Node 20) au profit du binaire Gitleaks installé depuis la dernière release puis `scripts/security/secrets-scan.sh`; l'upload SARIF Trivy passe à `github/codeql-action/upload-sarif@v4`.
