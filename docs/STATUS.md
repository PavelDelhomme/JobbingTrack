...
- **Crash-report SMTP** : documentation alignée sur la règle de délivrabilité : `CRASH_REPORT_FROM` sur le compte SMTP authentifié si le fournisseur l’exige, `CRASH_REPORT_REPLY_TO` et `CRASH_REPORT_EMAIL` sur l’alias métier `crash-report@...` redirigé hors Git.
