...
**Dependabot / npm supply-chain (12/05)** : première passe réelle effectuée sur les manifestes présents. Corrections appliquées : `frontend` (`next` 16.2.6, `axios` 1.16.0, `postcss` 8.5.14, `jspdf` 4.2.1, overrides runtime pour `postcss`/`glob`), backend (`axios` 1.16.0, `nodemailer` 8.0.7, `express` 4.22.2, `jsonwebtoken` 9.0.3, lockfiles hors workspace synchronisés). Audits runtime verts : `frontend`, `api-gateway`, `application-service`, `auth-service`, `deployment-service`, `workflow-service`. Compat Next 16 corrigée : `useSearchParams` de `/verify-email` enveloppé par `Suspense`, route handlers catch-all alignés sur `params: Promise<...>`, `next.config.js` nettoyé des options invalides. Tests : `frontend npm run type-check`, `npm run lint` et `npm run build` OK avec warnings historiques (`middleware` à renommer `proxy`, Browserslist, trace NFT `test-reports/schedule`). `api-gateway` Jest **27/27**, `application-service` Jest **3/3**. `auth-service` Jest reste rouge hors stack DB car il force encore `localhost:5432` dans ce contexte de test ; à corriger séparément. `security-service` garde une dette connue `geoip-lite`/`ip-address` : la version corrigée amont demande Node 24, donc remplacement ou migration Node contrôlée à planifier.
