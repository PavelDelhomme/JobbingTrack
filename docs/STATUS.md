...
- **Statistics — homogénéisation UI (20/05 suite)** : `app-data`, `security` et `log-stats` passent sur un shell commun (largeur, en-tête, bouton refresh, surfaces). `app-data` reste volontairement limitée aux **totaux** + **timeline globale** dans ce lot ; le détail métier (actifs vs total, nouveaux sur période, candidatures par statut, entreprises/contacts, relances, entretiens, appels, événements, états vides par source) est noté pour le lot suivant.
...
- **Sécurité / forensics à cadrer** : enrichissement des IP sources externes demandé côté menaces détectées. À faire via sources techniques contrôlées (reverse DNS, RDAP/ASN, réputation, VPN/proxy/Tor/datacenter, cache/rate-limit), pas via proxys gratuits non fiables ni collecte de données personnelles.
- **Reste à planifier** : smoke navigateur log-stats après login porteur, vérification fonctionnelle du **taux d’erreur dans le temps**, vérification granularité erreurs, audit lisibilité du mode clair backoffice, et enrichissement métier réel de `statistics/app-data`.
