...
- **Refonte filtres B10 (en cours)** : branche `refactor/backoffice-filter-facets` — hook `useAppliedFilters`, composants `FilterBar` / `FilterSelectField` / `FacetAutocompleteField`, `facetUtils`, client `fetchSecurityLogFacets`, `AutocompleteInput` enrichi (variant `plain`, suggestions libellées). Pages migrées : Logs sécurité + Menaces. Validations : Jest ciblé **13/13 OK**, `tsc --noEmit` OK, ESLint ciblé **0 erreur** (warnings `any` historiques menaces). Reste : incidents, emails, rapports, utilisateurs, statistics.
