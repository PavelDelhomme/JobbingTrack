...
**Dernière mise à jour** : 19 mai 2026 — **Branche** `feat/ui-motor` — chargement backoffice dark (`PageLoader`, `TableSkeleton`, layout admin corrigé) ; merge statistics sur `dev`. Ancienne branche : `feat/statistics-threat-logging-validation` (depuis `dev`) : validation porteur sécurité (navigation, pagination, corrélation) ; Statistics A1h (compteurs + graphes persistance) ; `make status-watch` touches **5–9** pour l’intervalle (défaut **5 s**) ; correctif page **Paramètres** (reset personnalisation, crash `duration`) — **dette** : pas encore de moteur UI/layout dédié (voir **`TODOS.md`** § moteur personnalisation). Détail BDD/CI **13/05** : section **Validation sécurité/backoffice (12/05)** ci-dessous, pas en tête de fichier. — **Politique env stricte (amorcée)** : sentinelle globale `__JT_ENV_INCOMPLETE__`, module `config/jt-env-policy.cjs`, doc `docs/configuration/STRICT_ENV.md`, validation renforcée dans `scripts/env/env-validate-runtime.cjs`, boot **api-gateway** via `src/bootstrap/strictGatewayEnv.js` et service **api-gateway** dans `docker-compose.yml` sans substitutions silencieuses sur les clés critiques (`:?`, Redis réseau Docker). **Organisation scripts** : helpers bypass tests/WAF regroupés sous **`scripts/env/`** (`dev-test-bypass-fetch.cjs`, `dev-test-bypass-curl.inc.sh`), suppression du dossier **`scripts/dev/`**, index **`scripts/env/README.md`** à jour. Le déploiement env strict sur les autres services / compose reste planifié dans **`docs/TODOS.md`**. _(Historique BDD/CI/metrics 13/05 : voir **Validation sécurité/backoffice (12/05)** — retiré de cette intro pour lisibilité.)_
