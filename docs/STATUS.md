...
**Dernière mise à jour** : 12 juin 2026 — **Branche** `refactor/backoffice-filter-facets` (base filtres B10) sur `dev`.
...
- **Validation porteur** : HTTPS Performances validé dans le chat, archivé dans `TODOS_DONE.md` avec Corrélation P1B et les deux validations P1A précédentes.
...
- **Refonte filtres B10 (en cours)** : branche `refactor/backoffice-filter-facets` — hook `useAppliedFilters`, composants `FilterBar` / `FilterSelectField` / `FacetAutocompleteField`, `facetUtils`, client `fetchSecurityLogFacets`, `AutocompleteInput` enrichi (variant `plain`, suggestions libellées). Pages migrées : Logs sécurité, Menaces, **Email Monitor** (filtres appliqués — fin du reload API à chaque caractère de recherche), **Incidents** (gravité/source/recherche), **Statistics log-stats** (période/niveau/service). Validations : Jest ciblé **9/9 OK** (`facetUtils`, `useAppliedFilters`, `incidentFilters`), `./node_modules/.bin/tsc --noEmit` OK, ESLint ciblé **0 erreur** (1 warning historique Email Monitor), `git diff --check` OK ; `npm run type-check` / `npm run lint` sortent encore code 1 sans sortie exploitable. Reste : réseau sécurité, rapports, utilisateurs, services/logs.
...
- **Pilotage validation porteur** : `TODOS_A_VALIDER.md` enrichi avec colonnes `Notes porteur`, `Preuves porteur`, `Décision porteur`, `Notes agent` pour permettre une validation manuelle ligne par ligne. Les validations porteur explicites **mode sombre** et **popup paramètres** sont archivées dans `TODOS_DONE.md`. La prochaine ligne active reste **P1A — Tests offensifs contrôlés par conteneur JobbingTrack**, puis P1A leurres, puis P1B Statistics/performances.
- **Audit final** : ajout d’une ligne P1D `Audit final A-Z avant déploiement / tests globaux finaux` dans `TODOS_A_VALIDER.md` et `docs/TODOS.md`, à lancer seulement en fin de programme pour vérifier actions UI, backend, API, stack, configuration, sécurité, performances et optimisations possibles.
