...
- **Branche B10 suite ouverte** : `fix/security-b10-forensics-score`. Correctif partiel livré : filtres serveur `eventType`/`q`/`order` pour `/api/v1/security/logs`, tri explicite dans l’UI Logs, et fiche menace avec `effectiveRiskScore`/`riskSource` pour éviter un `Risque max 0` trompeur quand seuls les attributs de menace high/critical sont connus. Reste : forensics B8 complet (timeline, endpoints, comptes, logs multi-sources) et politique B12 d’auto-blocage bornée.
