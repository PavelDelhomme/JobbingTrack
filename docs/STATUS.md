...
**Dernière mise à jour** : 13 mai 2026 — **CI/CD corrigé + accès admin restauré** : workflows GitHub migrés hors actions Node 20 dépréciées (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`) avec `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` sur tous les workflows. La validation DB CI reçoit maintenant une `DATABASE_URL` de test au niveau job/workflow ; le workflow dédié applique le schéma avec `prisma db push` au lieu de `migrate deploy` (pas de migrations versionnées dans le dépôt) et `prisma format` n’échoue plus le job pour un écart de format. Validations locales : YAML OK, plus aucune référence actions `@v4` ni `node-version: '20'`, `prisma validate` + `generate` OK, `db push` OK sur Postgres temporaire (**58 tables**). **Accès admin post-tests** : la suite complète avait banni l’IP Docker/proxy `172.19.0.1` ; blocage supprimé et `intrusionDetector` garde le `403` sur requête malveillante sans bannir durablement les IP privées/locales hors production. Le front HTTP `localhost:5003` utilise la gateway HTTP `127.0.0.1:5002` si l’HTTPS dev n’est pas approuvé par Chrome. URL admin publique **`/b4ck0ff1ce`**, ancien `/backoffice` redirige. Historique 12/05 conservé plus bas : sécurité B11/B15, fluidité backoffice, HTTPS dev, supply-chain, scripts, scans prod partiels.
