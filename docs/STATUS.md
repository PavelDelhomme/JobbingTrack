...
**Dernière mise à jour** : 12 mai 2026 — **sécurité exploitable (B11/B15) + fluidité backoffice** : socle alertes email critiques backend livré (`SecurityAlert` `critical/high` → `notification-service` via `X-Internal-Secret`), avec **12/05** une extension disponibilité `SERVICE_DOWN` depuis le metrics-aggregator, dédup anti-flap et services critiques configurables. Réorganisation documentaire `docs/`, matrice **tests sécurité offensifs contrôlés** (`docs/security/SECURITY_TESTING_MATRIX.md`), workflow `.github/workflows/security-audit.yml`, cibles `make security-*` et scripts passifs (`gitleaks`/`truffleHog`, Trivy via CVE scan, ports/nmap, JWT, ZAP actif protégé par opt-in). **12/05** : vues sécurité/backoffice stabilisées contre les erreurs réseau partielles, auto-refresh non concurrent, fiche menace enrichie par corrélations `network_connections`, validation Playwright sécurité ciblée **16/16**, puis audit `.env` outillé (`make env-check`, `make env-validate`) avec secrets dev régénérés localement, commandes WAF (`make waf-enable` / `waf-disable` / `waf-status`) et premiers retraits de fallbacks dangereux (CORS `*`, JWT metrics, password mobile hors `NEXT_PUBLIC_*`). **Suite config 12/05** : `scripts/` inventorié (`make scripts-inventory`, `docs/scripts/SCRIPTS_INVENTORY.md`), scripts env centralisés sous `scripts/env/` avec wrappers historiques, `.env.production.example` séparé, validation prod réelle via `ENV_FILE=.env.production make env-validate-prod`, modèle prod via `make env-validate-prod-example`, CORS gateway aligné sur `ALLOWED_ORIGINS`, prod HTTPS-only, metrics auth activée en dev via proxy serveur `/api/metrics-aggregator` sans exposer la clé, et HTTPS dev opérationnel via `dev-https-proxy` + CA locale installée en NSS (`https://jobbingtrack.localhost:5443`, `https://api.jobbingtrack.localhost:5443`). **Supply-chain 12/05** : alertes Dependabot recoupées avec les lockfiles réels, upgrades npm appliqués sur les surfaces présentes, `frontend` passé à Next 16 pour fermer les advisories runtime, audits `npm audit --omit=dev` verts sur les services principaux, et intégration future Dependabot → table `vulnerabilities` cadrée dans `docs/security/DEPENDABOT_ALERTS_INTEGRATION.md`. **Règle perf** : ne pas inspecter le trafic inter-conteneurs ; runtime léger côté gateway/public, audits lourds hors chemin critique. **Scan prod** : cible compose fusionnée corrigée ; reste durcissement ports exposés. Historique 6–7 mai conservé plus bas : B14 durcissement Compose/runtime, A3 corrélation, logs-watch, rechargements axios, budgets ressources.
...
- [x] **Test automatisé inscription Gmail + log email** : script `tests/run-inscription-gmail-email-check.js` — inscription `redacted@example.invalid` via API puis vérification que l’email de vérification est loggé à la bonne adresse. À lancer avec la gateway + auth-service démarrés : `cd tests && npm run test:inscription-gmail`. E2E Playwright (inscription 3 comptes + Email Monitor) : `frontend/tests/e2e/email-verification-monitor.spec.ts` (nécessite frontend + API + auth admin).
...
Après `make up-full`, tu peux te **connecter** directement au backoffice : **admin@jobbingtrack.test** / **password123**. L’admin est créé ou mis à jour automatiquement avec email vérifié. Si besoin, `make seed-auth` force la création/mise à jour de l’admin avec `emailVerified=true`.
