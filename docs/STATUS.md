...
- **Branche B10 suite ouverte** : `fix/security-b10-forensics-score`. Correctif partiel livré : filtres serveur `eventType`/`q`/`order` pour `/api/v1/security/logs`, tri explicite dans l’UI Logs, bouton `Date` triable avec flèche, autocomplétion catégories/types/recherche, et fiche menace avec `effectiveRiskScore`/`riskSource` pour éviter un `Risque max 0` trompeur quand seuls les attributs de menace high/critical sont connus. Reste : forensics B8 complet (timeline, endpoints, comptes, logs multi-sources), politique B12 d’auto-blocage bornée, et généralisation progressive des filtres/autocomplétions sur les autres listes backoffice.
- **Validation UI B10 logs** : navigateur MCP sur `https://jobbingtrack.localhost:5443/b4ck0ff1ce/security/logs` OK : titre `Logs sécurité`, tri visible `Plus récent d’abord`, champs Catégorie/Type/Recherche en autocomplétion, remplissage `Catégorie=network`, bascule du tri vers `Plus ancien d’abord`, état vide lisible, console récente sans erreur applicative. Spec Playwright ciblée ajoutée : `frontend/tests/e2e/security-logs-filters.spec.ts`. Non exécuté en shell : `npm ci` / `npm --version` sortent code 1 sans sortie dans cette session, donc Playwright/Jest/TS/ESLint complets restent bloqués par la dette P1D/npm locale.
