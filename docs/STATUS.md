...
**Dernière mise à jour** : 13 mai 2026 — **accès admin restauré après tests offensifs** : la suite complète avait déclenché des injections lab depuis l’IP Docker/proxy `172.19.0.1`, créant `blocked_ip:172.19.0.1` dans Redis et pouvant gêner le backoffice local. Correctif : suppression du blocage local, puis `intrusionDetector` garde le `403` sur la requête malveillante mais ne bannit plus durablement les IP privées/locales hors production. Le front HTTP `localhost:5003` utilise désormais la gateway HTTP `127.0.0.1:5002` quand `NEXT_PUBLIC_API_URL` pointe vers l’HTTPS dev non approuvé par Chrome. URL admin publique renommée **`/b4ck0ff1ce`**, ancien `/backoffice` redirige. Validations : login admin `200`, profil `200`, `/b4ck0ff1ce` `200`, `/backoffice` `307`, injection dev `403` sans clé Redis bloquée, HTTPS front/gateway `200` avec CA locale, type-check/lint frontend OK. **Historique 12/05** — sécurité exploitable (B11/B15), fluidité backoffice, HTTPS dev, supply-chain, scripts et scans prod partiels : socle alertes email critiques backend livré (`SecurityAlert` `critical/high` → `notification-service` via `X-Internal-Secret`), extension disponibilité `SERVICE_DOWN`, docs sécurité offensifs contrôlés, workflow `.github/workflows/security-audit.yml`, cibles `make security-*`, audits passifs, outillage `.env`, métriques auth via proxy serveur, et règle perf : runtime léger côté gateway/public, audits lourds hors chemin critique. **Scan prod** : cible compose fusionnée corrigée ; reste durcissement ports exposés. Historique 6–7 mai conservé plus bas : B14 durcissement Compose/runtime, A3 corrélation, logs-watch, rechargements axios, budgets ressources.
