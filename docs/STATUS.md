...
**Audit `.env` / `.env.example` (12/05)** : `.env` et `.env.example` sont alignés en clés (**179/179**) après ajout explicite de `ENABLE_METRICS_AUTH`. Nouveau `make env-validate` : vérifie sans afficher les valeurs les variables critiques, placeholders/secrets faibles, SMTP si alertes activées, exposition de secrets en `NEXT_PUBLIC_*`, et contraintes prod (`ALLOWED_ORIGINS` HTTPS, WAF actif, auth metrics active, bypass dev absent). Résultat local dev : configuration acceptable avec avertissements à traiter ; résultat strict prod : bloque encore sur secrets faibles/placeholders, CORS localhost, `WAF_ENABLED=false`, `ENABLE_METRICS_AUTH` non prod et `DEV_AUTH_BYPASS_TOKEN`.
