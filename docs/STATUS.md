...
| **F** | Tests ciblés + bilan final chantier | **En cours** — Jest `test:unit-and-analytics` : `make test-unit-frontend`, inclus dans **`make test` / `make tests`** via `scripts/run-all-tests-with-reports.sh` (sortie capturée dans `tests/results/.../frontend-jest.json`) ; **ne couvre pas** tout le frontend Jest (`npm test` dans `frontend/` = suite plus large + mobile-emulator, services, etc.). **`npm run test:audit-jest-scope`** : liste explicite des fichiers de test hors gate (audit manuel / CI optionnelle). **F3 (12/05)** : `tests/services/smoke-gateway-services.js` couvre les services exposés via **API Gateway** (**14/14** réel local) ; restent seulement les sondes infra/non-gateway à cadrer si nécessaires, notamment `metrics-aggregator` et `deployment-service`. Voir **`PLAN.md`** § **F3**. | `PLAN.md` § F |
