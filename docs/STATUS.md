...
**Dernière mise à jour** : 20 mai 2026 — **`feat/ui-motor` validée porteur puis merge `dev` prévu** — moteur UI + **Statistics/backoffice** : KPI services unifiés (`ServiceHealthKpiCards`, `summarizeDockerServiceHealth`), légende séries, filtres log-stats, layout dashboard. Validation porteur : section **État du système** cohérente, onglet **Vue d’ensemble** avec mêmes ordres de grandeur, filtres log-stats OK. À planifier ensuite : tests frontend/backend des états conteneurs (**arrêté**, **dégradé**, `unknown` actif), vérification fonctionnelle du **taux d’erreur dans le temps**, et compléments log-stats (période / `log_collector_logs`). Prochaine branche : central logging (`ENABLE_CENTRAL_LOGGING`, `SERVICE_NAME`, `METRICS_SERVICE_URL`) sur les microservices restants du profil `full`. Détail BDD/CI **13/05** : section **Validation sécurité/backoffice (12/05)** ci-dessous. — **Politique env stricte (amorcée)** : sentinelle globale `__JT_ENV_INCOMPLETE__`, module `config/jt-env-policy.cjs`, doc `docs/configuration/STRICT_ENV.md`, validation renforcée dans `scripts/env/env-validate-runtime.cjs`, boot **api-gateway** via `src/bootstrap/strictGatewayEnv.js` et service **api-gateway** dans `docker-compose.yml` sans substitutions silencieuses sur les clés critiques (`:?`, Redis réseau Docker). **Organisation scripts** : helpers bypass tests/WAF regroupés sous **`scripts/env/`** (`dev-test-bypass-fetch.cjs`, `dev-test-bypass-curl.inc.sh`), suppression du dossier **`scripts/dev/`**, index **`scripts/env/README.md`** à jour. Le déploiement env strict sur les autres services / compose reste planifié dans **`docs/TODOS.md`**. _(Historique BDD/CI/metrics 13/05 : voir **Validation sécurité/backoffice (12/05)** — retiré de cette intro pour lisibilité.)_
