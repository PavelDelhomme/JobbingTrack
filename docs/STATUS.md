...
| | |
|---|---|
| **Phase** | **C** — Déploiement VPS |
| **Point** | **DEPLOY-GHA-01** / Étape J — OTA mobile 3 canaux |
| **Branche** | `feat/mobile-ota-three-channels` |
| **APK** | `1.0.42+42` (flavors `dev` / `preprod` / `prod`) |
...
4. Branches : [`development/BRANCHES.md`](development/BRANCHES.md) — **pas de commit direct sur `dev`/`main`**

## Chantier actif — OTA 3 canaux (29/08)

- Flavors Android côte à côte : **JT Dev** / **JT Préprod** / **JobbingTrack**
- OTA `downloadUrl` publié local + préprod + prod
- Install Samsung + Nothing : sideload OK ; **install OTA in-app** à tester porteur après bump `>1.0.42`
...
- **Pilotage** : lignes P1C ajoutées dans `docs/pilotage/TODOS_A_VALIDER.md` (validation porteur locale) et preuves agent dans `docs/pilotage/TODOS_A_VERIFIER.md` ; en attente OK porteur avant archivage `docs/pilotage/TODOS_DONE.md`.
...
- **Symptôme porteur** : pages Performances (Conteneurs vides, Corrélation Axios 404 sur `persistence/system/metrics`) en accès `https://jobbingtrack.localhost:5443`.
- **Cause** : Nginx dev HTTPS envoyait `/api/metrics-aggregator/*` vers api-gateway (bloc `location ^~ /api/`) au lieu du proxy Next App Router.
- **Correctif** : `production/nginx/dev-https/default.conf` — bloc `location ^~ /api/metrics-aggregator/` **avant** `/api/` ; redémarrage `jobbingtrack-dev-https-proxy` requis après modification.
- **Frontend** : module `metricsAggregatorClient.ts` (URLs client unifiées) ; `analytics.service` / `centralMetricsService` alignés ; page Conteneurs : message d’erreur HTTP explicite (hint proxy HTTPS) si la liste échoue.
- **Validation agent** : `curl -sk` `…/api/metrics-aggregator/docker/services/all` et `…/persistence/system/metrics` → **200** sur **5443** ; Jest `metricsAggregatorClient` + `centralMetricsService` **4/4 OK** ; `tsc --noEmit` OK ; doc `docs/operations/DEV_HTTPS.md` enrichie.
- **Validation porteur** : HTTPS Performances validé dans le chat, archivé dans `docs/pilotage/TODOS_DONE.md` avec Corrélation P1B et les deux validations P1A précédentes.
- **Sous-navigation Performances** : l’onglet **Temps de réponse** pointe vers `/b4ck0ff1ce/performances/latency` (plus `#latence`). Vérification navigateur HTTPS : clic sur l’onglet depuis la page Latence reste sur `/performances/latency`.
- **Tooltips graphes** : le thème Recharts partagé ne force plus `itemStyle.color`; les items tooltip reprennent la couleur de leur courbe/barre. Test Jest `rechartsTooltipTheme.test.ts` ajouté.
...
- **Validation porteur B10 sécurité** : `OK Sécurité - titres FR, libellés et navigation` reçu le 12/06 ; ligne P1C archivée dans `docs/pilotage/TODOS_DONE.md` et retirée de `docs/pilotage/TODOS_A_VALIDER.md`.
...
- **Branche B10 suite ouverte** : `fix/security-b10-forensics-score`. Correctif partiel livré : filtres serveur `eventType`/`q`/`order` pour `/api/v1/security/logs`, tri explicite dans l’UI Logs, bouton `Date` triable avec flèche, API `/api/v1/security/logs/facets` pour suggestions issues de la base, filtres Logs appliqués explicitement par bouton (plus de fetch à chaque frappe), et fiche menace avec `effectiveRiskScore`/`riskSource` pour éviter un `Risque max 0` trompeur quand seuls les attributs de menace high/critical sont connus. Reste : forensics B8 complet (timeline, endpoints, comptes, logs multi-sources), politique B12 d’auto-blocage bornée, et refonte transverse des filtres/autocomplétions backoffice.
- **Refonte filtres B10 (mergée `dev`, `809f38c6`)** : patron `useAppliedFilters` + `FilterBar` + helpers (`userFilters`, `serviceLogFilters`, options centralisées). Pages migrées : Logs sécurité, Menaces, Email Monitor, Incidents, log-stats, **Réseau sécurité**, **Utilisateurs**, **Rapports tests**, **Logs services** (`service-logs` + rewrite `/services/logs`). Validations : Jest filtres **15/15 OK**, `./node_modules/.bin/tsc --noEmit` OK. Reste : liste Services (CPU/mémoire), sync URL, Playwright patron partagé.
- **Logs sécurité — liens menace (en cours)** : branche `fix/security-logs-threat-links` — l’API `/api/v1/security/logs` enrichit désormais les lignes avec `correlatedThreatId`/`linkReason` ; l’UI affiche **Menace liée**, **Menace corrélée** ou une raison explicite. Les logs lab sans `threatId` restent à rattacher côté seed.
- **Validation UI B10 logs** : 6 logs lab générés via `POST http://localhost:5017/api/v1/security/logs` (`auth`, `waf`, `firewall`, `intrusion`, `network`, `ddos`) pour tester les suggestions. API : `/security/logs?startDate=2026-06-01...` retourne `count=6,total=6`; `/security/logs/facets` retourne les catégories/types/IP calculés depuis la base. Navigateur MCP sur `https://jobbingtrack.localhost:5443/b4ck0ff1ce/security/logs` OK : logs visibles, remplissage `Catégorie=network` sans rechargement immédiat du tableau, clic **Appliquer les filtres** puis badge `catégorie network` et ligne filtrée “Menace réseau”. `createSecurityLog` corrigé : `requestId` reste dans `metadata` car le modèle Prisma `SecurityLog` n’a pas de champ top-level `requestId`. Non exécuté en shell complet : `npm run type-check` / `npm run lint` sortent encore code 1 sans sortie ; Playwright ciblé bloqué par serveur Next déjà actif sur le port configuré.
- **B8 forensics partiel** : `firewallController` corrèle maintenant aussi les `intrusionAttempt` et `dDoSAttack` dans le contexte applicatif d’une fiche menace (endpoints, méthodes, score, source du score). Après `docker compose restart security-service`, la menace lab `cmpbtgo9v0002lwlv74po2fh6` affiche `Risque retenu 75` avec explication depuis la sévérité, plus `0`. Pour cette menace, endpoints/comptes restent non corrélés car aucun log sécurité/app/auth lié n’est présent : suite B8 = relier vraiment gateway/auth/app logs + comptes de test.
- **Cadrage futur Lot M** : demande porteur notée dans `docs/TODOS.md` pour des mini-modèles prédictifs locaux et sobres après B10/B8/B11 immédiats : sécurité/logs/anomalies, corrélation incidents, extraction entreprise/contact/prénom/poste/ville depuis emails/offres, autocomplétion et tri agent email. Règle : commencer déterministe, modèles minuscules explicables seulement si utiles, pas de cloud, pas de secrets/données personnelles brutes, feature flags et fallback sans modèle.
...
- **Apps Script v4 récupéré** : reprise depuis `docs/email-triage-agent-roadmap` de `docs/emails/triage_email_agent.gs` v4 : boutons d’action dans le mail (`FAIT`, `PAS PERTINENT`, `REPORTER +3j`), Web App `doGet`, recherche d’offres Rennes via flux publics France Travail / Indeed / Hellowork / Meteojob et section offres fraîches dans le récap. Durcissement local : email personnel/PIN fixe retirés du script, configuration via propriétés Apps Script (`TRIAGE_AGENT_EMAIL`, `TRIAGE_DIGEST_RECIPIENT`, `JBT_AGENT_PIN`). README d’installation v4 mis à jour.
- **Tests** : socle `tests/email-triage/` étendu (classification, Calendar, digest programmé/identité, permissions `JOB_SEARCH_AGENT_ENABLED`, rendu digest HTML/texte, politique connexion Gmail/IMAP) ; suite **8 fichiers / 45 passed, 1 skipped** ; dernier rapport via `run-with-report.sh`.
...
- **Suite CLI globale** : `scripts/run-all-tests-with-reports.sh` inclut la section **34b Suite Agent email / triage** (8 fichiers Jest : classification, Calendar, digest, permissions, rendu, connexion boîte).
...
- **P1A leurres VPS-Portainer** : relecture de design sans action offensive ni déploiement. Position confirmée : réduction d’exposition d’abord (ports publics minimaux, NPM réseau partagé limité, Portainer/NPM admin restreints), leurres/honeypot seulement en préprod isolée après validation explicite.
- **P1B corrélation performances** : ajout du script de validation `scripts/ops/run-performance-correlation-validation-with-report.sh`. Rapport `tests/results/performance-correlation/20260612-015102` : **5/5 étapes OK**, central logging compose **15 services**, runtime **15/15**, `aggregated_logs` **27604** lignes / **18** services distincts, endpoints metrics OK, Jest modèle corrélation **5/5 OK**, page corrélation HTTP **200** et rapport listé/ouvert dans `/api/test-reports`. Email récap P1B envoyé vers `pauldelhomme.pro@gmail.com`, `dev@delhomme.ovh`, `test@delhomme.ovh`, `EmailLog` **SENT** x3.
...
- **P1B temps de réponse endpoints** : `metrics-aggregator` expose `responseTime.per_service` (23 services) ; script `scripts/ops/run-statistics-endpoint-latency-validation-with-report.sh` ; rapport `tests/results/statistics-endpoint-latency/20260612-134940` **3/3 OK** ; 7 services prioritaires mesurés ou état explicite (postgres = Santé Docker) ; pages Performances/latency/statistics HTTP **200**.
- **P1C mode sombre + popup paramètres** : tests Jest `theme.test.ts`, `apply.test.ts` (**9/9 OK**) et `SettingsPopup.test.tsx` (**5/5 OK**) ; script `scripts/ops/run-p1c-ux-theme-settings-validation-with-report.sh` ; rapport `tests/results/p1c-ux-theme-settings/20260612-135642` **4/4 OK** ; `setStoredTheme` synchronise les 3 clés localStorage ; script init thème dans `layout.tsx` ; rapport listé `p1c-ux-theme-settings-20260612-135642`. Email récap P1C envoyé, HTTP **202** x3, `EmailLog` **SENT** x3.
- **Pilotage validation porteur** : `docs/pilotage/TODOS_A_VALIDER.md` enrichi avec colonnes `Notes porteur`, `Preuves porteur`, `Décision porteur`, `Notes agent` pour permettre une validation manuelle ligne par ligne. Les validations porteur explicites **mode sombre** et **popup paramètres** sont archivées dans `docs/pilotage/TODOS_DONE.md`. La prochaine ligne active reste **P1A — Tests offensifs contrôlés par conteneur JobbingTrack**, puis P1A leurres, puis P1B Statistics/performances.
- **Audit final** : ajout d’une ligne P1D `Audit final A-Z avant déploiement / tests globaux finaux` dans `docs/pilotage/TODOS_A_VALIDER.md` et `docs/TODOS.md`, à lancer seulement en fin de programme pour vérifier actions UI, backend, API, stack, configuration, sécurité, performances et optimisations possibles.
- **Backoffice metrics / Performances** : correction du client live `centralMetricsService` pour utiliser le proxy Next `/api/metrics-aggregator` côté navigateur, comme `analyticsService`, au lieu d’appels directs `localhost:5004` susceptibles de casser l’UI selon auth/CORS/clé API. Mapping historique `services` corrigé (`item.services || item.containers`). Validations 12/06 14h35 : Jest ciblé **5 suites / 22 tests OK** ; `npm run type-check` OK ; `npm run lint` OK (**0 erreur**, warnings historiques) ; rapport `tests/results/statistics-endpoint-latency/20260612-143538` **3/3 OK** ; navigateur authentifié `/performances`, `/performances/latency`, `/performances/containers` : requêtes `/api/metrics-aggregator` HTTP **200**, graphes visibles, conteneurs **23** / **199** points.
- **Backoffice frontend — smoke interfaces élargi** : relance Playwright sur `statistics-smoke`, `performances-range-smoke`, `backoffice`, `backoffice-extended`, `backoffice-interactions`. Premier run : **150/151 OK**, seul échec = faux positif strict-mode dans `backoffice-interactions.spec.ts` (heading Statistics trop large, 2 éléments trouvés). Correctif test : ciblage du `h1` “Statistiques & Monitoring”. Validation finale 12/06 14h45 : **151/151 OK** ; `npm run type-check` OK ; `npm run lint` OK (**0 erreur**, warnings historiques 1458) ; diagnostics IDE OK.
- **Performances — graphes réels et plages** : retour porteur « aucun graphe » investigué. Les endpoints metrics-aggregator renvoyaient bien des points ; problèmes UI = échelle Y CPU conteneurs écrasée par pics >100 % (`frontend` ~199 %), Corrélation avec graphes sous la liste services sur écrans non XL, smoke Playwright insuffisant. Correctifs : domaine Y adaptatif dans `AnalyticsContainersChartsBundle`, réordonnancement Corrélation, sous-nav Performances sur Latence, spec `performances-range-smoke` renforcée (SVG avec courbes, Synthèse + Corrélation, plages `7d`/`168`). Validation 12/06 15h30 : `npm run type-check` OK ; Playwright performances-range **7/7 OK**.
...
| `ac057bf5` | pilotage relance validation | `docs/pilotage/TODOS_A_VALIDER.md`, `docs/pilotage/TODOS_A_VERIFIER.md` |
...
- **Archivage pilotage** : ligne P1A alertes email retirée de `docs/pilotage/TODOS_A_VALIDER.md`, ajoutée à `docs/pilotage/TODOS_DONE.md` (commits `800e9d08`, `b6ff92d0`).
...
- **Validations porteur P0** : comparaison de rapports sécurité CVE, menaces historiques/lab avant nettoyage, et ouverture/téléchargement rapports sécurité validés explicitement puis archivés dans `docs/pilotage/TODOS_DONE.md`.
...
- **Pilotage** : entrée ajoutée dans `docs/TODOS.md`, `docs/BACKLOG.md`, `docs/PLAN.md` (lot I) et `docs/pilotage/TODOS_A_VALIDER.md` en P2 futur. L’implémentation n’est pas démarrée tant que la première ligne P0 de `docs/pilotage/TODOS_A_VALIDER.md` reste ouverte.
...
- **Matrice sécurité** : `docs/security/SECURITY_TESTING_MATRIX.md` détaille désormais outils, protections attendues et preuves pour ces menaces ; `ROADMAP_SECURITE_API_ET_BACKOFFICE.md`, `docs/TODOS.md` et `docs/pilotage/TODOS_A_VALIDER.md` renvoient vers ce cadrage.
...
- **Pilotage bloquant porteur** : nouveau flux racine `docs/pilotage/PILOTAGE.md` → `docs/pilotage/TODOS_A_VALIDER.md` → `docs/pilotage/TODOS_A_VERIFIER.md` → `docs/TODOS.md`; validations acceptées archivées dans `docs/pilotage/TODOS_DONE.md`; gates préprod/prod séparés dans `docs/production/A_VALIDER_AVANT_PRODUCTION.md`, `docs/production/DEPLOIEMENT_PRODUCTION.md`, `docs/production/VALIDATION_PRODUCTION.md`; règle Cursor `.cursor/rules/pilotage-validation.mdc` ajoutée. **21/05 suite** : `docs/pilotage/TODOS_A_VALIDER.md` enrichi (échelle P0/P1/P2, file complète backoffice/statistics/performances/sécurité) ; régression porteur **comparaison rapports CVE** → correctif `resolveReport.ts` + route compare + affichage sécurité dédié (surfaces, Critical/High/Medium/Low/Info, sans notes/payloads bruts).
...
- **CI doc** : run GitHub `26202796200` marqué succès dans `TODOS.md` et `docs/pilotage/TODOS_A_VALIDER.md` (commit `058e3ca0`).
...
- **CI frontend / Jest coverage** : après déblocage Prettier, le run GitHub échouait plus loin sur `Tests Frontend` : `babel-plugin-istanbul` / `test-exclude` cassait avec `glob@13` (API non compatible avec `promisify(require("glob"))`). Correctif : retrait de l’override global `glob` dans `frontend/package.json` et régénération de `package-lock.json` pour restaurer les versions attendues par Jest/Istanbul. Ensuite, les tests passaient mais échouaient sur le seuil global coverage **70%** alors que le coverage réel est très inférieur ; retrait du seuil global irréaliste dans `jest.config.js`, la génération du rapport restant active et vérifiée par l’étape CI dédiée. Validation directe : `npm run test:ci -- --runInBand` → **24 suites / 118 tests OK**.
- **CI frontend / fuseau horaire Jest** : le run GitHub suivant a révélé deux tests dépendants du fuseau local (`date-metrics-display`, `analytics-metric-rows-normalize`) : GitHub affichait UTC au lieu de Paris. Correctif : les tests fixent `NEXT_PUBLIC_CHART_TIMEZONE=Europe/Paris`, qui est le contrat supporté par `displayTimeZoneOptions`. Validation directe répétée : `npm run test:ci -- --runInBand` → **24 suites / 118 tests OK**.
- **CI backend / security-service smoke** : après le passage frontend, le job `Tests Backend` échouait sur `tests/backend/test-security-service.test.js` par timeout du `beforeAll` (Jest 10s) alors que le helper `waitForApiGateway` peut attendre 45s. Correctif : attendre la gateway avant le login admin et porter le timeout du hook à 70s pour refléter l’attente réelle CI. Le test accepte aussi explicitement `401` quand l’admin JWT n’est pas disponible, ainsi que l’erreur réseau sans `response` quand la gateway n’est pas lancée dans le job backend générique. Validation directe : `npm run test:backend -- --runTestsByPath backend/test-security-service.test.js --runInBand` depuis `tests/` → **12/12 OK**.
- **CI backend / DB migrations smoke** : le job backend suivant échouait dans `tests/database/test-migrations.js` car le script appelait `/usr/bin/npx`, chemin absent sur le runner GitHub. Correctif : résolution portable via `npx` / `npx.cmd` dans le `PATH`.
...
- **Validation porteur précédente intégrée** : `finish-feat/ui-motor` est fusionnée dans `dev` ; section **État du système** cohérente, onglet **Vue d’ensemble** dans les mêmes ordres de grandeur, filtres `log-stats` OK.
...
- **Graphes multi-séries (20/05 suite)** : besoin porteur acté : les graphes qui affichent plusieurs services/conteneurs doivent garantir des couleurs uniques et stables. Socle `seriesColors` ajouté côté frontend et branché sur Performances → Conteneurs en mode “Tous les conteneurs” ; extension restante aux autres graphes Statistics/Analytics/Services.
- **Performances — Disque (20/05 suite)** : page `/backoffice/performances/disk` reprise pour sortir du stub 24h fixe : retour + sous-navigation + période partagée, cartes alignées, courbes usage disque/volume, et Block I/O agrégé lecture/écriture + débit estimé Mo/min depuis les historiques conteneurs.
...
- **Documents porteur / projet** : ajout initial de `docs/security/AUDIT_SEC_PROJECT.md`, registre porteur, `docs/development/BRANCHES.md` et `monitoring/RAPPORT_MONITORING_GOOD_PRACTICE_GO_AND_C.md`. **21/05** : le registre porteur est restructuré en flux `docs/pilotage/PILOTAGE.md`, `docs/pilotage/TODOS_A_VALIDER.md`, `docs/pilotage/TODOS_A_VERIFIER.md`, `docs/pilotage/TODOS_DONE.md`, puis gates préprod/prod dédiés.
...
- [x] **Page tests-emails supprimée** (`/backoffice/tests-emails`) : accès remplacés par **Emails → Configuration**, **Emails → Déliverabilité**, **Email Monitor** et lien MailHog.
