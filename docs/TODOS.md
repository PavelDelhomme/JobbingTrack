...
- [ ] **Discipline tests & rapports au fil du dev** : à chaque lot, continuer les tests ciblés (type-check, Jest ciblé, scripts smoke API/UI selon surface) et noter les commandes dans `STATUS.md`. Le porteur lancera ensuite la suite complète avec rapports via `scripts/run-all-tests-with-reports.sh` / hub Backoffice Tests ; après génération, analyser `tests/results/<timestamp>/summary.json` + `report.html` et reporter les vrais échecs dans `TODOS.md` / `A_VALIDER_VERIFIER.md`.
