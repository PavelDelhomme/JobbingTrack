...
**Dernière mise à jour : 21 mai 2026** — les tâches à faire passent en premier. Les succès techniques restent conservés plus bas, mais ils ne valent pas validation porteur. La validation finale est centralisée dans **`A_VALIDER_VERIFIER.md`**.

## Pilotage (comment utiliser ce fichier)

- **« Priorités à faire maintenant »** = ordre conseillé pour le chantier global ; **une puce peut couvrir plusieurs PR ou sessions** (ne pas viser tout cocher en un seul merge).
...
- [x] **Mode clair backoffice — lisibilité globale** : **20/05 partiel** : surfaces/cartes/champs renforcés, puis familles colorées (`blue`, `green/emerald`, `amber/orange/yellow`, `red/rose`, `violet/purple/indigo`, `sky/cyan`) rendues plus visibles en clair. **21/05** : validation porteur provisoire acceptée pour ne plus bloquer le lot apparence. Les cas résiduels restent suivis dans **“UX unifiée Statistics / Performances / Vue d’ensemble”** et dans `A_VALIDER_VERIFIER.md` si une page précise redevient illisible.
...
- [x] **`AUDIT_SEC_PROJECT.md`** rangé sous **`docs/security/`** ; liens **`docs/README.md`**, **`A_VALIDER_VERIFIER.md`**, index **`docs/security/README.md`**.
