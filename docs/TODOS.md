...
- [ ] **Scripts — restructuration sûre par lots** : `make scripts-inventory` reste utilisable et signale encore **18 scripts racine** et **26 sans référence automatique** après déplacements sûrs (`setup/setup-ports.sh`, `db/create-prisma-tables-safe.sh`, `reports/show-mobile-report.sh`, `reports/clean-all-reports-docker.sh`). Prochain lot recommandé : continuer sur les scripts racine à cible évidente (`reports/monitoring`, `ops`, `testing`) en mettant à jour Make/CI/docs, puis auditer les `non-reference` avant suppression.
