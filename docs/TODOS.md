...
- [ ] **Statistics — onglet Données applicatives (`app-data`)** : **20/05 cadrage** : page centrée sur **totaux** + **timeline globale**. **20/05 suite** : détails ajoutés côté API/UI pour actifs vs total, nouveaux sur période, candidatures par statut, utilisateurs par rôle, entretiens/relances par statut, appels, événements et états vides par source. **Reste** : valider en navigateur, puis décider si la timeline doit être persistée historiquement (aujourd’hui snapshot courant enrichi).
