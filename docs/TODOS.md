...
**Dernière mise à jour : 13 mai 2026** — **CI/CD Node 24 + validation DB** : workflows GitHub alignés sur actions Node 24 (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`) avec `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` partout. La validation DB reçoit une `DATABASE_URL` de test au niveau job/workflow, le workflow dédié utilise `prisma db push` (pas `migrate deploy`, car les migrations versionnées ne sont pas présentes) et le format Prisma n’échoue plus le job pour un simple écart de format. **Suite tests complète stabilisée** : équivalent direct de `make tests` exécuté sans `make` via `bash scripts/run-all-tests-with-reports.sh` avec gateway locale, Playwright smoke et `PERF_LIGHT=1` ; résultat **436/436 réussis**, **1 ignoré**, rapport local `tests/results/20260513-002408/`. Correctifs associés : chargement `.env` sûr dans le runner, `exit 1` si échec réel, secrets seed masqués, contrats admin `TEST_ADMIN_*` alignés sur `ADMIN_*`, auth JWT pour les tests firewall/WAF, routes status applicatives replacées avant `/:id`, enrichissement du statut candidature, et fix SQL `Application.isTestData`. **Correctif post-run 13/05** : l’IP proxy Docker locale `172.19.0.1`, bloquée par les tests d’injection, a été débloquée ; en dev, les intrusions critiques continuent à renvoyer `403` mais ne bannissent plus durablement les IP privées/locales. Le front HTTP `localhost:5003` retombe sur la gateway HTTP `127.0.0.1:5002` si l’API HTTPS dev n’est pas approuvée par Chrome. URL admin publique renommée en **`/b4ck0ff1ce`** ; ancien `/backoffice` redirige. **Suite F3 infra 13/05** : smokes directs ajoutés pour `metrics-aggregator` et `deployment-service`, branchés automatiquement par `scripts/run-all-tests-with-reports.sh`; validations locales réelles OK (`metrics` health/auth/docker services, `deployment` health/detailed/list). **Historique 12/05** — sécurité B11/B15, docs, HTTPS dev, supply-chain, scripts, scans prod partiels.
