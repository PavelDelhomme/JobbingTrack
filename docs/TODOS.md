...
**Dernière mise à jour : 13 mai 2026** — suivi restructuré en succès courts et tâches restantes. Les validations techniques sont notées dans les listes ci-dessous ; les validations manuelles du porteur restent en cases `[ ]` tant qu'elles n'ont pas été confirmées.

## Succès récents — 13 mai 2026
...
- [x] **CI `npm ci` backend réparé** : `backend/package-lock.json` resynchronisé avec `backend/package.json` pour inclure les dépendances ESLint manquantes (`eslint-plugin-import`, `eslint-plugin-n`, `eslint-plugin-promise` et transitives) ; validation locale `/usr/bin/npm ci --dry-run` OK. Le workflow `ci-cd.yml` n'avale plus les erreurs critiques lint/type-check/tests/build via `|| echo` et remplace les appels Make backend/système par les scripts npm/directs disponibles.
...
- [x] **Monitoring Rust restructuré** : les crates `monitoring-agent`, `log-collector` et `metrics-aggregator` ont des `main.rs` minimalistes et des modules dédiés (`config`, `constants`, `http`, `metrics`, `storage`, etc.) ; `cargo fmt --all` et `cargo check --workspace` OK.
...
- [ ] **Sécurité — détails forensics menaces avant CVE** : prioritaire sur la suite CVE. Pour chaque menace détectée (DDoS, WAF, intrusion, firewall), afficher IP source/destination, géolocalisation, VPN/proxy/ASN quand un provider threat-intel est branché, route/flux réseau, requêtes et payloads dangereux, logs applicatifs corrélés, services et comptes impactés. **11/05 partiel** : page menaces responsive + auto-refresh silencieux + fiche menace enrichie avec GeoIP, logs sécurité 24h, intrusions/DDoS liés, ports/protocoles/services et télémétrie manquante explicite. **12/05 suite** : fallback sur `network_connections` quand `network_threats.destIp` / `metadata.connectionDetails` sont vides ; corrélation logs via `metadata.sourceIp` / `metadata.threatId` en plus de `sourceIP`; cartes Analyse cliquables vers menaces filtrées ; auto-refresh protégé contre les requêtes concurrentes ; vue réseau avec IPs surveillées actionnables. **Reste** : provider IP intelligence (ASN/VPN/proxy/Tor), conservation payloads dangereux/request samples, tests WAF on/off en dev/prod-like.
