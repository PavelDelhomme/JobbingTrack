...
- [ ] **Scripts — restructuration sûre par lots** : `make scripts-inventory` reste utilisable mais signale encore **20 scripts racine** et **28 sans référence automatique** après déplacement de logique env (`sync-env`, `verify-env-usage`, `generate-env-example`) sous `scripts/env/` avec wrappers. Prochain lot recommandé : déplacer d'abord les scripts racine à cible évidente (`db`, `ops`, `reports/monitoring`, `testing`) avec wrappers temporaires, puis auditer les `non-reference` avant suppression.
