...
- [ ] **Menaces historiques/lab/IP privées — tri avant nettoyage** : la page **Menaces** peut encore afficher des données historiques de lab (`10.0.0.99/100/101/102`, `198.51.100.42`) et d’anciennes anomalies Docker (`172.19.x`, `172.20.x`) même après correction des faux positifs runtime. Ne pas supprimer directement : ajouter un dry-run d’inventaire `network_threats` (source lab/test, IP privée, sévérité, statut bloqué/non bloqué, âge), marquer ce qui relève du lab ou du bruit contrôlé, puis purger seulement après validation porteur/sauvegarde.
