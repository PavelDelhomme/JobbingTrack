...
**Dernière mise à jour : 20 mai 2026** — les tâches à faire passent en premier. Les succès techniques restent conservés plus bas, mais ils ne valent pas validation porteur. La validation finale est centralisée dans **`A_VALIDER_VERIFIER.md`**.
...
- [ ] **Stockage emails/logs/métriques — compression sans perte** : définir une stratégie globale pour limiter l’espace disque sans perdre les données : conserver les enregistrements actifs en base, compresser/archiver les anciens contenus lourds (corps emails `EmailLog.emailContent`, logs agrégés, métriques historiques, rapports tests), garder index/metadata requêtables, restaurer à la demande, et documenter les délais de rétention par famille. Priorité : mesurer par table avant toute purge ; actions destructives interdites sans validation porteur.
