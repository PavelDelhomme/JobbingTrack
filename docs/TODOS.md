...
- [x] **Validation BDD complète post-erreur `Application.isTestData` (13/05)** : `make db-push-all`, `DATABASE_URL=... npm run test:database`, `make test-database`, `prisma validate/generate`, checks SQL colonnes critiques, et Jest ciblé `api/test-archive-trash.test.js` OK (**21/21**). `test-postgresql-config.js` et `test-migrations.js` ont été réalignés sur le flux actuel `db push` + correctifs SQL `db-push-all`; `make test-database` lance maintenant les tests en local pour accéder aux fichiers racine et échoue vraiment si la suite DB échoue.
