...
- [ ] **Exécuter les scans P0 en environnement autorisé** : `gitleaks` historique complet, `trivy` images prod, `nmap` cible préprod/prod autorisée, `jwt_tool` avec token lab, ZAP active scan local via gateway (`SECURITY_ACTIVE_SCAN=1` uniquement).
- [ ] **Traiter les rapports sécurité** : classer findings `critical/high`, ouvrir tâches ciblées, justifier faux positifs avec date/environnement/commit, et reporter les résultats synthétiques dans `STATUS.md` / `security/STATS.md`. **Premier finding local** : scan ports compose prod fusionné OK mais trop de ports internes sont publiés sur `0.0.0.0` (backend services, Postgres, Redis, MailHog, collecteurs) ; à durcir avant prod.
