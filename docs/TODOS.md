...
**Dernière mise à jour : 12 mai 2026** — **Sécurité B11/B15 + documentation + fluidité backoffice** : alertes email critiques branchées côté backend (`security-service` → `notification-service`), réorganisation `docs/`, matrice **tests sécurité offensifs contrôlés** (`docs/security/SECURITY_TESTING_MATRIX.md`), outillage passif initial (`make security-audit`, `security-scan-secrets`, `security-scan-images`, `security-scan-ports`, `security-scan-jwt`, `security-zap-active`, `security-report`) et workflow `.github/workflows/security-audit.yml`. **Règle perf confirmée** : ne pas analyser tout le trafic inter-conteneurs ; runtime léger uniquement sur l’entrée gateway/public. **12/05** : stabilisation UI sécurité contre `Failed to fetch`, auto-refresh non concurrent, fiche menace plus robuste, validation applicative ciblée Playwright sécurité **16/16**, puis audit `.env` / `.env.example` avec `make env-validate`, secrets dev régénérables et premiers retraits de fallbacks dangereux (CORS `*`, JWT metrics, password mobile hors `NEXT_PUBLIC_*`). **Suite config 12/05** : inventaire `scripts/` automatisé (`make scripts-inventory`), scripts env centralisés sous `scripts/env/`, `.env.production.example` séparé, `make env-validate-prod` dédié à `ENV_FILE=.env.production`, `make env-validate-prod-example`, `ALLOWED_ORIGINS` HTTPS-only en prod, metrics auth activée en dev via proxy serveur `/api/metrics-aggregator` sans exposer `METRICS_API_KEY`, et HTTPS dev complet via `make dev-https-install-ca` / `make dev-https-up` (`https://jobbingtrack.localhost:5443`). **Supply-chain 12/05** : première passe Dependabot réelle sur les lockfiles présents, upgrades `next`/`axios`/`nodemailer`/`express`/`jsonwebtoken`/`postcss`/`jspdf`, audits runtime npm verts sur les surfaces principales, et cadrage `Dependabot alerts → vulnerabilities` dans `docs/security/DEPENDABOT_ALERTS_INTEGRATION.md`. **Scan prod** : cible compose fusionnée corrigée ; reste durcissement ports exposés. Les notes **6–7 mai** restent conservées plus bas comme priorité précédente et historique technique.
...
- [x] F3 — **Couverture `tests/services/` exposés gateway** : ajout de `tests/services/smoke-gateway-services.js` (health gateway/auth + routes typiques applications, companies, contacts, interviews, calls, events, followups, notifications, dashboard, profile, workflows, security) et branchement dans `run-all-tests-with-reports.sh`. Validation réelle 12/05 : `API_GATEWAY_URL=http://127.0.0.1:5002 /usr/bin/node tests/services/smoke-gateway-services.js` = **14/14 succès**. Reste hors ce smoke : services non exposés gateway ou sondes infra directes documentées, notamment metrics-aggregator / deployment-service si besoin.
