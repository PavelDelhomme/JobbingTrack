...
- [ ] **Corrélation performances — 0 logs / signaux (suite technique)** : propager **`ENABLE_CENTRAL_LOGGING` + `METRICS_SERVICE_URL` + `SERVICE_NAME=jobbingtrack-*`** sur **tous** les microservices du profil `full`. **20/05 — branche `feat/central-logging-full`** : premier lot Compose sur application/company (normalisation nom) + contact/interview/call/event/followup/profile/notification/dashboard/workflow/deployment ; garde-fou `scripts/ops/validate-central-logging-compose.cjs` OK (15 services). Reste : validation runtime `aggregated_logs` après recreate et vérification services sans transport central réel.
...
- [ ] **Statistics — onglet Logs (`/backoffice/statistics/log-stats`)** : filtres service/niveau ajoutés et validés porteur **20/05** ; reste à compléter plus tard avec **période**, `log_collector_logs` / **log-collector-rs**, routes metrics-aggregator, cohérence avec **`/backoffice/services/logs`** et central logging (`ENABLE_CENTRAL_LOGGING`, `SERVICE_NAME`).
