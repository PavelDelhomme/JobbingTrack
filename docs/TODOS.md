...
- [ ] **Scripts — restructuration sûre par lots** : `make scripts-inventory` reste utilisable et signale encore **9 scripts racine** et **24 sans référence automatique** après déplacements sûrs vers `scripts/env/`, `scripts/setup/`, `scripts/db/`, `scripts/reports/`, `scripts/performance/` et `scripts/ops/`. Make dry-run validés : `setup-ports`, `test-mobile-report`, `status-watch`, `logs-watch`, `menu`. Prochain lot recommandé : traiter les gros points d'entrée de tests (`generate-test-report`, `run-all-tests-with-reports`, `test-api-specific`, `verify-user-journey`) avec migration prudente des routes frontend/API et rapports historiques, puis auditer les `non-reference` avant suppression.
