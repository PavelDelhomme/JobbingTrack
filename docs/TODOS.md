...
- [x] **CI sécurité GitHub — correction Trivy/Node 24** : `security-audit.yml` ne référence plus `aquasecurity/trivy-action@0.28.0` introuvable ; passage à `aquasecurity/trivy-action@v0.36.0`, actions GitHub officielles Node 24 (`checkout` v6, `setup-node` v6, `upload-artifact` v7), `gitleaks-action@v2.3.9` remplacé par installation du binaire Gitleaks + `scripts/security/secrets-scan.sh`, et upload SARIF Trivy migré `github/codeql-action/upload-sarif@v4`. **Reste validation GitHub** : observer le prochain `Security Audit` et vérifier disparition des warnings Node 20 / CodeQL v3.
