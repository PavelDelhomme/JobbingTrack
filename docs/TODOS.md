...
- [ ] **CI GitHub #556 — Prettier frontend / tests frontend** : le job « Vérification du formatage » échouait sur des fichiers frontend non formatés. **21/05 correctif local** : ajout de `frontend/.prettierignore` pour exclure les artefacts générés (`performance-reports`, `tests/results`, `.next-local`, etc.) puis `npm run format` et `npm run format:check` OK depuis `frontend/`. **Observation GitHub** : l’étape Prettier passe ; les blocages suivants étaient `Tests Frontend` (`test-exclude` cassé par override `glob@13`, seuil global coverage 70% irréaliste, puis tests timezone dépendants de l’hôte CI UTC). Correctif local : retrait override `glob`, lockfile régénéré, retrait seuil global Jest, tests timezone fixés via `NEXT_PUBLIC_CHART_TIMEZONE=Europe/Paris` ; `npm run test:ci -- --runInBand` → **24 suites / 118 tests OK**. **Reste** : push correctif timezone et observer la fin du run.
