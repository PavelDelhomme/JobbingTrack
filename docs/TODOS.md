...
**Dernière mise à jour : 11 mai 2026** — **Sécurité B11/B15 + documentation** : alertes email critiques branchées côté backend (`security-service` → `notification-service`), réorganisation `docs/`, matrice **tests sécurité offensifs contrôlés** (`docs/security/SECURITY_TESTING_MATRIX.md`), outillage passif initial (`make security-audit`, `security-scan-secrets`, `security-scan-images`, `security-scan-ports`, `security-scan-jwt`, `security-zap-active`, `security-report`) et workflow `.github/workflows/security-audit.yml`. **Règle perf confirmée** : ne pas analyser tout le trafic inter-conteneurs ; runtime léger uniquement sur l’entrée gateway/public. **Point bloquant noté** : `docker-compose.prod.yml` ne se résout pas seul pour le scan ports/images prod (`auth-service` sans `image`/`build` dans le fichier prod seul). Les notes **6–7 mai** restent conservées plus bas comme priorité précédente et historique technique.
