...
- [ ] **Statistics — onglet Données applicatives (`app-data`)** : **20/05 cadrage** : dans le lot actuel, exposer seulement les **totaux** + une **timeline globale** propre. **20/05 constat porteur** : page encore trop pauvre / parfois vide côté ressenti produit. **20/05 UI** : alignée sur le gabarit Statistics commun + état “à améliorer” explicite. **Lot suivant** : détailler actifs vs total, nouveaux sur période, candidatures par statut, entreprises, contacts, relances, entretiens, appels, événements, et états vides par source.
