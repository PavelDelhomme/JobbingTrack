...
|------|--------|---------------------------|
| **CI / formatage frontend** | Correctif local appliqué : `frontend/.prettierignore` + `npm run format` + `npm run format:check` OK. | Commit/push, puis observer le prochain workflow GitHub #556. |
...
- [ ] **CI GitHub #556 — Prettier frontend** : le job « Vérification du formatage » échouait sur des fichiers frontend non formatés. **21/05 correctif local** : ajout de `frontend/.prettierignore` pour exclure les artefacts générés (`performance-reports`, `tests/results`, `.next-local`, etc.) puis `npm run format` et `npm run format:check` OK depuis `frontend/`. **Reste** : commit/push, puis observer le prochain run GitHub pour confirmer le déblocage des jobs backend/frontend/intégration/sécurité.
