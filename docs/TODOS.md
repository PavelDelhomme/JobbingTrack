...
- [ ] **Logs sécurité — compression & rétention** : **21/05 cadrage** dans `docs/security/SECURITY_LOGS_RETENTION.md` : classes de rétention par gravité, archive JSONL gzip sans perte, manifest SHA-256, dry-run obligatoire, restauration temporaire et purge après validation. **21/05 dry-run** : `scripts/security/security-logs-retention-dry-run.cjs` mesure table/index et candidats archive par classe sans suppression ; validation locale : **42 MB**, **42 344** logs, candidats `standard=228`, `noise=9654`, `critical/high=0`. **18/05 (partiel UI)** : `/security/logs` paginé côté API (`offset`/`limit`, `pagination.total`) + composant `Pagination` + `?page=` (`useUrlPagination`) ; refresh 15 s **sans** reset de page ; vue d’ensemble sécurité signale déjà le plafond `SECURITY_LOGS_FETCH_LIMIT=2000`. **Reste** : implémenter archive JSONL gzip/restauration/purge validée + indicateurs volume/rétention backoffice.
