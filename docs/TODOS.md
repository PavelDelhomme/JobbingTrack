...
- [ ] **Forensics menaces — preuves exploitables** : collecter et afficher des preuves techniques utiles par menace (`requestId`, endpoint, IP, payload/request redige si sensible, service touche, compte impacte si fiable, logs correles, decision WAF/firewall, export horodate). L'enrichissement reseau ASN/VPN/proxy/Tor reste un contexte optionnel et ne doit jamais etre presente comme preuve d'attaque.
...
- [ ] **Tests métriques conteneurs (à planifier)** : ajouter tests frontend + backend sur les cas **running healthy**, **running degraded/unknown**, **stopped/exited**, doublons `rawName`, et vérifier que `unknown` actif ne retombe pas en faux **offline**. Cible : `serviceHealthOverview.ts`, routes `metrics-aggregator` Docker/services, plus smoke UI Statistics/backoffice.
- [ ] **Statistics — séries temporelles erreurs & disponibilité (P0)** : vérifier fonctionnellement les graphes **taux d’erreur / min** et **disponibilité %** sur la fenêtre choisie — brancher / confirmer la **persistance réelle** (`system_metrics`, snapshots conteneurs, historique agrégateur) plutôt que dérivés locaux ou séries vides ; états **vide / partiel / source** explicites (comme Performances **A5**) ; même plage temporelle partagée (`usePersistedSharedAnalyticsRange` ou équivalent). À traiter dans un lot ultérieur, pas dans le merge moteur UI.
