...
**Dernière mise à jour : 15 mai 2026** — les tâches à faire passent en premier. Les succès techniques restent conservés plus bas, mais ils ne valent pas validation porteur. La validation finale est centralisée dans **`A_VALIDER_VERIFIER.md`**.
...
- [ ] **Backoffice navigation Performances — coût premier chargement** : investiguer et réduire le délai au clic depuis `/b4ck0ff1ce` vers `/b4ck0ff1ce/performances` observé en dev autour de **6,2 s** (`Fast Refresh` / compilation chunk). Ne pas retirer de graphes, de contrôles auth ou de protections HTTPS/WAF. **13/05 début** : ajout d'un `loading.tsx` dédié pour afficher un squelette immédiat pendant la navigation. **Reste** : mesurer le chunk route avec build/analyse, extraire les graphes Recharts inline restants de `performances/page.tsx` vers composants dynamiques client, conserver le prefetch/navigation des liens admin, puis valider en navigateur que Synthèse, Temps de réponse, Conteneurs, Disque, Réseau et Corrélation gardent leurs données.
...
- [ ] **Backoffice avec le mot de passe `.env`** : ouvrir `http://localhost:5003/login`, utiliser uniquement les valeurs locales `ADMIN_EMAIL` et `ADMIN_PASSWORD` du `.env`, puis confirmer l'accès à `http://localhost:5003/b4ck0ff1ce`. Si la connexion échoue après changement du `.env`, vérifier d'abord que la stack est lancée et que `auth-service` est prêt, puis resynchroniser l'admin sans afficher le secret : depuis `backend/auth-service`, lancer `npx prisma db seed` avec l'environnement du projet chargé, ou utiliser manuellement la cible de seed auth prévue pour le porteur. Le seed doit créer/mettre à jour le compte admin, marquer l'email comme vérifié et recalculer le hash du mot de passe ; ne jamais copier la valeur du mot de passe dans un ticket, un commit, un log ou une capture.
...
- [x] **Secrets PostgreSQL + `DATABASE_URL` hôte** : `env-generate-secrets.cjs --with-postgres` réécrit **`DATABASE_URL`** pour Prisma/scripts (`POSTGRES_CLIENT_HOST` / `localhost` + `POSTGRES_PORT`) ; Compose injecte toujours `@postgres:5432` dans les conteneurs. Doc **`scripts/env/README.md`**.
...
- [ ] **Connexion backoffice — validation navigateur porteur** : ouvrir `http://localhost:5003/login` avec `ADMIN_EMAIL` / `ADMIN_PASSWORD` du `.env`, puis confirmer l'accès à `http://localhost:5003/b4ck0ff1ce`. Si cela renvoie `401` ou reste bloqué, refaire le seed admin côté `auth-service` avant de conclure à un bug front.
...
- [ ] **Sécurité — logs sans plafond arbitraire** : remplacer les limites fixes UI par pagination/curseur exploitable (total réel, page suivante, filtres date/niveau/catégorie), afin que les vues sécurité ne donnent pas une fausse impression de “500 maximum”.
...
| **`/backoffice/performances/network`** | `frontend/src/app/(admin)/backoffice/performances/network/page.tsx` | **Partiel** : corrélation en `dynamic` (ex-`/backoffice/analytics/network`) |
