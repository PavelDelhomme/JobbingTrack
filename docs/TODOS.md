...
- [ ] **Backoffice avec le mot de passe `.env`** : ouvrir `http://localhost:5003/login`, utiliser `ADMIN_EMAIL` et `ADMIN_PASSWORD` du `.env`, puis confirmer l'accès à `http://localhost:5003/b4ck0ff1ce`. Si le mot de passe `.env` change, synchroniser d'abord l'admin via la cible de seed auth documentée dans les Makefiles.
...
- [ ] **Connexion backoffice — validation navigateur porteur** : ouvrir `http://localhost:5003/login` avec `ADMIN_EMAIL` / `ADMIN_PASSWORD` du `.env`, puis confirmer l'accès à `http://localhost:5003/b4ck0ff1ce`.
