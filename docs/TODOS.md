...
**Dernière mise à jour : 13 mai 2026** — **Validation BDD complète** : `make db-push-all` relancé sur la stack Docker, puis `make test-database` exécute maintenant la suite complète réelle (`test-database.js` + `test-postgresql-config.js` + `test-migrations.js`) en local avec `DATABASE_URL` hôte explicite. Résultats : 61 tables runtime, 121 contraintes, intégrité OK, config Postgres/docs/Prisma OK, db push sur base temporaire OK (**59 tables**, **58 FK**, **269 index**), table `log_collector_logs` présente, colonnes critiques présentes (`Application.isTestData`, `Application.isArchived`, `Application.thankYouEmailSentAt`, `Company.isTestData`), et test API archive/corbeille candidature **21/21**. Logs Postgres post-push : aucune erreur `isTestData` / `log_collector_logs` / `does not exist`. **CI/CD Node 24 + validation DB** : workflows GitHub alignés sur actions Node 24 (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`) avec `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` partout. La validation DB reçoit une `DATABASE_URL` de test au niveau job/workflow, le workflow dédié utilise `prisma db push` (pas `migrate deploy`) et le format Prisma n’échoue plus le job pour un simple écart de format. **Suite tests complète stabilisée** : équivalent direct de `make tests` exécuté sans `make` via `bash scripts/run-all-tests-with-reports.sh` avec gateway locale, Playwright smoke et `PERF_LIGHT=1` ; résultat **436/436 réussis**, **1 ignoré**. **Scripts metrics** : `scripts/testing/verify-all-metrics.sh` modernisé pour les ports actuels (`5004` metrics, `5002` gateway), `METRICS_API_KEY` optionnelle lue depuis l'environnement ou `.env`, et validé en réel sur la stack active : **52/52 réussis**, **0 échec**, **0 avertissement**.
...
- [x] **F1 sexies — Mobile E2E** : mode **smoke** activé par défaut dans l’agrégat (`PLAYWRIGHT_MOBILE_MODE=smoke`, timeout 240s) + suite **full** conservée à la demande ; validation locale smoke: `scripts/playwright-mobile-e2e.sh` = **3 pass** (iPhone 13 Pro, auth mobile).
- [x] **F1 septies — Frontend E2E smoke dans l’agrégat** : `scripts/playwright-frontend-e2e.sh` supporte `PLAYWRIGHT_FRONTEND_MODE=smoke|full` (smoke par défaut) + timeout agrégat réduit à 420s ; validation smoke locale = **31 pass / 3 skip / 0 fail** (`login + api-e2e + suivi-intérim`).
...
## Lot H — Hygiène dépôt : `scripts/`, `tools/`, `services/`, mobile
