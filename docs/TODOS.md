...
- [ ] **Corriger la cible prod scannable** : définir la bonne commande compose prod (fichier unique complet ou merge `docker-compose.yml + docker-compose.prod.yml`) pour rendre fiables `nmap` exposition prod et Trivy images prod.
...
- [ ] **Traiter les rapports sécurité** : classer findings `critical/high`, ouvrir tâches ciblées, justifier faux positifs avec date/environnement/commit, et reporter les résultats synthétiques dans `STATUS.md` / `security/STATS.md`.
