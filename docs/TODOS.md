...
- [ ] **Env sans défauts codés** : voir **HX1** en bas de fichier (audit `|| '…'` / ports magiques sur tout le monorepo). **12/05 partiel** : `make env-check` aligne `.env` / `.env.example` (**179/179 clés**) ; nouveau `make env-validate` signale sans afficher les valeurs les secrets faibles/placeholders, `NEXT_PUBLIC_*` sensibles, SMTP requis si alertes activées, et erreurs prod (`ALLOWED_ORIGINS` HTTPS, `WAF_ENABLED`, `ENABLE_METRICS_AUTH`, absence de bypass dev). Correctifs appliqués : `ENABLE_METRICS_AUTH` explicite, `metrics-aggregator` n’utilise plus `JWT_SECRET || your-secret-key`, `application-service` / `company-service` n’utilisent plus CORS `*` par défaut. **Reste** : générateur Makefile de secrets forts, suppression progressive des fallbacks `ADMIN_*`, migration des credentials mobile hors `NEXT_PUBLIC_*`, durcissement compose prod avec `${VAR:?…}`.
