...
Lors de la réinitialisation de la base de données ou lors du premier démarrage, la table `User` n'existe pas encore. Cela causait une erreur 500 lors de la tentative de connexion avec `admin@jobbingtrack.com` / `password123`.
...
if (!prisma.user || typeof prisma.user.findUnique !== 'function') {
  // Création utilisateur mock pour admin@jobbingtrack.com
  user = {
    id: 'dev_user_1',
...
- **Email Spécifique** : L'utilisateur mock n'est créé que pour `admin@jobbingtrack.com`
