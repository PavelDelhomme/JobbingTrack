...
| **Vue d’ensemble backoffice — KPI cohérents** | local / preprod | Carte **Sessions actives** : valeur, sous-titre et page de destination doivent refléter la **même** définition (sessions 30 min vs comptes activés) ; comptes E2E/test exclus ou clairement séparés ; pas d’écart type **2** sur la carte vs **~100** au clic. Voir `docs/pilotage/TODOS.md` § Sessions actives. | [ ] | Retour porteur 22/06 : 2 sessions, sous-titre 10 utilisateurs, clic → 100 comptes actifs (majorité E2E). |
