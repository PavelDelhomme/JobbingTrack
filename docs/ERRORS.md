...
**Dernière mise à jour** : 12 mai 2026 — **sécurité B11/B15/B14** : alertes email critiques backend branchées mais dépendantes de la config SMTP/interne ; alertes disponibilité `SERVICE_DOWN` ajoutées via metrics-aggregator avec dédup anti-flap ; audit `.env` outillé (`make env-validate`) et premiers retraits de fallbacks dangereux ; tests sécurité offensifs cadrés et partiellement outillés ; cible compose prod désormais scannable via merge `docker-compose.yml + docker-compose.prod.yml` avec profile `full`, mais le rapport ports montre encore trop de ports publiés en `0.0.0.0` pour une production. **Suite config 12/05** : `.env.production.example` séparé, `make env-validate-prod` réservé à `ENV_FILE=.env.production`, `make env-validate-prod-example` pour le modèle, scripts env centralisés, inventaire `scripts/`, prod HTTPS-only sur `ALLOWED_ORIGINS`, metrics auth active en dev par proxy serveur sans exposer `METRICS_API_KEY`, et HTTPS dev complet par proxy Nginx + CA locale navigateur. **Supply-chain 12/05** : première passe Dependabot réelle, audits runtime npm verts sur les principales surfaces après upgrades, dette `security-service` `geoip-lite`/`ip-address` conservée car la correction amont exige Node 24, et intégration Dependabot → BDD cadrée. **12/05** : les suites ciblées et applicatives liées au chantier sécurité/backoffice sont vertes (`security-service` 15/15, frontend Jest 86/86, Playwright sécurité 16/16, lint/type-check OK) ; tests ciblés disponibilité sécurité **10/10**. Historique actif : **6–7 mai** dette sécurité Compose/runtime (**B14**), analyse run `make tests` `tests/results/20260505-113157`, corrélation/logs/console ; **24 avril** `/health` frontend ; **7 avril** `type-check` / journal `tsc` ; **22 avril** `security/STATS.md`.
...
**GitHub Actions — Trivy action introuvable (12/05)** : l'erreur `Unable to resolve action aquasecurity/trivy-action@0.28.0` vient d'une référence sans préfixe `v`. Les tags Trivy publiés sont `v0.x.y`; le workflow sécurité référence maintenant `aquasecurity/trivy-action@v0.36.0`. Les avertissements Node 20 sont atténués par les actions officielles Node 24 et `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`, mais le résultat réel reste à confirmer dans GitHub Actions.

**Dependabot — alertes historiques vs checkout réel (12/05)** : les alertes GitHub `mobile/package-lock.json` ne correspondent pas au dépôt courant (`mobile/` est Flutter et ne contient pas de `package-lock.json`). Si elles restent ouvertes, vérifier la branche scannée par Dependabot ou un ancien lockfile supprimé. Côté surfaces présentes, les audits runtime npm sont verts pour `frontend`, `api-gateway`, `application-service`, `auth-service`, `deployment-service`, `workflow-service`; `security-service` conserve `geoip-lite`/`ip-address` parce que `geoip-lite` 2.x demande Node 24 et ne doit pas être appliqué tant que les conteneurs ne sont pas alignés.

**Auth-service Jest hors stack DB (12/05)** : après les upgrades npm, `backend/auth-service npm test -- --runInBand` échoue encore sur `tests/server.test.js` avec `Can't reach database server at localhost:5432`. Le test passe par le serveur réel et ne reprend pas le port Docker local `5000` dans ce contexte. Ce n'est pas lié aux upgrades `axios`/`nodemailer`, mais à la configuration DB de test à corriger comme cela a déjà été fait côté `security-service`.
