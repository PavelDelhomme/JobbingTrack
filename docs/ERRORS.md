...
**Dernière mise à jour** : 13 mai 2026 — **validation BDD complète + CI/CD** : l’ancien bruit Postgres `column Application.isTestData does not exist` est résolu sur la base Docker active après `make db-push-all`; les colonnes critiques `Application.isTestData`, `Application.isArchived`, `Application.thankYouEmailSentAt` et `Company.isTestData` sont présentes. `make test-database` exécute maintenant la suite DB complète en local avec `DATABASE_URL` explicite et passe : **60 tables**, **120 contraintes**, intégrité OK, config Postgres/docs/Prisma OK, base temporaire `prisma db push` OK (**58 tables**, **58 FK**, **225 index**). Le parcours API archive/corbeille candidature passe **21/21**, et les logs Postgres post-push/tests ne montrent plus `isTestData`, `does not exist`, `FATAL` ou `PANIC`. Les avertissements GitHub Actions Node 20 ont été traités côté workflows (`checkout@v6.0.2`, `setup-node@v6.4.0`, `upload-artifact@v7.0.1`, `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`). **Reste actif** : sécurité B11/B15/B14, scans P0 à trier, ports prod trop exposés, alertes SMTP/UI admin, et durcissement prod ; voir `TODOS.md`, `STATUS.md` et `PLAN.md`.
