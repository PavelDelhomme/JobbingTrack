...
**GitHub Actions — Trivy / Node 20 (12–13/05)** : l'erreur `Unable to resolve action aquasecurity/trivy-action@0.28.0` venait d'une référence sans préfixe `v` et a été corrigée avec `aquasecurity/trivy-action@v0.36.0`. Les workflows projet utilisent désormais des actions compatibles runtime Node 24 (`actions/checkout@v6.0.2`, `actions/setup-node@v6.4.0`, `actions/upload-artifact@v7.0.1`) et `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true`. À surveiller : résultat réel du prochain run GitHub Actions.
