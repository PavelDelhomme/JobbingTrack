...
- **Validation prod `.env` encore bloquante** : `make env-validate-prod` doit échouer tant que les secrets restent faibles/placeholders, que `ALLOWED_ORIGINS` contient du localhost, que `WAF_ENABLED=false`, que `ENABLE_METRICS_AUTH` n’est pas activé en prod, ou qu’un bypass dev reste défini. C’est un garde-fou volontaire, pas un bug.
- **Variables `NEXT_PUBLIC_*` sensibles** : tout mot de passe/token dans `NEXT_PUBLIC_*` est exposé au navigateur/build. Les credentials mobile/tests doivent migrer vers un runner serveur ou variables non publiques.
