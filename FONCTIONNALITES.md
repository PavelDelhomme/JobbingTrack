...
5. Backend → envoie email a `CRASH_REPORT_EMAIL` (par defaut: infos@delhomme.ovh)
...
- Destinataire configurable via `CRASH_REPORT_EMAIL` (defaut: infos@delhomme.ovh)
- **Tests / parcours** : pour les tests utilisateur ou E2E, on peut configurer `CRASH_REPORT_EMAIL=candidatures@delhomme.ovh` (ou autre boite accessible) pour recevoir les rapports de test
