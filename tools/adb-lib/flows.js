 ...
 *   await flows.login(adb, 'admin@jobbingtrack.com', 'password123');
...
async function login(adb, email = 'admin@jobbingtrack.com', password = 'password123') {
  await adb.wait(500);
...
async function forgotPassword(adb, email = 'admin@jobbingtrack.com') {
  await adb.tap('oubli');
  await adb.wait(2000);
  await adb.typeInField('Email', email);
  await adb.wait(500);
