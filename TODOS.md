...
- [ ] **Contrat logs forensics (gateway + services)** : propager et persister systématiquement `requestId`/`correlationId`, IP client, endpoint, méthode, protocole/port, code HTTP sur les logs `WARN/ERROR` pour rendre la corrélation incidents réellement exploitable (anti-DDoS, brute force, usurpation, injection). **30/04 partiel** : `auth-service` + `dashboard-service` + `security-service` enrichis via contexte requête auto-injecté dans le logger (et central logging quand disponible) ; reste : généraliser aux autres services + validation bout-en-bout dans la vue corrélation.
- [ ] **Observabilité qualité données corrélation** : afficher explicitement, par service, la couverture réelle des points **I/O** et la source **TR** (historique vs fallback stats) pour éviter de masquer les trous de collecte derrière des `--` ou des hypothèses.
- [ ] **Observabilité qualité données corrélation** : afficher explicitement, par service, la couverture réelle des points **I/O** et la source **TR** (historique vs fallback stats) pour éviter de masquer les trous de collecte derrière des `--` ou des hypothèses. **30/04 partiel** : panneau “Qualité des données” ajouté + parsing renforcé mémoire/réseau/I/O (bytes/MB/% dérivé), chargement multi-services dès l’ouverture, deltas moins sensibles aux séries non strictement cumulatives.
- [ ] **Observabilité qualité données corrélation** : afficher explicitement, par service, la couverture réelle des points **I/O** et la source **TR** (historique vs fallback stats) pour éviter de masquer les trous de collecte derrière des `--` ou des hypothèses. **30/04 partiel** : panneau “Qualité des données” ajouté + parsing renforcé mémoire/réseau/I/O (bytes/MB/% dérivé), chargement multi-services dès l’ouverture, deltas moins sensibles aux séries non strictement cumulatives, + fallback **live container stats** pour I/O/réseau/mémoire max quand l’historique est insuffisant.
- [ ] **Observabilité qualité données corrélation** : afficher explicitement, par service, la couverture réelle des points **I/O** et la source **TR** (historique vs fallback stats) pour éviter de masquer les trous de collecte derrière des `--` ou des hypothèses. **30/04 partiel** : panneau “Qualité des données” ajouté + parsing renforcé mémoire/réseau/I/O (bytes/MB/% dérivé), chargement multi-services dès l’ouverture, deltas moins sensibles aux séries non strictement cumulatives, + fallback **live container stats** pour I/O/réseau/mémoire max quand l’historique est insuffisant, + résumé service même sans historique (évite des lignes entièrement vides).
