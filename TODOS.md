...
- [ ] **Contrat logs forensics (gateway + services)** : propager et persister systématiquement `requestId`/`correlationId`, IP client, endpoint, méthode, protocole/port, code HTTP sur les logs `WARN/ERROR` pour rendre la corrélation incidents réellement exploitable (anti-DDoS, brute force, usurpation, injection). **30/04 partiel** : `auth-service`, `dashboard-service`, `security-service`, puis **`application-service`**, **`company-service`**, **`contact-service`** (middleware `requestContext`, logger enrichi, envoi central WARN/ERROR vers metrics-aggregator quand activé) ; reste : autres microservices + gateway + validation bout-en-bout dans la vue corrélation.
