...
- [ ] Monitoring C: remplacer les `popen` coûteux (`docker stats`/`inspect`/`curl`) par collecte non-forkée et checks parallèles. **07/05 partiel** : health checks sans `docker inspect`/`docker port`/`curl` shell (ports/paths connus + libcurl multi en parallèle), suppression du `docker ps` séparé et de la substitution `docker ps` dans `docker stats` (un seul `popen docker stats`, filtrage `jobbingtrack-` côté C) ; reste API Docker/cgroups pour supprimer le dernier fork.
