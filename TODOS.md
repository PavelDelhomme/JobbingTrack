...
- [ ] **F1 ter — API Gateway Jest** : adapter `tests/server.test.js` (CORS avec/ sans `Origin`) et `tests/controllers/admin.controller.test.js` (route logs admin sous auth) aux règles actuelles ; cible : repasser `backend/api-gateway` à 22/22.
- [ ] **F1 quater — Frontend Jest analytics** : mettre à jour/remplacer `src/app/(admin)/backoffice/analytics/__tests__/tab-components.test.tsx` pour tester le hub actuel (`analytics/page.tsx`) au lieu de l’ancienne page graphique.
- [ ] **F1 quinquies — Playwright E2E login + suivi-intérim** : stabiliser `tests/e2e/login.spec.ts` (token après submit, invalid login, toggle mot de passe) et `tests/e2e/suivi-interim.spec.ts` (sélecteur menu « Gestion des données »).
- [ ] **F1 sexies — Mobile E2E** : `playwright-mobile` sort en timeout global (exit 124, 600s) ; ajouter un mode smoke court dans l’agrégat ou isoler la suite longue hors `make tests` par défaut.
