...
- [ ] **Contrat logs forensics (gateway + services)** : propager et persister systématiquement `requestId`/`correlationId`, IP client, endpoint, méthode, protocole/port, code HTTP sur les logs `WARN/ERROR` pour rendre la corrélation incidents réellement exploitable (anti-DDoS, brute force, usurpation, injection). **05/2026 partiel** : services ci-dessus + **`interview-service`**, **`call-service`**, **`followup-service`**, **`event-service`**, **`profile-service`**, **`notification-service`**, **`deployment-service`** (même schéma : `requestContext`, Winston + `centralLogger`, CORS headers corrélation, `trust proxy`) ; **reste** : **`api-gateway`** (enrichissement logs proxy au-delà de `X-Request-Id` / forward headers), éventuellement **`workflow-service`**, validation bout-en-bout vue corrélation.
...
- Poursuivre **contrat logs forensics** (**`api-gateway`** puis **`workflow-service`** si pertinent) et **corrélation perf** (brush, causalité, icônes tri) listés en haut de ce fichier (**§ Suite & suivi explicite**).
