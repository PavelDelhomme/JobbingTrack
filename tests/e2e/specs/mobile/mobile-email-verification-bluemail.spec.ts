 ...
 *
 * Lancer : cd tests && TEST_REAL_EMAILS=<adresse configuree hors Git> npx playwright test e2e/specs/mobile/mobile-email-verification-bluemail.spec.ts --project=chromium
 */
...
const BLUEMAIL_EMAIL = process.env.TEST_REAL_EMAILS?.split(',').map((s) => s.trim()).find(Boolean) || requireEnv('TEST_REAL_EMAIL');
