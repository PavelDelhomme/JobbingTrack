 ...
 * Lancer : cd tests && TEST_REAL_EMAILS=paul.delhomme@proton.me npx playwright test e2e/specs/mobile/mobile-email-verification-proton.spec.ts --project=chromium
 */
...
const PROTON_EMAIL = process.env.TEST_REAL_EMAILS?.split(',').map((s) => s.trim()).find((e) => e.includes('proton')) || process.env.TEST_REAL_EMAIL || 'paul.delhomme@proton.me';
...
test.describe('Mobile – Vérification email Proton', () => {
  test('inscription Proton → email vérification → verify → login', async ({ request }) => {
    const adminToken = await loginAdmin(request);
    const testEmail = PROTON_EMAIL;

    const registerRes = await request.post(`${GATEWAY_URL}/api/v1/auth/register`, {
      data: { email: testEmail, password: TEST_PASSWORD, firstName: 'Test', lastName: 'Proton', phone: '+33600000000' },
    });
