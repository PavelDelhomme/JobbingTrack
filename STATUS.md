...
- **Migration Rust monitoring (07/05)** : `monitoring/rust/crates/log-collector` porte le contrat principal du collecteur logs (scan Docker logs, offsets, parsing JSON Docker, persistance `log_collector_logs`, `/health`, `/api/v1/logs`). `monitoring/rust/crates/monitoring-agent` expose aussi un conteneur Rust optionnel avec `/health` + `/api/v1/metrics` système minimal compatible. Services Compose optionnels `log-collector-rs` (`5109`) et `monitoring-agent-rs` (`5118`) sous profil `monitoring-rust`, non activés par défaut. Smoke local OK sur les deux conteneurs ; conteneurs optionnels arrêtés après validation. Reste avant remplacement du C : métriques conteneurs/cgroups, réseau, health checks services et persistance/historique côté `monitoring-agent-rs`.
