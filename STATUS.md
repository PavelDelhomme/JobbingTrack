...
- **`monitoring-c` (perf, partiel)** : les health checks ne forkent plus `docker inspect` / `docker port` / `curl` ; `collector.c` construit les URL via ports/paths connus et mesure avec **libcurl multi** en parallèle. Le comptage `docker ps` séparé et la substitution `docker ps` dans `docker stats` sont supprimés (un seul `docker stats`, filtrage C). **Reste** : remplacer ce dernier fork par API Docker/cgroups.
