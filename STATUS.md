...
- **`monitoring-c` (perf)** : plus aucun `popen` dans `ex-systems/monitoring-c`. L’inventaire conteneurs passe par le Docker socket Unix sans shell, les métriques CPU/mémoire/réseau par cgroups/proc, et les health checks par **libcurl multi** en parallèle. QA Docker OK : service `healthy`, `/api/v1/metrics` répond, sauvegardes PostgreSQL actives.
- **Budget ressources monitoring (baseline 07/05, 40 min)** : `make resource-budget-sample` → `tests/results/resource-budget/20260507-123612/summary.md`. Résultat : `metrics-aggregator` trop haut (**CPU p95 93.04%, RAM p95 588.4 MB**) et `frontend` trop haut (**CPU p95 19.61%, RAM p95 868.4 MB**) ; collecteurs C OK (`monitoring-c` **1.22% / 2.7 MB**, `log-collector-c` **0% / 5.1 MB**). Correctifs appliqués/en validation : fallback Docker aggregator désactivé par défaut si `monitoring-c` répond, collecte Docker logs aggregator désactivée (source dédiée = `log-collector-c`), cache `/api/v1/docker/services/all`, `docker stats` groupé côté route Docker, disponibilité persistée au plus toutes les 60s/service, rafraîchissement backoffice services ≥ 60s et métriques ≥ 30s.
