...
1. **Démarrer la stack** (dans le terminal, à la racine du projet) — **une seule commande** (inclut la création du super admin) :
   ...
   ```
   Attendre que chaque étape se termine (21/21 services UP à la fin). `make create-admin-user` crée ou met à jour l’utilisateur **SUPER_ADMIN** (admin@jobbingtrack.test / password123).
...
2. **Ouvrir le backoffice** (navigateur) : URL du front (ex. http://localhost:5003 ou celle configurée). Se connecter avec **admin@jobbingtrack.test** / **password123**.
   ...
   **Si vous voyez « Accès Refusé – Votre rôle actuel : USER »** : l’utilisateur admin existe en BDD mais avec le rôle USER. Corriger en lançant une fois :
   ...
   ```
   Puis se **déconnecter** et se **reconnecter** (ou rafraîchir après déconnexion). Le script met à jour le rôle en **SUPER_ADMIN** pour admin@jobbingtrack.test.
...
**Symptôme** : Connexion avec admin@jobbingtrack.test OK, mais message « Accès Refusé – Votre rôle actuel : USER ». Le backoffice exige les rôles **ADMIN** ou **SUPER_ADMIN**.

**Cause** : L’utilisateur admin@jobbingtrack.test existe en base mais avec le rôle **USER** (création par inscription ou ancien seed). `make up-full` ne fait que vérifier qu’un utilisateur avec cet email existe ; il ne corrige pas le rôle.

**Solution** :
1. À la racine du projet : `make create-admin-user`
2. Se déconnecter du backoffice, puis se reconnecter avec admin@jobbingtrack.test / password123.
...
| 5 | `make create-admin-user` | **Crée ou met à jour** l’utilisateur super admin (admin@jobbingtrack.test / password123, rôle **SUPER_ADMIN**). Sans cette étape, l’admin peut exister en **USER** et le backoffice affiche « Accès Refusé ». |
...
- Lancer la **séquence complète** (section « DÉMARRAGE PROJET » ci-dessus) :  
  `make up-full && make db-push-all && make build && make up-full && make create-admin-user && make status`
- Se connecter au backoffice : admin@jobbingtrack.test / password123.
...
- **Symptôme** : Connexion avec admin@jobbingtrack.test OK mais message « Accès Refusé – Votre rôle actuel : USER ».
...
- **Reply-To** : `SMTP_REPLY_TO=noreply@jobbingtrack.test` ; headers `Auto-Submitted: auto-generated` et `X-Auto-Response-Suppress: All`.
...
4. **Créer le compte test** : Utiliser Backoffice → Utilisateurs → « Nouvel utilisateur » pour créer `test@example.invalid` (formulaire création opérationnel).
