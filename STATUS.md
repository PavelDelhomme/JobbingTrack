...
- **`monitoring-c` (perf, partiel)** : les health checks ne forkent plus `docker inspect` / `docker port` / `curl` ; `collector.c` construit les URL via ports/paths connus et mesure avec **libcurl**. **Reste** : supprimer `docker stats`/`docker ps` shell et paralléliser les checks.
