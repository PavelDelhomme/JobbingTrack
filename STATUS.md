...
- **Services C actifs promus à la racine (07/05)** : `monitoring-c/` et `log-collector-c/` ne sont plus sous `ex-systems/` car ils sont utilisés par Compose/Makefile en stack locale. `monitoring-c` est la source bas niveau consommée par `jobbingtrack-metrics-aggregator` (`MONITORING_C_URL=http://monitoring-c:8015`) ; `log-collector-c` est la source dédiée logs Docker/PostgreSQL. Les chemins Compose/docs/scripts utilisent désormais `./monitoring-c`, `./log-collector-c` et `monitoring-c/sql/...`.
