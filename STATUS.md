...
**Comportement connu** : après `make up-full`, la BDD peut être vide. La création automatique de l’admin est activée par défaut ; si elle échouait (ex. colonne `id` manquante dans l’INSERT), c’est corrigé. Sinon lancer **`make create-admin-user`** puis se connecter avec `admin@jobbingtrack.com` / `password123`.
