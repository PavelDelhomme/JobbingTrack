...
# Script de démarrage minimal pour tester les métriques

echo "🚀 Démarrage minimal pour tester les métriques"
echo "=============================================="
...
# Démarrer seulement les services essentiels
echo ""
...
# Démarrer l'API Gateway sans les middlewares problématiques
echo ""
echo "🌐 Démarrage de l'API Gateway (mode minimal)..."

# Créer une version minimale du serveur API Gateway
cat > /tmp/server-minimal.js << 'EOF'
...
# Démarrer l'API Gateway en mode minimal
echo ""
echo "🚀 Démarrage de l'API Gateway en mode minimal..."
docker run -d --name api-gateway-minimal \
  -p 3000:3000 \
  --network jobbingtrack_default \
  -v /tmp/server-minimal.js:/app/server.js \
  node:20-alpine sh -c "cd /app && node server.js"
...
echo ""
echo "✅ Système de métriques minimal démarré!"
echo ""
echo "💡 Pour arrêter :"
echo "   docker stop api-gateway-minimal"
