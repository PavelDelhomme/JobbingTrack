...
function main() {
  ...
  const envFile = args.includes('--example') ? EXAMPLE_PATH : ENV_PATH;
  ...
  for (const [key, value] of values.entries()) {
    ...
    if (weakCheckKeys.has(key) && hasWeakValue(value)) {
  ...
  if (productionLike) {
    if (allowedOrigins.includes('localhost') || allowedOrigins.includes('127.0.0.1') || allowedOrigins.includes('*')) {
      errors.push('ALLOWED_ORIGINS ne doit pas contenir localhost, 127.0.0.1 ou * en production');
    }
    if (!allowedOrigins.split(',').some((origin) => origin.trim().startsWith('https://'))) {
      errors.push('ALLOWED_ORIGINS doit contenir au moins une origine HTTPS en production');
    }
