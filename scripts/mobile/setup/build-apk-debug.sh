...
# Un seul flavor à la fois — sans --flavor, Flutter assemble les 3 canaux
# (dev/preprod/prod) et package*Debug peut échouer avec !zip.isFile().
FLAVOR="${FLAVOR:-dev}"
...
run_build() {
  set +e
  "$FLUTTER_BIN" build apk --debug --flavor "$FLAVOR" "${DART_DEFINES[@]}" 2>&1 | tee "$LOG"
...
if ! run_build; then
  if grep -qiE 'kernel_blob|compressDebugAssets|already contains entry|zip\.isFile' "$LOG"; then
    ...
    "$FLUTTER_BIN" pub get
    if ! run_build; then
      echo "[build-apk-debug] ÉCHEC après retry Zip — voir log ci-dessus" >&2
      exit 1
    fi
  else
...
OUT="$MOBILE_DIR/build/app/outputs/flutter-apk/app-${FLAVOR}-debug.apk"
if [[ ! -f "$OUT" ]]; then
  OUT="$MOBILE_DIR/build/app/outputs/flutter-apk/app-debug.apk"
fi
# Alias stable pour les scripts d’install (reinstall-apk-adb, etc.)
STABLE="$MOBILE_DIR/build/app/outputs/flutter-apk/app-debug.apk"
if [[ -f "$OUT" && "$OUT" != "$STABLE" ]]; then
  mkdir -p "$(dirname "$STABLE")"
  cp -f "$OUT" "$STABLE"
fi
echo "[build-apk-debug] OK → $OUT (alias $STABLE)"
