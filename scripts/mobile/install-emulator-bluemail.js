 ...
 *   node scripts/mobile/install-emulator-bluemail.js --sideload-only
 *
...
(async () => {
  ...
  if (!sideloadOnly && hasStore) {
    ...
    if (pkg) {
      await openBlueMail(phone, pkg);
      console.log('OK — lancez : node scripts/mobile/configure-emulator-bluemail.js');
      return;
    }
  ...
  if (sideloaded) {
    ...
    await openBlueMail(phone, pkg);
    console.log('OK (sideload) — lancez : node scripts/mobile/configure-emulator-bluemail.js');
    return;
  }
  ...
  console.error('  node scripts/mobile/install-emulator-bluemail.js');
  console.error('');
  console.error('Option B — APK manuel :');
  console.error(`  Copier me.bluemail.mail.apk dans ${APK_DIR}/`);
  console.error('  node scripts/mobile/install-emulator-bluemail.js --sideload-only');
  process.exit(2);
})().catch((err) => {
