...
full_check() {
    ...
    if docker-compose exec postgres psql -U jobbingtrack -d jobbingtrack -c "SELECT 1" > /dev/null 2>&1; then
        ...
        if docker-compose exec postgres psql -U jobbingtrack -d jobbingtrack -c "SELECT id FROM \"User\" WHERE email = 'admin@jobbingtrack.test'" | grep -q "admin_"; then
...
case "$ACTION" in
    ...
    "help"|*)
        ...
        echo "  SUPER_ADMIN_EMAIL    - Email de l'admin (défaut: admin@jobbingtrack.test)"
