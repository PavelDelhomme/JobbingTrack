...
- ✅ Domaine `example.invalid` (déjà configuré, à ne pas toucher)
- ✅ Email existant `noreply@example.invalid` (mais vous ne voulez pas l'utiliser)

**Vous voulez** :
- ✅ Envoyer des emails automatiques (reset password, vérification compte)
- ✅ Ne PAS dévoiler `example.invalid` publiquement
...
1. **Séparation claire** 🎯
   ...
   - Personnel → `example.invalid` (privé)

2. **Aucune interférence** 🔒
   - Pas de risque de toucher à votre config `example.invalid`
   - Configuration dédiée et isolée

3. **Professionnalisme** 💼
   - `redacted@example.invalid` est générique et pro
...
4. **Sélectionner le domaine `maily.ovh`** (pas example.invalid !)
...
**Résultat** : Vous avez maintenant `redacted@example.invalid` ✅
...
│                                                         │
│  Domaine personnel (example.invalid) :                    │
│  → PAS utilisé ✅                                       │
│  → PAS dévoilé ✅                                       │
...
- [ ] J'ai créé `redacted@example.invalid` chez OVH
...
2. Email `redacted@example.invalid` pas créé chez OVH
...
**🎉 Vous êtes prêt ! Suivez les étapes et vos emails partiront via `redacted@example.invalid` sans dévoiler `example.invalid` !**
